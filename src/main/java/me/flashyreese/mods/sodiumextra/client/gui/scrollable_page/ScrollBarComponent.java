...
public class ScrollBarComponent extends AbstractWidget {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      this.drawBorder(context, this.dim.x(), this.dim.y(), this.dim.getLimitX(), this.dim.getLimitY(), -5592406);
      this.drawRect(context, this.scrollThumb.x(), this.scrollThumb.y(), this.scrollThumb.getLimitX(), this.scrollThumb.getLimitY(), -5592406);
   }
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (this.dim.containsCursor(mouseX, mouseY)) {
         if (this.scrollThumb.containsCursor(mouseX, mouseY)) {
            ...
            int value = (int)(
               (mouseY - this.dim.y() - this.scrollThumb.height() / 2) / (this.dim.height() - this.scrollThumb.height()) * this.maxScrollBarOffset
            );
   ...
   public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
      if (this.isDragging) {
         int value = (int)(
            (mouseY - this.scrollThumbClickOffset - this.dim.y() - this.scrollThumb.height() / 2)
               / (this.dim.height() - this.scrollThumb.height())
               * this.maxScrollBarOffset
         );
         this.setOffset(value);
         return true;
      } else {
   ...
   public boolean mouseScrolled(double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
      if ((this.dim.containsCursor(mouseX, mouseY) || this.extendedScrollArea != null && this.extendedScrollArea.containsCursor(mouseX, mouseY))
         ...
         && this.offset >= 0) {
         int value = (int)(this.offset - verticalAmount * 6.0);
         this.setOffset(value);
         return true;
      } else {
         return false;
