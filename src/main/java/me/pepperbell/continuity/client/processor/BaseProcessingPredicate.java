...
public class BaseProcessingPredicate implements ProcessingPredicate {
	...
	public static class BiomeCache {
		...
		public Biome get(BlockRenderView blockView, BlockPos pos) {
			if (invalid) {
				if (blockView instanceof LevelSlice slice) {
					biome = slice.getBiomeFabric(pos).value();
				} else if (blockView instanceof WorldView worldView) {
					biome = worldView.getBiome(pos).value();
				} else {
					biome = null;
				}
				invalid = false;
