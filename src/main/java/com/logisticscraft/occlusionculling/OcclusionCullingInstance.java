...
public class OcclusionCullingInstance {
   ...
   public boolean isAABBVisible(Vec3d aabbMin, Vec3d aabbMax, Vec3d viewerPosition) {
      try {
         ...
         if (relX == OcclusionCullingInstance.Relative.INSIDE
            && relY == OcclusionCullingInstance.Relative.INSIDE
            && relZ == OcclusionCullingInstance.Relative.INSIDE) {
            return true;
         }
         ...
         for (int x = minX; x <= maxX; x++) {
            ...
            visibleOnFaceX = (byte)(visibleOnFaceX | (x == minX && relX == OcclusionCullingInstance.Relative.POSITIVE ? 1 : 0));
            ...
            for (int y = minY; y <= maxY; y++) {
               ...
               visibleOnFaceY = (byte)(visibleOnFaceY | (y == minY && relY == OcclusionCullingInstance.Relative.POSITIVE ? 4 : 0));
               ...
               for (int z = minZ; z <= maxZ; z++) {
                  ...
                  visibleOnFace = (byte)(visibleOnFace | (z == minZ && relZ == OcclusionCullingInstance.Relative.POSITIVE ? 16 : 0));
   ...
   private boolean rayIntersection(int[] b, Vec3d rayOrigin, Vec3d rayDir) {
      ...
      return tmax > 0.0 ? false : !(tmin > tmax);
   }
   ...
   private int getCacheValue(int x, int y, int z) {
      ...
      y -= this.cameraPos[1];
      z -= this.cameraPos[2];
      return Math.abs(x) <= this.reach - 2 && Math.abs(y) <= this.reach - 2 && Math.abs(z) <= this.reach - 2
         ? this.cache.getState(x + this.reach, y + this.reach, z + this.reach)
         : -1;
   }
