...
public class ItemPacketRewriter1_9 extends ItemRewriter<ClientboundPackets1_8, ServerboundPackets1_9, Protocol1_8To1_9> {
    ...
    public ItemPacketRewriter1_9(final Protocol1_8To1_9 protocol) {
        ...
        for (final Entry<String, JsonElement> entry : itemIdentifiers.entrySet()) {
        ...
        for (final Entry<String, JsonElement> itemEntry : itemAttributes.entrySet()) {
            final Map<String, Pair<String, ModifierData>> attributes = new HashMap<>();
            for (final Entry<String, JsonElement> attributeEntry : itemEntry.getValue().getAsJsonObject().entrySet()) {
                final JsonObject data = attributeEntry.getValue().getAsJsonObject();
                final ModifierData modifier = new ModifierData(
                    UUID.fromString(data.get("id").getAsString()), data.get("name").getAsString(),
                    data.get("amount").getAsDouble(), data.get("operation").getAsInt()
                );
                attributes.put(attributeEntry.getKey(), new Pair<>(data.get("slot").getAsString(), modifier));
            }
            viaFabricPlus$itemAttributes.put(itemEntry.getKey(), attributes);
        }
    }
    ...
    public @Nullable Item handleItemToClient(final UserConnection connection, @Nullable final Item item) {
        ...
        viaFabricPlus$addAttributeFixData(item);
        return item;
    }
    ...
    public @Nullable Item handleItemToServer(final UserConnection connection, @Nullable final Item item) {
        ...
        viaFabricPlus$removeAttributeFixData(item);
        return item;
    }

    private void viaFabricPlus$addAttributeFixData(final Item item) {
        ...
        if (tag.getListTag("AttributeModifiers", CompoundTag.class) != null) return;

        final ListTag<CompoundTag> modifiers = new ListTag<>(CompoundTag.class);
        for (final Entry<String, Pair<String, ModifierData>> entry : attributes.entrySet()) {
            final ModifierData data = entry.getValue().value();
            final CompoundTag modifier = new CompoundTag();
            modifier.putString("AttributeName", entry.getKey());
            modifier.putString("Name", data.name());
            modifier.putDouble("Amount", data.amount());
            modifier.putInt("Operation", data.operation());
            modifier.putLong("UUIDMost", data.uuid().getMostSignificantBits());
            modifier.putLong("UUIDLeast", data.uuid().getLeastSignificantBits());
            modifier.putString("Slot", entry.getValue().key());
    ...
    private void viaFabricPlus$removeAttributeFixData(final Item item) {
        final CompoundTag tag = item.tag();
        if (tag == null) return;

        final CompoundTag attributeFixTag = tag.removeUnchecked(nbtTagName("attributeFix"));
        if (attributeFixTag == null) return;
        if (attributeFixTag.contains("RemoveAttributeModifiers")) tag.remove("AttributeModifiers");
        if (attributeFixTag.contains("RemoveTag")) item.setTag(null);
    }
}
