...
public class GlStateManager {
   private static final boolean ON_LINUX = Util.getOperatingSystem() == OperatingSystem.LINUX;
   ...
   public static GlStateManager.Texture2DState[] TEXTURES = IntStream.range(0, 128)
      .mapToObj(i -> new GlStateManager.Texture2DState())
      .toArray(GlStateManager.Texture2DState[]::new);
   private static final GlStateManager.ColorMask COLOR_MASK = new GlStateManager.ColorMask();

   static {
      StateUpdateNotifiers.blendFuncNotifier = listener -> iris$blendFuncListener = listener;
   }

   public static GlStateManager.BlendFuncState iris$getBlendState() {
      return BLEND;
   }

   public static GlStateManager.DepthTestState iris$getDepthState() {
      return DEPTH;
   }

   public static GlStateManager.ColorMask iris$getColorMask() {
      return COLOR_MASK;
   }
   ...
   public static void _blendFuncSeparate(int srcFactorRGB, int dstFactorRGB, int srcFactorAlpha, int dstFactorAlpha) {
      if (BlendModeStorage.isBlendLocked()) {
         BlendModeStorage.deferBlendFunc(srcFactorRGB, dstFactorRGB, srcFactorAlpha, dstFactorAlpha);
         return;
      }
   ...
   public static void _glUseProgram(int program) {
      boolean skipBind = iris$program == 0 && program == 0;
   ...
   public static void _glDeleteFramebuffers(int framebuffer) {
      RenderSystem.assertOnRenderThreadOrInit();
      READ_FRAMEBUFFER.invalidate(framebuffer);
      DRAW_FRAMEBUFFER.invalidate(framebuffer);
   ...
   private static void iris$onDeleteTexture(int texture) {
   ...
   public static class BlendFuncState {
   ...
   public static class CapabilityTracker implements BooleanStateAccessor, BooleanStateExtended {
   ...
   static class FramebufferState {
      ...
      public void invalidate(int framebuffer) {
