...
public class VertexSerializerFactory {
   ...
   public static VertexSerializerFactory.Bytecode generate(VertexFormat srcFormat, VertexFormat dstFormat, String identifier) {
      List<VertexSerializerFactory.MemoryTransfer> memoryCopies = createMemoryTransferList(srcFormat, dstFormat);
      String name = "net/caffeinemc/mods/sodium/client/render/vertex/serializers/generated/VertexSerializer$Impl$" + identifier;
      ...
      methodVisitorx.visitVarInsn(22, 1);
      methodVisitorx.visitLdcInsn((long)srcFormat.getVertexSizeByte());
      methodVisitorx.visitInsn(97);
      ...
      methodVisitorx.visitVarInsn(22, 3);
      methodVisitorx.visitLdcInsn((long)dstFormat.getVertexSizeByte());
      methodVisitorx.visitInsn(97);
   ...
   private static List<VertexSerializerFactory.MemoryTransfer> createMemoryTransferList(VertexFormat srcVertexFormat, VertexFormat dstVertexFormat) {
      ArrayList<VertexSerializerFactory.MemoryTransfer> ops = new ArrayList<>();

      for (int elementIndex = 0; elementIndex < 32; elementIndex++) {
         VertexFormatElement elementType = VertexFormatElement.get(elementIndex);
         if (elementType != null && dstVertexFormat.has(elementType)) {
            if (!srcVertexFormat.has(elementType)) {
               throw new RuntimeException("Source format is missing element %s as required by destination format".formatted(elementType));
            }

            int srcOffset = srcVertexFormat.getOffset(elementType);
            int dstOffset = dstVertexFormat.getOffset(elementType);
            ops.add(new VertexSerializerFactory.MemoryTransfer(srcOffset, dstOffset, elementType.getSizeInBytes()));
         }
      }
