...
public class BlockRenderer extends AbstractBlockRenderContext {
   ...
   public void renderModel(BakedModel model, BlockState state, BlockPos pos, BlockPos origin) {
      ...
      Iterable<RenderLayer> renderTypes = PlatformModelAccess.getInstance().getModelRenderTypes(this.level, model, state, pos, this.random, this.modelData);
      this.allowDowngrade = true;
      Iterator<RenderLayer> it = renderTypes.iterator();

      for (RenderLayer defaultType = RenderLayers.getBlockLayer(state);
         it.hasNext();
         ((FabricBakedModel)model).emitBlockQuads(this.getEmitter(), this.level, state, pos, this.randomSupplier, this::isFaceCulled)
      ) {
         this.type = it.next();
   ...
   private boolean validateQuadUVs(Sprite atlasSprite) {
      float spriteUMin = atlasSprite.getMinU();
      float spriteUMax = atlasSprite.getMaxU();
      float spriteVMin = atlasSprite.getMinV();
      float spriteVMax = atlasSprite.getMaxV();
   ...
   @Nullable
   private TerrainRenderPass attemptPassDowngrade(Sprite sprite, TerrainRenderPass pass) {
   ...
   private static TerrainRenderPass getDowngradedPass(Sprite sprite, TerrainRenderPass pass) {
      if (sprite instanceof TextureAtlasSpriteExtension spriteExt) {
         if (spriteExt.sodium$hasUnknownImageContents()) {
            return pass;
         }

         if (sprite.getContents() instanceof SpriteContentsExtension contentsExt) {
