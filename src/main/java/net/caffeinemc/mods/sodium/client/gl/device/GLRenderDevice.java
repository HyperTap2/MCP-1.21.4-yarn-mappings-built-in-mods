...
public class GLRenderDevice implements RenderDevice {
   ...
   private final CommandList commandList = new GLRenderDevice.ImmediateCommandList(this.stateTracker);
   private final DrawCommandList drawCommandList = new GLRenderDevice.ImmediateDrawCommandList();
   ...
   private class ImmediateCommandList implements CommandList {
      ...
      public void copyBufferSubData(GlBuffer src, GlBuffer dst, long readOffset, long writeOffset, long bytes) {
         ...
         GL31C.glCopyBufferSubData(36662, 36663, readOffset, writeOffset, bytes);
      }
      ...
      public GlBufferMapping mapBuffer(GlBuffer buffer, long offset, long length, EnumBitField<GlBufferMapFlags> flags) {
         ...
         } else if (flags.contains(GlBufferMapFlags.PERSISTENT) && !(buffer instanceof GlImmutableBuffer)) {
   ...
   private class ImmediateDrawCommandList implements DrawCommandList {
      ...
      public void multiDrawElementsBaseVertex(MultiDrawBatch batch, GlIndexType indexType) {
         ...
         GL32C.nglMultiDrawElementsBaseVertex(
            ImmediateState.usingTessellation ? 14 : primitiveType.getId(),
            batch.pElementCount,
