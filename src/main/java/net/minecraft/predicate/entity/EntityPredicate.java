...
public record EntityPredicate(
   ...
   public boolean test(ServerWorld world, @Nullable Vec3d pos, @Nullable Entity entity) {
      ...
      if (this.passenger.isPresent() && entity.getPassengerList().stream().noneMatch(entityx -> this.passenger.get().test(world, pos, entityx))) {
         return false;
      }
      ...
      return this.slots.isPresent() && !this.slots.get().matches(entity) ? false : !this.nbt.isPresent() || this.nbt.get().test(entity);
   }
