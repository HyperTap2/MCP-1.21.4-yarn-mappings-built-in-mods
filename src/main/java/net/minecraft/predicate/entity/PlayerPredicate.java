...
public record PlayerPredicate(
   ...
   public boolean test(Entity entity, ServerWorld world, @Nullable Vec3d pos) {
      if (!(entity instanceof ServerPlayerEntity serverPlayerEntity)) {
         ...
         if (!this.gameMode.contains(serverPlayerEntity.interactionManager.getGameMode())) {
            return false;
         }
         ...
         while (var13.hasNext()) {
            Entry<RegistryKey<Recipe<?>>> entry = (Entry<RegistryKey<Recipe<?>>>)var13.next();
         ...
         if (!this.advancements.isEmpty()) {
            ...
            for (java.util.Map.Entry<Identifier, PlayerPredicate.AdvancementPredicate> entry2 : this.advancements.entrySet()) {
               ...
               if (advancementEntry == null || !entry2.getValue().test(playerAdvancementTracker.getProgress(advancementEntry))) {
                  return false;
               }
            }
         }
         ...
         if (this.lookingAt.isPresent()) {
            ...
            EntityHitResult entityHitResult = ProjectileUtil.getEntityCollision(
               ...
               vec3d,
               vec3d3,
               new Box(vec3d, vec3d3).expand(1.0),
