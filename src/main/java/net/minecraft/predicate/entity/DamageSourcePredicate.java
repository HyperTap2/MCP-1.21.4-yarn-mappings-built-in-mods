...
public record DamageSourcePredicate(
   ...
   public boolean test(ServerWorld world, Vec3d pos, DamageSource damageSource) {
      ...
      if (this.directEntity.isPresent() && !this.directEntity.get().test(world, pos, damageSource.getSource())) {
         ...
         return this.sourceEntity.isPresent() && !this.sourceEntity.get().test(world, pos, damageSource.getAttacker())
            ? false
   ...
   public static class Builder {
      private final com.google.common.collect.ImmutableList.Builder<TagPredicate<DamageType>> tagPredicates = ImmutableList.builder();
