...
public record BlockPredicate(Optional<RegistryEntryList<Block>> blocks, Optional<StatePredicate> state, Optional<NbtPredicate> nbt) {
   ...
   public boolean test(ServerWorld world, BlockPos pos) {
      if (!world.isPosLoaded(pos)) {
         ...
         return !this.testState(world.getBlockState(pos)) ? false : !this.nbt.isPresent() || testBlockEntity(world, world.getBlockEntity(pos), this.nbt.get());
      }
   }

   public boolean test(CachedBlockPosition pos) {
      return !this.testState(pos.getBlockState()) ? false : !this.nbt.isPresent() || testBlockEntity(pos.getWorld(), pos.getBlockEntity(), this.nbt.get());
   }
