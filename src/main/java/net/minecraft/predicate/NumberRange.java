...
public interface NumberRange<T extends Number> {
   ...
   static <T extends Number, R extends NumberRange<T>> Codec<R> createCodec(Codec<T> valueCodec, NumberRange.Factory<T, R> rangeFactory) {
      Codec<R> codec = RecordCodecBuilder.create(
         instance -> instance.group(
               valueCodec.optionalFieldOf("min").forGetter(NumberRange::min), valueCodec.optionalFieldOf("max").forGetter(NumberRange::max)
            )
            .apply(instance, rangeFactory::create)
      );
      return Codec.either(codec, valueCodec)
         .xmap(either -> either.map(range -> range, value -> rangeFactory.create(Optional.of((T)value), Optional.of((T)value))), range -> {
   ...
   record DoubleRange(Optional<Double> min, Optional<Double> max, Optional<Double> squaredMin, Optional<Double> squaredMax) implements NumberRange<Double> {
      public static final NumberRange.DoubleRange ANY = new NumberRange.DoubleRange(Optional.empty(), Optional.empty());
       public static final Codec<NumberRange.DoubleRange> CODEC = NumberRange.createCodec(Codec.DOUBLE, (Optional<Double> min, Optional<Double> max) -> new DoubleRange(min, max));
      ...
      public boolean testSqrt(double value) {
         return this.squaredMin.isPresent() && this.squaredMin.get() > value ? false : this.squaredMax.isEmpty() || !(this.squaredMax.get() < value);
      }
      ...
      public static NumberRange.DoubleRange parse(StringReader reader, Function<Double, Double> mapper) throws CommandSyntaxException {
         return NumberRange.parse(
            reader, NumberRange.DoubleRange::create, Double::parseDouble, CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidDouble, mapper
         );
      }
   }

   @FunctionalInterface
   interface Factory<T extends Number, R extends NumberRange<T>> {
      R create(Optional<T> min, Optional<T> max);
   }

   record IntRange(Optional<Integer> min, Optional<Integer> max, Optional<Long> minSquared, Optional<Long> maxSquared) implements NumberRange<Integer> {
      public static final NumberRange.IntRange ANY = new NumberRange.IntRange(Optional.empty(), Optional.empty());
       public static final Codec<NumberRange.IntRange> CODEC = NumberRange.createCodec(Codec.INT, (Optional<Integer> min, Optional<Integer> max) -> new IntRange(min, max));
      ...
      public boolean testSqrt(long value) {
         return this.minSquared.isPresent() && this.minSquared.get() > value ? false : this.maxSquared.isEmpty() || this.maxSquared.get() >= value;
      }
      ...
      public static NumberRange.IntRange fromStringReader(StringReader reader, Function<Integer, Integer> converter) throws CommandSyntaxException {
         return NumberRange.parse(
            reader, (StringReader r, Optional<Integer> min, Optional<Integer> max) -> IntRange.parse(r, min, max), Integer::parseInt,
            CommandSyntaxException.BUILT_IN_EXCEPTIONS::readerInvalidInt, converter
         );
      }
   }
}
