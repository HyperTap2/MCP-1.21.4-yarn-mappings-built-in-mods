...
public class ScoreboardState extends PersistentState {
   ...
   public ScoreboardState readNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
      this.readObjectivesNbt(nbt.getList("Objectives", 10), registries);
      this.scoreboard.readNbt(nbt.getList("PlayerScores", 10), registries);
      if (nbt.contains("DisplaySlots", 10)) {
         this.readDisplaySlotsNbt(nbt.getCompound("DisplaySlots"));
      }

      if (nbt.contains("Teams", 9)) {
         this.readTeamsNbt(nbt.getList("Teams", 10), registries);
      }
   ...
   private void readObjectivesNbt(NbtList nbt, RegistryWrapper.WrapperLookup registries) {
      for (int i = 0; i < nbt.size(); i++) {
         ...
         ScoreboardCriterion scoreboardCriterion = ScoreboardCriterion.getOrCreateStatCriterion(string).orElseGet(() -> {
         ...
         NumberFormat numberFormat = (NumberFormat)NumberFormatTypes.CODEC
            .parse(registries.getOps(NbtOps.INSTANCE), nbtCompound.get("format"))
            .result()
            .orElse(null);
      ...
      for (ScoreboardObjective scoreboardObjective : this.scoreboard.getObjectives()) {
         ...
         if (numberFormat != null) {
            NumberFormatTypes.CODEC
               ...
               .ifSuccess(nbtElement -> nbtCompound.put("format", nbtElement));
         }
