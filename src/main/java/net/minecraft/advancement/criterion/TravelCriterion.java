   ...
   public record Conditions(Optional<LootContextPredicate> player, Optional<LocationPredicate> startPosition, Optional<DistancePredicate> distance)
      ...
      public static AdvancementCriterion<TravelCriterion.Conditions> rideEntityInLava(EntityPredicate.Builder entity, DistancePredicate distance) {
         return Criteria.RIDE_ENTITY_IN_LAVA
            .create(
               new TravelCriterion.Conditions(Optional.of(EntityPredicate.contextPredicateFromEntityPredicate(entity)), Optional.empty(), Optional.of(distance))
            );
      }
      ...
      public boolean matches(ServerWorld world, Vec3d pos, Vec3d endPos) {
         return this.startPosition.isPresent() && !this.startPosition.get().test(world, pos.x, pos.y, pos.z)
            ? false
            : !this.distance.isPresent() || this.distance.get().test(pos.x, pos.y, pos.z, endPos.x, endPos.y, endPos.z);
      }
   }
}
