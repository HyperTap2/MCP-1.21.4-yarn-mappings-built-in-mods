...
public class AdvancementProgress implements Comparable<AdvancementProgress> {
   ...
   private static final Codec<Map<String, CriterionProgress>> MAP_CODEC = Codec.unboundedMap(Codec.STRING, TIME_CODEC)
      .xmap(
         map -> map.entrySet().stream().collect(Collectors.toMap(Entry::getKey, entry -> new CriterionProgress((Instant)entry.getValue()))),
         map -> map.entrySet()
            .stream()
            .filter(entry -> ((CriterionProgress)entry.getValue()).isObtained())
            .collect(Collectors.toMap(Entry::getKey, entry -> Objects.requireNonNull(((CriterionProgress)entry.getValue()).getObtainedTime())))
      );
