...
public record AdvancementRewards(int experience, List<RegistryKey<LootTable>> loot, List<RegistryKey<Recipe<?>>> recipes, Optional<LazyContainer> function) {
   ...
   public void apply(ServerPlayerEntity player) {
      ...
      for (RegistryKey<LootTable> registryKey : this.loot) {
         ObjectListIterator var6 = player.server.getReloadableRegistries().getLootTable(registryKey).generateLoot(lootWorldContext).iterator();
      ...
      this.function
         ...
         .ifPresent(
            function -> minecraftServer.getCommandFunctionManager()
               .execute((CommandFunction<ServerCommandSource>)function, player.getCommandSource().withSilent().withLevel(2))
         );
   }

   public static class Builder {
      private int experience;
      private final com.google.common.collect.ImmutableList.Builder<RegistryKey<LootTable>> loot = ImmutableList.builder();
      private final com.google.common.collect.ImmutableList.Builder<RegistryKey<Recipe<?>>> recipes = ImmutableList.builder();
