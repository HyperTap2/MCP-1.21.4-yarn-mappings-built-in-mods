...
public class VanillaItemTagProvider extends ItemTagProvider {
   ...
   protected void configure(RegistryWrapper.WrapperLookup registries) {
      ...
      registries.getOrThrow(RegistryKeys.TRIM_MATERIAL)
         .streamEntries()
         .sorted(Comparator.comparing(reference -> reference.registryKey().getValue()))
         .forEach(reference -> this.getOrCreateTagBuilder(ItemTags.TRIM_MATERIALS).add(reference.value().ingredient().value()));
