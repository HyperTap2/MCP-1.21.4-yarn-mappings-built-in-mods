...
public abstract class BlockLootTableGenerator implements LootTableGenerator {
      ...
      return LootTable.builder()
         .pool(
            LootPool.builder()
               ...
               .with(ItemEntry.builder(drop).conditionally(conditionBuilder).alternatively(child))
         );
   }
                     ...
                     ItemEntry.builder(drop)
                        .apply(
                           ...
                           age -> SetCountLootFunction.builder(BinomialLootNumberProvider.create(3, (age + 1) / 15.0F))
                              .conditionally(
                                 BlockStatePropertyLootCondition.builder(stem)
                                    .properties(StatePredicate.Builder.create().exactMatch(StemBlock.AGE, age.intValue()))
                              )
                        )
                  )
            )
                     ...
                     ItemEntry.builder(drop)
                        ...
                        .apply(
                           ...
                           direction -> SetCountLootFunction.builder(ConstantLootNumberProvider.create(1.0F), true)
                              .conditionally(
                                 BlockStatePropertyLootCondition.builder(drop)
                                    .properties(StatePredicate.Builder.create().exactMatch(MultifaceBlock.getProperty(direction), true))
                              )
                        )
                     ...
                     ItemEntry.builder(block)
                        .conditionally(
                           BlockStatePropertyLootCondition.builder(block)
                              .properties(StatePredicate.Builder.create().exactMatch(PaleMossCarpetBlock.BOTTOM, true))
                        )
                  )
               )
         );
            ...
            LootPool.builder()
               ...
               .with(
                  ((LeafEntry.Builder)this.addSurvivesExplosionCondition(leaves, ItemEntry.builder(Items.APPLE)))
                     .conditionally(
                        TableBonusLootCondition.builder(impl.getOrThrow(Enchantments.FORTUNE), 0.005F, 0.0055555557F, 0.00625F, 0.008333334F, 0.025F)
                     )
               )
         );
   }
                           ...
                           candles -> SetCountLootFunction.builder(ConstantLootNumberProvider.create(candles.intValue()))
                              .conditionally(
                                 BlockStatePropertyLootCondition.builder(candle)
                                    .properties(StatePredicate.Builder.create().exactMatch(CandleBlock.CANDLES, candles.intValue()))
                              )
                        )
                  )
               )
                  ...
                  (LootPoolEntry.Builder<?>)this.applyExplosionDecay(
                     ...
                     ItemEntry.builder(flowerbed)
                        .apply(
                           ...
                           flowerAmount -> SetCountLootFunction.builder(ConstantLootNumberProvider.create(flowerAmount.intValue()))
                              .conditionally(
                                 BlockStatePropertyLootCondition.builder(flowerbed)
                                    .properties(StatePredicate.Builder.create().exactMatch(FlowerbedBlock.FLOWER_AMOUNT, flowerAmount.intValue()))
                              )
                        )
                  )
               )
                     ...
                     if (set.add((RegistryKey<LootTable>)lootTableKey)) {
                        ...
                        if (builder == null) {
                           throw new IllegalStateException(
                              String.format(Locale.ROOT, "Missing loottable '%s' for '%s'", lootTableKey.getValue(), Registries.BLOCK.getId(block))
                           );
                        }
   ...
   public void addDrop(Block block, LootTable.Builder lootTable) {
      this.lootTables.put(block.getLootTableKey().orElseThrow(() -> new IllegalStateException("Block " + block + " does not have loot table")), lootTable);
   }
}
