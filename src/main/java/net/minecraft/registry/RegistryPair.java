...
public record RegistryPair<T>(Optional<RegistryEntry<T>> entry, RegistryKey<T> key) {
   public RegistryPair(RegistryEntry<T> entry) {
      this(Optional.of(entry), entry.getKey().orElseThrow());
   }
   ...
   public static <T> Codec<RegistryPair<T>> createCodec(RegistryKey<Registry<T>> registryRef, Codec<RegistryEntry<T>> entryCodec) {
      return Codec.either(
            ...
            RegistryKey.createCodec(registryRef)
               .comapFlatMap(registryKey -> DataResult.error(() -> "Cannot parse as key without registry"), Function.identity())
         )
   ...
   public Either<RegistryEntry<T>, RegistryKey<T>> asEither() {
      return this.entry.<Either<RegistryEntry<T>, RegistryKey<T>>>map(Either::left).orElseGet(() -> Either.right(this.key));
   }

   public static <T> RegistryPair<T> create(Either<RegistryEntry<T>, RegistryKey<T>> entryOrKey) {
      return (RegistryPair<T>)entryOrKey.map(RegistryPair::new, RegistryPair::new);
   }

   public Optional<T> getValue(Registry<T> registry) {
      return this.entry.map(RegistryEntry::value).or(() -> registry.getOptionalValue(this.key));
   }

   public Optional<RegistryEntry<T>> getEntry(RegistryWrapper.WrapperLookup registries) {
      return this.entry.or(() -> registries.getOrThrow(this.key.getRegistryRef()).getOptional(this.key));
   }
}
