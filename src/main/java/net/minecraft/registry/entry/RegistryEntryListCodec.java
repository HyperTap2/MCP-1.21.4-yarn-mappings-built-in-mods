...
public class RegistryEntryListCodec<E> implements Codec<RegistryEntryList<E>> {
      ...
      if (ops instanceof RegistryOps<T> registryOps) {
         ...
         if (optional.isPresent()) {
            ...
            return this.entryListStorageCodec
               ...
               .flatMap(
                  pair -> {
                     DataResult<RegistryEntryList<E>> dataResult = ((Either<TagKey<E>, List<RegistryEntry<E>>>)pair.getFirst())
                        .map(tag -> get(registryEntryLookup, tag), entries -> DataResult.success((RegistryEntryList<E>)RegistryEntryList.of(entries)));
   ...
   private static <E> DataResult<RegistryEntryList<E>> get(RegistryEntryLookup<E> registry, TagKey<E> tag) {
      DataResult<RegistryEntryList<E>> result = registry.getOptional(tag)
      ...
      return result.isError() && ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_21)
         ? DataResult.success(RegistryEntryList.empty())
         : result;
   }
