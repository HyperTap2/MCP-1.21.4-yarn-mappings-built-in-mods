...
public class PacketByteBuf extends ByteBuf {
   ...
   public <T> T decode(DynamicOps<NbtElement> ops, Codec<T> codec, NbtSizeTracker sizeTracker) {
      ...
      return (T)codec.parse(ops, nbtElement).getOrThrow(error -> new DecoderException("Failed to decode: " + error + " " + nbtElement));
   }

   @Deprecated
   public <T> PacketByteBuf encode(DynamicOps<NbtElement> ops, Codec<T> codec, T value) {
      NbtElement nbtElement = (NbtElement)codec.encodeStart(ops, value).getOrThrow(error -> new EncoderException("Failed to encode: " + error + " " + value));
   ...
   public <T> void encodeAsJson(Codec<T> codec, T value) {
      ...
      this.writeString(GSON.toJson((JsonElement)dataResult.getOrThrow(error -> new EncoderException("Failed to encode: " + error + " " + value))));
   }
