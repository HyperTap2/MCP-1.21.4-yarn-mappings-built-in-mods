         ...
         engine.executeSync(() -> {
            if (listener.accepts(packet)) {
               try {
                  ...
                  if (exception instanceof CrashException crashException && crashException.getCause() instanceof OutOfMemoryError) {
                     throw createCrashException(exception, packet, listener);
                  }
