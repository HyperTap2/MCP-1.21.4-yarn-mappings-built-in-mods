...
public class BossBarS2CPacket implements Packet<ClientPlayPacketListener> {
   ...
   public static BossBarS2CPacket updateProperties(BossBar bar) {
      return new BossBarS2CPacket(
         bar.getUuid(), new BossBarS2CPacket.UpdatePropertiesAction(bar.shouldDarkenSky(), bar.hasDragonMusic(), bar.shouldThickenFog())
      );
   }
   ...
   enum Type {
      ...
      Type(final PacketDecoder<RegistryByteBuf, BossBarS2CPacket.Action> parser) {
         this.parser = parser;
      }
   }

   record UpdateNameAction(Text name) implements BossBarS2CPacket.Action {
   ...
   record UpdateProgressAction(float progress) implements BossBarS2CPacket.Action {
