...
public class ClientConnection extends SimpleChannelInboundHandler<Packet<?>> implements IClientConnection {
   ...
   private boolean viaFabricPlus$bedrockChannelActivated;
   ...
   public void channelActive(ChannelHandlerContext context) throws Exception {
      boolean bedrock = BedrockProtocolVersion.bedrockLatest.equals(this.viaFabricPlus$serverVersion);
      if (!bedrock || !this.viaFabricPlus$bedrockChannelActivated) {
         super.channelActive(context);
      }
      this.viaFabricPlus$bedrockChannelActivated = bedrock;
   ...
   public void exceptionCaught(ChannelHandlerContext context, Throwable ex) {
      if ((Boolean)com.viaversion.viafabricplus.settings.impl.DebugSettings.INSTANCE.printNetworkingErrorsToLogs.getValue()
         && !(ex instanceof java.net.SocketException)
         && !(ex instanceof java.net.ConnectException)) {
         com.viaversion.viafabricplus.ViaFabricPlusImpl.INSTANCE.logger().error("An exception occurred while handling a packet", ex);
      }
   ...
   public static ClientConnection connect(InetSocketAddress address, boolean useEpoll, @Nullable MultiValueDebugSampleLogImpl packetSizeLog) {
      ...
      if (packetSizeLog instanceof IMultiValueDebugSampleLogImpl versionHolder && versionHolder.viaFabricPlus$getForcedVersion() != null) {
         clientConnection.viaFabricPlus$setTargetVersion(versionHolder.viaFabricPlus$getForcedVersion());
      } else if (packetSizeLog != null) {
   ...
   public static ChannelFuture connect(InetSocketAddress address, boolean useEpoll, ClientConnection connection) {
      ProtocolVersion targetVersion = connection.viaFabricPlus$getTargetVersion();
      ...
      connection.viaFabricPlus$setTargetVersion(targetVersion);
      boolean bedrock = BedrockProtocolVersion.bedrockLatest.equals(targetVersion);
      ...
      Bootstrap bootstrap = (Bootstrap)new Bootstrap().group(eventLoopGroup).handler(new ChannelInitializer<Channel>() {
      ...
      if (bedrock) {
         bootstrap.channelFactory(
            class_ == EpollSocketChannel.class ? RakChannelFactory.client(EpollDatagramChannel.class) : RakChannelFactory.client(NioDatagramChannel.class)
         );
      } else {
         bootstrap.channel(class_);
      }
      if (bedrock && !useEpoll) {
                  ...
                  (ChannelFutureListener)future -> {
                     if (future.isSuccess()) {
                        future.channel()
                           .pipeline()
                           .replace(
                              "viabedrock-frame-encapsulation",
                              "viabedrock-ping-encapsulation",
                              new PingEncapsulationCodec(address)
                           );
                        future.channel().pipeline().remove("viabedrock-packet-encapsulation");
                        future.channel().pipeline().remove("splitter");
                     }
                  }
               }
            );
   ...
   public static void addHandlers(ChannelPipeline pipeline, NetworkSide side, boolean local, @Nullable PacketSizeLogger packetSizeLogger) {
      ...
      pipeline.addLast("splitter", getSplitter(packetSizeLogger, local))
         ...
         .addLast(
            getInboundHandlerName(bl),
            (ChannelHandler)(bl ? new DecoderHandler<ServerHandshakePacketListener>(C2S_HANDSHAKE_STATE) : new NetworkStateTransitions.InboundConfigurer())
         )
         .addLast("prepender", getPrepender(local))
         .addLast(
            getOutboundHandlerName(bl2),
            (ChannelHandler)(bl2 ? new EncoderHandler<ServerHandshakePacketListener>(C2S_HANDSHAKE_STATE) : new NetworkStateTransitions.OutboundConfigurer())
         );
   }
   ...
   public void setupEncryption(Cipher decryptionCipher, Cipher encryptionCipher) {
      if (this.viaFabricPlus$serverVersion != null && this.viaFabricPlus$serverVersion.olderThanOrEqualTo(LegacyProtocolVersion.r1_6_4)) {
         ...
         this.encrypted = true;
         this.channel.pipeline().addBefore("vialegacy-pre-netty-length-remover", "encrypt", new PacketEncryptor(encryptionCipher));
         return;
      }
      this.encrypted = true;
   ...
   public void viaFabricPlus$setupPreNettyDecryption() {
      ...
      this.encrypted = true;
      this.channel.pipeline().addBefore("vialegacy-pre-netty-length-prepender", "decrypt", new PacketDecryptor(this.viaFabricPlus$decryptionCipher));
   }
   ...
   @Override
   public void viaFabricPlus$setTargetVersion(ProtocolVersion version) {
      this.viaFabricPlus$serverVersion = version;
   }
