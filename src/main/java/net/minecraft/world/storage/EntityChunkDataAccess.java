...
public class EntityChunkDataAccess implements ChunkDataAccess<Entity> {
   ...
   public CompletableFuture<ChunkDataList<Entity>> readChunkData(ChunkPos pos) {
      ...
      return completableFuture.thenApplyAsync(nbt -> {
         ...
         try {
            ChunkPos chunkPos2 = getChunkPos(nbt.get());
         ...
         NbtCompound nbtCompound = this.storage.update(nbt.get(), -1);
         NbtList nbtList = nbtCompound.getList("Entities", 10);
         List<Entity> list = EntityType.streamFromNbt(nbtList, this.world, SpawnReason.LOAD).collect(ImmutableList.toImmutableList());
