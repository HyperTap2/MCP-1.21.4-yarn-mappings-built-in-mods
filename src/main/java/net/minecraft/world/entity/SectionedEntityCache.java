...
public class SectionedEntityCache<T extends EntityLike> implements ChunkAwareEntityIterable<T> {
   ...
   private EntityTrackingSection<T> addSection(long sectionPos) {
      ...
      EntityTrackingSection<T> section = new EntityTrackingSection<>(this.entityClass, entityTrackingStatus);
      section.lithium$setPos(sectionPos);
      return section;
   }
   ...
   public Iterable<T> lithium$iterateEntitiesInTrackedSections() {
      return () -> this.trackingSections.values().stream()
         .filter(section -> section.getStatus().shouldTrack() && !section.isEmpty())
         .flatMap(EntityTrackingSection::stream)
         .iterator();
   }
