...
public class LargeOakTrunkPlacer extends TrunkPlacer {
   ...
   public List<FoliagePlacer.TreeNode> generate(
      ...
      int i = 5;
      int j = height + 2;
      int k = MathHelper.floor(j * 0.618);
      setToDirt(world, replacer, random, startPos.down(), config);
      double d = 1.0;
      int l = Math.min(1, MathHelper.floor(1.382 + Math.pow(1.0 * j / 13.0, 2.0)));
   ...
   private boolean makeOrCheckBranch(
      ...
      for (int j = 0; j <= i; j++) {
         BlockPos blockPos2 = startPos.add(MathHelper.floor(0.5F + j * f), MathHelper.floor(0.5F + j * g), MathHelper.floor(0.5F + j * h));
         if (make) {
