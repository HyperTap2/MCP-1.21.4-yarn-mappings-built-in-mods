...
public final class DensityFunctionTypes {
   ...
   public static final Codec<DensityFunction> CODEC = Codec.either(CONSTANT_RANGE, DYNAMIC_RANGE)
      .xmap(
         either -> (DensityFunction)either.map(DensityFunctionTypes::constant, Function.identity()),
   ...
   public static DensityFunction weirdScaledSampler(
      ...
      RegistryEntry<DoublePerlinNoiseSampler.NoiseParameters> parameters,
      DensityFunctionTypes.WeirdScaledSampler.RarityValueMapper mapper
   ) {
   ...
   public enum Beardifier implements DensityFunctionTypes.Beardifying {
      INSTANCE;
      ...
      public void fill(double[] densities, DensityFunction.EachApplier applier) {
         ...
         switch (this.type) {
            ...
            case MIN:
               ...
               break;
            case MAX:
               double e1 = this.argument2.maxValue();

               for (int k = 0; k < densities.length; k++) {
                  double f = densities[k];
                  densities[k] = f > e1 ? f : Math.max(f, this.argument2.sample(applier.at(k)));
               }
         }
      }
   ...
   public enum BlendAlpha implements DensityFunction.Base {
      INSTANCE;
   ...
   public enum BlendOffset implements DensityFunction.Base {
      INSTANCE;
   ...
   protected record WeirdScaledSampler(
      ...
      public enum RarityValueMapper implements StringIdentifiable {
         ...
         RarityValueMapper(final String name, final Double2DoubleFunction scaleFunction, final double maxValueMultiplier) {
            this.name = name;
   ...
   public record Wrapping(DensityFunctionTypes.Wrapping.Type type, DensityFunction wrapped) implements DensityFunctionTypes.Wrapper {
      @Override
      public double sample(DensityFunction.NoisePos pos) {
   ...
   record YClampedGradient(int fromY, int toY, double fromValue, double toValue) implements DensityFunction.Base {
      private static final MapCodec<DensityFunctionTypes.YClampedGradient> Y_CLAMPED_GRADIENT_CODEC = RecordCodecBuilder.mapCodec(
         instance -> instance.group(
               ...
               Codec.intRange(DimensionType.MIN_HEIGHT * 2, DimensionType.MAX_COLUMN_HEIGHT * 2)
                  .fieldOf("to_y")
                  .forGetter(DensityFunctionTypes.YClampedGradient::toY),
      ...
      @Override
      public double sample(DensityFunction.NoisePos pos) {
         return MathHelper.clampedMap(pos.blockY(), this.fromY, this.toY, this.fromValue, this.toValue);
      }
