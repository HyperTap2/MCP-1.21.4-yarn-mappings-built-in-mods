...
public interface DensityFunction {
   ...
   Codec<DensityFunction> FUNCTION_CODEC = REGISTRY_ENTRY_CODEC.xmap(
      ...
      function -> function instanceof DensityFunctionTypes.RegistryEntryHolder registryEntryHolder
   ...
   interface Base extends DensityFunction {
      @Override
   ...
   record Noise(RegistryEntry<DoublePerlinNoiseSampler.NoiseParameters> noiseData, @Nullable DoublePerlinNoiseSampler noise) {
   ...
   record UnblendedNoisePos(int blockX, int blockY, int blockZ) implements DensityFunction.NoisePos {
   }
}
