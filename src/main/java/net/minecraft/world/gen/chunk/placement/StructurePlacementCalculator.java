...
public class StructurePlacementCalculator {
   ...
   public static StructurePlacementCalculator create(
      ...
      List<RegistryEntry<StructureSet>> list = structureSets.filter(structureSet -> hasValidBiome(structureSet.value(), biomeSource)).toList();
            ...
            structureSet -> {
               ...
               for (StructureSet.WeightedEntry weightedEntry : structureSet2.structures()) {
                  ...
                  if (structure.getValidBiomes().stream().anyMatch(set::contains)) {
                     this.structuresToPlacements.computeIfAbsent(structure, structurex -> new ArrayList<>()).add(structureSet2.placement());
                     bl = true;
                  }
               }
               ...
               if (bl && structureSet2.placement() instanceof ConcentricRingsStructurePlacement concentricRingsStructurePlacement) {
                  this.concentricPlacementsToPositions
                     .put(
                        ...
                        this.calculateConcentricsRingPlacementPos((RegistryEntry<StructureSet>)structureSet, concentricRingsStructurePlacement)
                     );
               }
            }
         );
