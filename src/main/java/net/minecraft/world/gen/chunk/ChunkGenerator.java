   ...
   public ChunkGenerator(BiomeSource biomeSource, Function<RegistryEntry<Biome>, GenerationSettings> generationSettingsGetter) {
      ...
      this.indexedFeaturesListSupplier = Suppliers.memoize(
         () -> PlacedFeatureIndexer.collectIndexedFeatures(
            List.copyOf(biomeSource.getBiomes()), biomeEntry -> generationSettingsGetter.apply(biomeEntry).getFeatures(), true
         )
      );
   }
      ...
      for (Entry<StructurePlacement, Set<RegistryEntry<Structure>>> entry : map.entrySet()) {
         ...
         if (structurePlacement2 instanceof ConcentricRingsStructurePlacement concentricRingsStructurePlacement) {
            Pair<BlockPos, RegistryEntry<Structure>> pair2 = this.locateConcentricRingsStructure(
               entry.getValue(), world, structureAccessor, center, skipReferencedStructures, concentricRingsStructurePlacement
            );
            if (pair2 != null) {
            ...
            for (StructureStart structureStart : world.getChunk(n, o).getStructureStarts().values()) {
               try {
                  ...
                  crashReportSection.add(
                     "Id",
                     () -> optional.<String>map(structureTypeRegistry -> structureTypeRegistry.getId(structureStart.getStructure()).toString())
                        .orElse("UNKNOWN")
                  );
