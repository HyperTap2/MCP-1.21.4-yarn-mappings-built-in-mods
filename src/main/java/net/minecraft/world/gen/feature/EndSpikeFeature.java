...
public class EndSpikeFeature extends Feature<EndSpikeFeatureConfig> {
   ...
   private void generateSpike(ServerWorldAccess world, Random random, EndSpikeFeatureConfig config, EndSpikeFeature.Spike spike) {
      ...
      for (BlockPos blockPos : BlockPos.iterate(
         ...
         if (blockPos.getSquaredDistance(spike.getCenterX(), blockPos.getY(), spike.getCenterZ()) <= i * i + 1 && blockPos.getY() < spike.getHeight()) {
      ...
      if (endCrystalEntity != null) {
         ...
         endCrystalEntity.refreshPositionAndAngles(spike.getCenterX() + 0.5, spike.getHeight() + 1, spike.getCenterZ() + 0.5, random.nextFloat() * 360.0F, 0.0F);
   ...
   public static class Spike {
      ...
      public Spike(int centerX, int centerZ, int radius, int height, boolean guarded) {
         ...
         this.boundingBox = new Box(
            centerX - radius, DimensionType.MIN_HEIGHT, centerZ - radius, centerX + radius, DimensionType.MAX_COLUMN_HEIGHT, centerZ + radius
         );
      }
