...
public class IcebergFeature extends Feature<SingleStateFeatureConfig> {
   ...
   public boolean generate(FeatureContext<SingleStateFeatureConfig> context) {
      ...
      for (int p = -o; p < o; p++) {
         for (int q = -o; q < o; q++) {
            for (int r = -1; r > -m; r--) {
               int s = bl2 ? MathHelper.ceil(o * (1.0F - (float)Math.pow(r, 2.0) / (m * 8.0F))) : o;
   ...
   private void placeBlockOrSnow(
      ...
      if (blockState.isAir() || blockState.isOf(Blocks.SNOW_BLOCK) || blockState.isOf(Blocks.ICE) || blockState.isOf(Blocks.WATER)) {
         ...
         int i = lessSnow ? 3 : 2;
         if (placeSnow && !blockState.isOf(Blocks.WATER) && heightRemaining <= random.nextInt(Math.max(1, height / i)) + height * 0.6 && bl) {
   ...
   private double getDistance(int x, int z, BlockPos pos, int divisor1, int divisor2, double randomSine) {
      return Math.pow(((x - pos.getX()) * Math.cos(randomSine) - (z - pos.getZ()) * Math.sin(randomSine)) / divisor1, 2.0)
         + Math.pow(((x - pos.getX()) * Math.sin(randomSine) + (z - pos.getZ()) * Math.cos(randomSine)) / divisor2, 2.0)
         - 1.0;
   }
