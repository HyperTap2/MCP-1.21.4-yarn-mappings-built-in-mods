...
public class Structures {
   public static void bootstrap(Registerable<Structure> structureRegisterable) {
         ...
         new JigsawStructure(
            new Structure.Config.Builder(registryEntryLookup.getOrThrow(BiomeTags.PILLAGER_OUTPOST_HAS_STRUCTURE))
               .spawnOverrides(
                  Map.of(
                     SpawnGroup.MONSTER,
                     new StructureSpawns(StructureSpawns.BoundingBox.STRUCTURE, Pool.of(new SpawnSettings.SpawnEntry(EntityType.PILLAGER, 1, 1, 1)))
                  )
               )
      ...
      structureRegisterable.register(
         StructureKeys.IGLOO, new IglooStructure(new Structure.Config(registryEntryLookup.getOrThrow(BiomeTags.IGLOO_HAS_STRUCTURE)))
      );
      structureRegisterable.register(
            ...
            new Structure.Config.Builder(registryEntryLookup.getOrThrow(BiomeTags.ANCIENT_CITY_HAS_STRUCTURE))
               .spawnOverrides(
                  Arrays.stream(SpawnGroup.values())
                     .collect(
                        Collectors.toMap(
                           spawnGroup -> (SpawnGroup)spawnGroup, spawnGroup -> new StructureSpawns(StructureSpawns.BoundingBox.STRUCTURE, Pool.empty())
                        )
                     )
               )
               .step(GenerationStep.Feature.UNDERGROUND_DECORATION)
            ...
            new Structure.Config.Builder(registryEntryLookup.getOrThrow(BiomeTags.TRIAL_CHAMBERS_HAS_STRUCTURE))
               ...
               .spawnOverrides(
                  Arrays.stream(SpawnGroup.values())
                     .collect(
                        Collectors.toMap(
                           spawnGroup -> (SpawnGroup)spawnGroup, spawnGroup -> new StructureSpawns(StructureSpawns.BoundingBox.PIECE, Pool.empty())
                        )
                     )
               )
               .build(),
