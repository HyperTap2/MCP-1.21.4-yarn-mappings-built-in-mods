...
public class MaterialRules {
   ...
   record BlockMaterialRule(BlockState resultState, MaterialRules.SimpleBlockStateRule rule) implements MaterialRules.MaterialRule {
      static final CodecHolder<MaterialRules.BlockMaterialRule> CODEC = CodecHolder.of(
         BlockState.CODEC.xmap(MaterialRules.BlockMaterialRule::new, MaterialRules.BlockMaterialRule::resultState).fieldOf("result_state")
      );
   ...
   record ConditionMaterialRule(MaterialRules.MaterialCondition ifTrue, MaterialRules.MaterialRule thenRun) implements MaterialRules.MaterialRule {
      ...
      public MaterialRules.BlockStateRule apply(MaterialRules.MaterialRuleContext materialRuleContext) {
         return new MaterialRules.ConditionalBlockStateRule(this.ifTrue.apply(materialRuleContext), this.thenRun.apply(materialRuleContext));
      }
   }
   ...
   record NotMaterialCondition(MaterialRules.MaterialCondition target) implements MaterialRules.MaterialCondition {
      static final CodecHolder<MaterialRules.NotMaterialCondition> CODEC = CodecHolder.of(
         MaterialRules.MaterialCondition.CODEC.xmap(MaterialRules.NotMaterialCondition::new, MaterialRules.NotMaterialCondition::target).fieldOf("invert")
      );
         ...
         class StoneDepthPredicate extends MaterialRules.FullLazyAbstractPredicate {
            ...
            protected boolean test() {
               ...
               int k = StoneDepthMaterialCondition.this.secondaryDepthRange == 0
                  ? 0
                  : (int)MathHelper.map(this.context.getSecondaryDepth(), -1.0, 1.0, 0.0, StoneDepthMaterialCondition.this.secondaryDepthRange);
   ...
   record VerticalGradientMaterialCondition(Identifier randomName, YOffset trueAtAndBelow, YOffset falseAtAndAbove) implements MaterialRules.MaterialCondition {
      ...
      public MaterialRules.BooleanSupplier apply(MaterialRules.MaterialRuleContext materialRuleContext) {
         final int trueAtAndBelowY = this.trueAtAndBelow().getY(materialRuleContext.heightContext);
         final int falseAtAndAboveY = this.falseAtAndAbove().getY(materialRuleContext.heightContext);
         ...
         class VerticalGradientPredicate extends MaterialRules.FullLazyAbstractPredicate {
            ...
            protected boolean test() {
               ...
               double d = MathHelper.map(blockY, trueAtAndBelowY, falseAtAndAboveY, 1.0, 0.0);
