...
public class GameRules {
   ...
   public static final GameRules.Key<GameRules.BooleanRule> DO_FIRE_TICK = register(
      "doFireTick", GameRules.Category.UPDATES, GameRules.BooleanRule.create(true)
   );
   ...
   public static final GameRules.Key<GameRules.BooleanRule> DO_TILE_DROPS = register(
      "doTileDrops", GameRules.Category.DROPS, GameRules.BooleanRule.create(true)
   );
         ...
         (server, rule) -> {
            for (ServerPlayerEntity serverPlayerEntity : server.getPlayerManager().getPlayerList()) {
               serverPlayerEntity.networkHandler
                  .sendPacket(new GameStateChangeS2CPacket(GameStateChangeS2CPacket.LIMITED_CRAFTING_TOGGLED, rule.get() ? 1.0F : 0.0F));
            }
         }
      )
   );
   ...
   public static final GameRules.Key<GameRules.BooleanRule> DO_INSOMNIA = register(
      "doInsomnia", GameRules.Category.SPAWNING, GameRules.BooleanRule.create(true)
   );
   ...
   public static final GameRules.Key<GameRules.IntRule> MINECART_MAX_SPEED = register(
      ...
      GameRules.IntRule.create(8, 1, 1000, FeatureSet.of(FeatureFlags.MINECART_IMPROVEMENTS), (server, value) -> {})
   );
   ...
   public GameRules(FeatureSet enabledFeatures) {
      this(
         streamAllRules(enabledFeatures).collect(ImmutableMap.toImmutableMap(Entry::getKey, entry -> ((GameRules.Type)entry.getValue()).createRule())),
         enabledFeatures
      );
   }

   private static Stream<Entry<GameRules.Key<?>, GameRules.Type<?>>> streamAllRules(FeatureSet enabledFeatures) {
      return RULE_TYPES.entrySet().stream().filter(entry -> entry.getValue().requiredFeatures.isSubsetOf(enabledFeatures));
   }
   ...
   public GameRules copy(FeatureSet enabledFeatures) {
      return new GameRules(
         streamAllRules(enabledFeatures)
            .collect(
               ImmutableMap.toImmutableMap(
                  Entry::getKey,
                  entry -> this.rules.containsKey(entry.getKey()) ? this.rules.get(entry.getKey()) : ((GameRules.Type)entry.getValue()).createRule()
               )
            ),
         enabledFeatures
      );
   ...
   public void accept(GameRules.Visitor visitor) {
      RULE_TYPES.forEach((key, type) -> this.accept(visitor, (GameRules.Key<?>)key, (GameRules.Type<?>)type));
   }

   private <T extends GameRules.Rule<T>> void accept(GameRules.Visitor visitor, GameRules.Key<?> key, GameRules.Type<?> type) {
      GameRules.Key<T> key2 = (GameRules.Key<T>)key;
      GameRules.Type<T> type2 = (GameRules.Type<T>)type;
   ...
   public void setAllValues(GameRules rules, @Nullable MinecraftServer server) {
      rules.rules.keySet().forEach(key -> this.setValue((GameRules.Key<?>)key, rules, server));
   }
