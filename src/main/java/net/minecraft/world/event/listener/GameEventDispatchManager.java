...
public class GameEventDispatchManager {
   ...
   public void dispatch(RegistryEntry<GameEvent> event, Vec3d emitterPos, GameEvent.Emitter emitter) {
      ...
      for (int p = j; p <= m; p++) {
         for (int q = l; q <= o; q++) {
            Int2ObjectMap<GameEventDispatcher> dispatchers = ((LithiumData)this.world)
               .lithium$getData().gameEventDispatchers().get(ChunkPos.toLong(p, q));
            if (dispatchers != null) {
               for (int r = k; r <= n; r++) {
                  GameEventDispatcher dispatcher = dispatchers.get(r);
                  if (dispatcher != null) {
                     bl |= dispatcher.dispatch(event, emitterPos, emitter, dispatchCallback);
                  }
               }
            }
         }
