...
public class MultiNoiseUtil {
   ...
   public static MultiNoiseUtil.MultiNoiseSampler createEmptyMultiNoiseSampler() {
      ...
      return new MultiNoiseUtil.MultiNoiseSampler(
         densityFunction, densityFunction, densityFunction, densityFunction, densityFunction, densityFunction, List.of()
      );
   }
   ...
   public static class Entries<T> {
         ...
         return Codecs.nonEmptyList(
               RecordCodecBuilder.<Pair<MultiNoiseUtil.NoiseHypercube, T>>create(
                     instance -> instance.group(
                           MultiNoiseUtil.NoiseHypercube.CODEC.fieldOf("parameters").forGetter(Pair::getFirst), entryCodec.forGetter(Pair::getSecond)
                        )
      ...
      public T getValueSimple(MultiNoiseUtil.NoiseValuePoint point) {
         ...
         Pair<MultiNoiseUtil.NoiseHypercube, T> pair = iterator.next();
         long l = ((MultiNoiseUtil.NoiseHypercube)pair.getFirst()).getSquaredDistance(point);
         T object = (T)pair.getSecond();

         while (iterator.hasNext()) {
            Pair<MultiNoiseUtil.NoiseHypercube, T> pair2 = iterator.next();
            long m = ((MultiNoiseUtil.NoiseHypercube)pair2.getFirst()).getSquaredDistance(point);
            if (m < l) {
               l = m;
               object = (T)pair2.getSecond();
            }
         }
   ...
   protected static final class SearchTree<T> {
      ...
      public static <T> MultiNoiseUtil.SearchTree<T> create(List<Pair<MultiNoiseUtil.NoiseHypercube, T>> entries) {
         ...
         int i = ((MultiNoiseUtil.NoiseHypercube)entries.get(0).getFirst()).getParameters().size();
         if (i != 7) {
            throw new IllegalStateException("Expecting parameter space to be 7, got " + i);
         }

         List<MultiNoiseUtil.SearchTree.TreeLeafNode<T>> list = entries.stream()
      ...
      private static <T> MultiNoiseUtil.SearchTree.TreeNode<T> createNode(int parameterNumber, List<? extends MultiNoiseUtil.SearchTree.TreeNode<T>> subTree) {
         ...
         return new MultiNoiseUtil.SearchTree.TreeBranchNode<>(
            list.stream().map(node -> createNode(parameterNumber, Arrays.asList(node.subTree))).collect(Collectors.toList())
         );
      }
      ...
      private static <T> List<MultiNoiseUtil.SearchTree.TreeBranchNode<T>> getBatchedTree(List<? extends MultiNoiseUtil.SearchTree.TreeNode<T>> nodes) {
         List<MultiNoiseUtil.SearchTree.TreeBranchNode<T>> list = Lists.newArrayList();
         List<MultiNoiseUtil.SearchTree.TreeNode<T>> list2 = Lists.newArrayList();
      ...
      static <T> List<MultiNoiseUtil.ParameterRange> getEnclosingParameters(List<? extends MultiNoiseUtil.SearchTree.TreeNode<T>> subTree) {
         ...
         for (MultiNoiseUtil.SearchTree.TreeNode<T> treeNode : subTree) {
            for (int k = 0; k < 7; k++) {
               list.set(k, treeNode.parameters[k].combine(list.get(k)));
            }
         }
      ...
      public T get(MultiNoiseUtil.NoiseValuePoint point, MultiNoiseUtil.NodeDistanceFunction<T> distanceFunction) {
         ...
         MultiNoiseUtil.SearchTree.TreeLeafNode<T> treeLeafNode = this.firstNode.getResultingNode(ls, this.previousResultNode.get(), distanceFunction);
