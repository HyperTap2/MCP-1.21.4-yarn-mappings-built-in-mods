...
public class MultiNoiseBiomeSourceParameterList {
   ...
   public static Map<MultiNoiseBiomeSourceParameterList.Preset, MultiNoiseUtil.Entries<RegistryKey<Biome>>> getPresetToEntriesMap() {
      return MultiNoiseBiomeSourceParameterList.Preset.BY_IDENTIFIER
         ...
         .stream()
         .collect(
            Collectors.toMap(preset -> (MultiNoiseBiomeSourceParameterList.Preset)preset, preset -> preset.biomeSourceFunction().apply(biomeKey -> biomeKey))
         );
   }

   public record Preset(Identifier id, MultiNoiseBiomeSourceParameterList.Preset.BiomeSourceFunction biomeSourceFunction) {
      ...
      static final Map<Identifier, MultiNoiseBiomeSourceParameterList.Preset> BY_IDENTIFIER = Stream.of(NETHER, OVERWORLD)
      ...
      public Stream<RegistryKey<Biome>> biomeStream() {
         return this.biomeSourceFunction.apply(biomeKey -> biomeKey).getEntries().stream().<RegistryKey<Biome>>map(Pair::getSecond).distinct();
      }
