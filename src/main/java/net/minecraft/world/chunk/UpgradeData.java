...
public class UpgradeData {
   ...
   private static <T> void addNeighborTicks(NbtCompound nbt, String key, Function<String, Optional<T>> nameToType, List<Tick<T>> ticks) {
      if (nbt.contains(key, 9)) {
         for (NbtElement nbtElement : nbt.getList(key, 10)) {
   ...
   enum BuiltinLogic implements UpgradeData.Logic {
      BLACKLIST(
         Blocks.OBSERVER,
      ...
      LEAVES(
         ...
         public BlockState getUpdatedState(
            ...
            BlockState blockState = oldState.getStateForNeighborUpdate(
         ...
         public void postUpdate(WorldAccess world) {
            ...
            for (int i = 2; i < list.size(); i++) {
               ...
               while (var8.hasNext()) {
                  ...
                  BlockState blockState = world.getBlockState(blockPos);
                  if (blockState.get(Properties.DISTANCE_1_7) >= j) {
                     world.setBlockState(blockPos, blockState.with(Properties.DISTANCE_1_7, j), 18);
                     if (i != 7) {
                        for (Direction direction : DIRECTIONS) {
                           mutable.set(blockPos, direction);
                           BlockState blockState2 = world.getBlockState(mutable);
                           if (blockState2.contains(Properties.DISTANCE_1_7) && blockState.get(Properties.DISTANCE_1_7) > i) {
