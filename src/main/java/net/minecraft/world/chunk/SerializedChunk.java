...
public record SerializedChunk(
   ...
   public static SerializedChunk fromNbt(HeightLimitView world, DynamicRegistryManager registryManager, NbtCompound nbt) {
      ...
      for (Heightmap.Type type : chunkStatus.getHeightmapTypes()) {
         ...
         if (nbtCompound.contains(string, 12)) {
            map.put(type, nbtCompound.getLongArray(string));
         }
      }

      List<Tick<Block>> list = Tick.tick(nbt.getList("block_ticks", 10), id -> Registries.BLOCK.getOptionalValue(Identifier.tryParse(id)), chunkPos);
      List<Tick<Fluid>> list2 = Tick.tick(nbt.getList("fluid_ticks", 10), id -> Registries.FLUID.getOptionalValue(Identifier.tryParse(id)), chunkPos);
      Chunk.TickSchedulers tickSchedulers = new Chunk.TickSchedulers(list, list2);
      NbtList nbtList = nbt.getList("PostProcessing", 9);
      ...
      List<NbtCompound> list3 = Lists.transform(nbt.getList("entities", 10), entity -> (NbtCompound)entity);
      List<NbtCompound> list4 = Lists.transform(nbt.getList("block_entities", 10), blockEntity -> (NbtCompound)blockEntity);
      NbtCompound nbtCompound2 = nbt.getCompound("structures");
      NbtList nbtList3 = nbt.getList("sections", 10);
      ...
      for (int k = 0; k < nbtList3.size(); k++) {
         ...
         if (n >= world.getBottomSectionCoord() && n <= world.getTopSectionCoord()) {
            ...
            if (nbtCompound3.contains("block_states", 10)) {
               palettedContainer = (PalettedContainer<BlockState>)CODEC.parse(NbtOps.INSTANCE, nbtCompound3.getCompound("block_states"))
            ...
            if (nbtCompound3.contains("biomes", 10)) {
               readableContainer = (ReadableContainer<RegistryEntry<Biome>>)codec.parse(NbtOps.INSTANCE, nbtCompound3.getCompound("biomes"))
                  .promotePartial(error -> logRecoverableError(chunkPos, n, error))
                  .getOrThrow(SerializedChunk.ChunkLoadingException::new);
            } else {
               readableContainer = new PalettedContainer<>(
                  registry.getIndexedEntries(), registry.getOrThrow(BiomeKeys.PLAINS), PalettedContainer.PaletteProvider.BIOME
               );
            }
         ...
         ChunkNibbleArray chunkNibbleArray = nbtCompound3.contains("BlockLight", 7) ? new ChunkNibbleArray(nbtCompound3.getByteArray("BlockLight")) : null;
         ChunkNibbleArray chunkNibbleArray2 = nbtCompound3.contains("SkyLight", 7) ? new ChunkNibbleArray(nbtCompound3.getByteArray("SkyLight")) : null;
   ...
   public NbtCompound serialize() {
      ...
      for (SerializedChunk.SectionData sectionData : this.sectionData) {
         ...
         if (chunkSection != null) {
            nbtCompound2.put("block_states", (NbtElement)CODEC.encodeStart(NbtOps.INSTANCE, chunkSection.getBlockStateContainer()).getOrThrow());
            nbtCompound2.put("biomes", (NbtElement)codec.encodeStart(NbtOps.INSTANCE, chunkSection.getBiomeContainer()).getOrThrow());
         }
