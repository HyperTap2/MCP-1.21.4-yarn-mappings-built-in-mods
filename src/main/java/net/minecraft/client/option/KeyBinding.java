...
public class KeyBinding implements Comparable<KeyBinding> {
   ...
   private static final Map<String, Integer> CATEGORY_ORDER_MAP = Util.make(Maps.newHashMap(), map -> {
   ...
   public int compareTo(KeyBinding keyBinding) {
      ...
      int categoryOrder = Integer.compare(
         CATEGORY_ORDER_MAP.getOrDefault(this.category, Integer.MAX_VALUE),
         CATEGORY_ORDER_MAP.getOrDefault(keyBinding.category, Integer.MAX_VALUE)
      );
      if (categoryOrder != 0) {
         return categoryOrder;
      }

      int localizedOrder = I18n.translate(this.category).compareTo(I18n.translate(keyBinding.category));
      return localizedOrder != 0 ? localizedOrder : this.category.compareTo(keyBinding.category);
   }
