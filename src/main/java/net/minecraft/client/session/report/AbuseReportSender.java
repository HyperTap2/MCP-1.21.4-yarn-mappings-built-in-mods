...
public interface AbuseReportSender {
   ...
   class AbuseReportException extends TextifiedException {
      public AbuseReportException(Text text, Throwable throwable) {
         super(text, throwable);
      }
   }

   record Impl(ReporterEnvironment environment, UserApiService userApiService) implements AbuseReportSender {
         ...
         return CompletableFuture.supplyAsync(
            () -> {
               ...
               try {
                  ...
                  throw new CompletionException(new AbuseReportSender.AbuseReportException(text, minecraftClientHttpException));
               } catch (MinecraftClientException minecraftClientException) {
                  Text text = this.getErrorText(minecraftClientException);
                  throw new CompletionException(new AbuseReportSender.AbuseReportException(text, minecraftClientException));
               }
            },
      ...
      private Text getErrorText(MinecraftClientHttpException exception) {
         return Text.translatable("gui.abuseReport.send.error_message", new Object[]{exception.getMessage()});
      }
