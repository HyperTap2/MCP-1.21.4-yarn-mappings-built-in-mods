      ...
      sender.send(
         ...
         properties -> {
            synchronized (this) {
                  ...
                  .forEach(
                     (property, stopwatch) -> {
                        if (!stopwatch.isRunning()) {
                           ...
                           properties.put((TelemetryEventProperty<GameLoadTimeEvent.Measurement>)property, new GameLoadTimeEvent.Measurement((int)l));
                        } else {
                           LOGGER.warn(
               ...
               this.bootstrapTime
                  .ifPresent(
                     bootstrapTime -> properties.put(TelemetryEventProperty.LOAD_TIME_BOOTSTRAP_MS, new GameLoadTimeEvent.Measurement((int)bootstrapTime))
                  );
