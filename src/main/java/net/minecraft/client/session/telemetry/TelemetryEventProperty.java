...
public record TelemetryEventProperty<T>(String id, String exportKey, Codec<T> codec, TelemetryEventProperty.PropertyExporter<T> exporter) {
   ...
   public static TelemetryEventProperty<LongList> ofLongList(String id, String exportKey) {
      return of(
         ...
         (container, key, value) -> container.addProperty(key, value.longStream().mapToObj(String::valueOf).collect(Collectors.joining(";")))
      );
   }
   ...
   public enum GameMode implements StringIdentifiable {
      ...
      GameMode(final String id, final int rawId) {
         this.id = id;
         this.rawId = rawId;
      }
