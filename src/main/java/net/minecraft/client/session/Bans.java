...
public class Bans {
   ...
   private static final Text SKIN_DESCRIPTION = Text.translatable("gui.banned.skin.description", new Object[]{Text.of(Urls.JAVA_MODERATION)});
   ...
   public static ConfirmLinkScreen createUsernameBanScreen(String username, Runnable onClose) {
      ...
      return new ConfirmLinkScreen(
         ...
         NAME_TITLE,
         Text.translatable("gui.banned.name.description", new Object[]{Text.literal(username).formatted(Formatting.YELLOW), Text.of(Urls.JAVA_MODERATION)}),
         uRI,
   ...
   private static Text getDescriptionText(BanDetails banDetails) {
      return Text.translatable("gui.banned.description", new Object[]{getReasonText(banDetails), getDurationText(banDetails), Text.of(Urls.JAVA_MODERATION)});
   }
   ...
   private static Text getReasonText(BanDetails banDetails) {
      ...
      if (StringUtils.isNumeric(string)) {
         ...
         if (banReason != null) {
            ...
            text = Text.translatable("gui.banned.description.reason_id_message", new Object[]{i, string2}).formatted(Formatting.BOLD);
         } else {
            text = Text.translatable("gui.banned.description.reason_id", new Object[]{i}).formatted(Formatting.BOLD);
         }

         return Text.translatable("gui.banned.description.reason", new Object[]{text});
      } else {
   ...
   private static Text getDurationText(BanDetails banDetails) {
      if (isTemporary(banDetails)) {
         ...
         return Text.translatable(
            ...
            new Object[]{Text.translatable("gui.banned.description.temporary.duration", new Object[]{text}).formatted(Formatting.BOLD)}
         );
      } else {
