...
public interface When extends Supplier<JsonElement> {
   ...
   class LogicalCondition implements When {
   ...
   class PropertyCondition implements When {
      ...
      private static <T extends Comparable<T>> String name(Property<T> property, Stream<T> valueStream) {
         return valueStream.<CharSequence>map(property::name).collect(Collectors.joining("|"));
      }
