   ...
   public JsonElement get() {
      ...
      for (BlockStateVariantMap blockStateVariantMap : this.variantMaps) {
         ...
         stream = stream.flatMap(pair -> map.entrySet().stream().map(entry -> {
            PropertiesMap propertiesMap = ((PropertiesMap)pair.getFirst()).copyOf(entry.getKey());
      ...
      jsonObject.add("variants", (JsonElement)Util.make(new JsonObject(), json -> map2.forEach(json::add)));
      return jsonObject;
   }
