...
public final class NativeImage implements AutoCloseable {
   ...
   public static NativeImage read(byte[] bytes) throws IOException {
      ...
      if (i < bytes.length) {
         ...
         try {
            return putAndRead(byteBuffer, bytes);
         } finally {
            MemoryUtil.memFree(byteBuffer);
         }
      } else {
   ...
   public void upload(int level, int offsetX, int offsetY, boolean close) {
      this.upload(level, offsetX, offsetY, 0, 0, this.width, this.height, close);
   }

   public void upload(int level, int offsetX, int offsetY, int unpackSkipPixels, int unpackSkipRows, int width, int height, boolean blur) {
      if (!RenderSystem.isOnRenderThreadOrInit()) {
         RenderSystem.recordRenderCall(() -> this.uploadInternal(level, offsetX, offsetY, unpackSkipPixels, unpackSkipRows, width, height, blur));
      } else {
         this.uploadInternal(level, offsetX, offsetY, unpackSkipPixels, unpackSkipRows, width, height, blur);
      }
   }

   private void uploadInternal(int level, int offsetX, int offsetY, int unpackSkipPixels, int unpackSkipRows, int width, int height, boolean blur) {
      try {
   ...
   public void mirrorVertically() {
      ...
      try {
         for (int k = 0; k < this.getHeight() / 2; k++) {
            ...
            MemoryUtil.memCopy(this.pointer + m, l, j);
            MemoryUtil.memCopy(this.pointer + n, this.pointer + m, j);
            MemoryUtil.memCopy(l, this.pointer + n, j);
         }
      } finally {
   ...
   public enum InternalFormat {
      ...
      InternalFormat(final int value) {
         this.value = value;
      }
