...
public class PlayerSkinWidget extends ClickableWidget {
   ...
   protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.getMatrices().translate(this.getX() + this.getWidth() / 2.0F, this.getY() + this.getHeight(), 100.0F);
   ...
   record Models(PlayerEntityModel wideModel, PlayerEntityModel slimModel) {
      ...
      public void draw(DrawContext context, SkinTextures skinTextures) {
         ...
         context.draw(
            vertexConsumers -> playerEntityModel.render(context.getMatrices(), vertexConsumers.getBuffer(renderLayer), 15728880, OverlayTexture.DEFAULT_UV)
         );
         context.getMatrices().pop();
      }
   }
