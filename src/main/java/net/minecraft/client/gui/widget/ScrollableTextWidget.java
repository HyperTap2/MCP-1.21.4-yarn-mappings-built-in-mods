...
public class ScrollableTextWidget extends ScrollableTextFieldWidget {
   ...
   protected void drawBox(DrawContext context) {
      if (this.overflows()) {
         ...
         this.draw(
            context,
            this.getX() - this.getTextMargin(),
            this.getY() - this.getTextMargin(),
            this.getWidth() + this.getPadding(),
            this.getHeight() + this.getPadding()
         );
      }
   }
   ...
   protected void renderContents(DrawContext context, int mouseX, int mouseY, float delta) {
      context.getMatrices().push();
      context.getMatrices().translate(this.getTextX(), this.getTextY(), 0.0F);
      this.wrapped.render(context, mouseX, mouseY, delta);
      context.getMatrices().pop();
   }
