...
public class InGameHud {
   ...
   public void render(DrawContext context, RenderTickCounter tickCounter) {
      if (this.client.currentScreen instanceof HudHideable || Zoomify.shouldHideHud()) {
         return;
      }
      ...
      try {
         ...
         if (SodiumExtraClientMod.isMixinEnabled("core.MixinGui")) {
            SodiumExtraClientMod.onHudRender(context, tickCounter);
         }
      } finally {
         GLDebug.popGroup();
      }
   ...
   private void renderMiscOverlays(DrawContext context, RenderTickCounter tickCounter) {
      ...
      if (this.client.options.getPerspective().isFirstPerson()) {
         if (Zoomify.shouldRenderSpyglassOverlay(this.client.player.isUsingSpyglass())) {
            ...
            for (EquipmentSlot equipmentSlot : EquipmentSlot.values()) {
               ...
               if (equippableComponent != null && equippableComponent.slot() == equipmentSlot && equippableComponent.cameraOverlay().isPresent()) {
                  this.renderOverlay(
                     context, ((Identifier)equippableComponent.cameraOverlay().get()).withPath(overlayTexture -> "textures/" + overlayTexture + ".png"), 1.0F
                  );
               }
            }
         }
   ...
   private void renderOverlayMessage(DrawContext context, RenderTickCounter tickCounter) {
      ...
      if (this.overlayMessage != null && this.overlayRemaining > 0) {
         ...
         float f = this.overlayRemaining - tickCounter.getTickDelta(false);
   ...
   private void renderTitleAndSubtitle(DrawContext context, RenderTickCounter tickCounter) {
      if (this.title != null && this.titleRemainTicks > 0) {
         ...
         float f = this.titleRemainTicks - tickCounter.getTickDelta(false);
         int i = 255;
   ...
   private void renderScoreboardSidebar(DrawContext context, RenderTickCounter tickCounter) {
      ...
      ScoreboardObjective scoreboardObjective2 = scoreboardObjective != null
         ? scoreboardObjective
   ...
   private void renderCrosshair(DrawContext context, RenderTickCounter tickCounter) {
      ...
      if (gameOptions.getPerspective().isFirstPerson() || VisualSettings.INSTANCE.alwaysRenderCrosshair.isEnabled()) {
   ...
   private void renderScoreboardSidebar(DrawContext drawContext, ScoreboardObjective objective) {
      ...
      int p = 3;
      int q = drawContext.getScaledWindowWidth() - l - 3;
      ...
      drawContext.drawText(this.getTextRenderer(), text, q + l / 2 - i / 2, u - 9, -1, false);

      for (int v = 0; v < m; v++) {
         SidebarEntry sidebarEntry2 = sidebarEntrys[v];
         int w = o - (m - v) * 9;
         drawContext.drawText(this.getTextRenderer(), sidebarEntry2.name, q, w, -1, false);
         drawContext.drawText(this.getTextRenderer(), sidebarEntry2.score, r - sidebarEntry2.scoreWidth, w, -1, false);
      }
   }
   ...
   private static void renderArmor(DrawContext context, PlayerEntity player, int i, int j, int k, int x) {
      ...
      if (l > 0) {
         int m = i - (j - 1) * k - 10;
         boolean legacyHud = VisualSettings.INSTANCE.hideModernHUDElements.isEnabled();
         if (legacyHud) {
            m += 10;
         }

         for (int n = 0; n < 10; n++) {
            int o = legacyHud ? context.getScaledWindowWidth() - x - 80 + n * 8 - 1 : x + n * 8;
            if (n * 2 + 1 < l) {
   ...
   private void renderAirBubbles(DrawContext context, PlayerEntity player, int heartCount, int top, int left) {
      ...
      if (bl || j < i) {
         ...
         for (int n = 1; n <= 10; n++) {
            int o = left - (n - 1) * 8 - 9;
            if (VisualSettings.INSTANCE.hideModernHUDElements.isEnabled()) {
               o = context.getScaledWindowWidth() - o - 9;
            }
            if (n <= k) {
            ...
            } else if (n > 10 - m && !VisualSettings.INSTANCE.hideEmptyBubbleIcons.getValue()) {
   ...
   private void renderSpyglassOverlay(DrawContext context, float scale) {
      ...
      float g = f;
      float h = Math.min(context.getScaledWindowWidth() / f, context.getScaledWindowHeight() / g) * scale;
      ...
      context.fill(RenderLayer.getGuiOverlay(), 0, n, context.getScaledWindowWidth(), context.getScaledWindowHeight(), -90, -16777216);
      context.fill(RenderLayer.getGuiOverlay(), 0, 0, context.getScaledWindowWidth(), l, -90, -16777216);
      context.fill(RenderLayer.getGuiOverlay(), 0, l, k, n, -90, -16777216);
      context.fill(RenderLayer.getGuiOverlay(), m, l, context.getScaledWindowWidth(), n, -90, -16777216);
   }
   ...
   public void setRecordPlayingOverlay(Text description) {
      Text text = Text.translatable("record.nowPlaying", new Object[]{description});
