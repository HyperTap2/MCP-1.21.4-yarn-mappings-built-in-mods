...
public class TickChart extends DebugChart {
   ...
   protected void renderThresholds(DrawContext context, int x, int width, int height) {
      float f = (float)TimeHelper.SECOND_IN_MILLIS / this.millisPerTickSupplier.get();
   ...
   protected int getHeight(double value) {
      return (int)Math.round(toMillisecondsPerTick(value) * 60.0 / this.millisPerTickSupplier.get().floatValue());
   }

   @Override
   protected int getColor(long value) {
      float f = this.millisPerTickSupplier.get();
