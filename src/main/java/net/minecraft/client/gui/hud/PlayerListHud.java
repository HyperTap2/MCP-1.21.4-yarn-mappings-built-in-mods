...
public class PlayerListHud implements IPlayerListHud {
   ...
   private static final Comparator<PlayerListEntry> VIAFABRICPLUS_FIFO_ORDERING = Comparator.comparingInt(
      entry -> ((IPlayerListEntry)entry).viaFabricPlusVisuals$getIndex()
   );
   ...
   public void setVisible(boolean visible) {
      if (this.visible != visible) {
         ...
         if (visible) {
            ...
            this.client.getNarratorManager().narrate(Text.translatable("multiplayer.player.list.narration", new Object[]{text}));
         }
      }
   }
   ...
   private List<PlayerListEntry> collectPlayerEntries() {
      if (VisualSettings.INSTANCE.enableLegacyTablist.isEnabled()) {
         return this.viaFabricPlusVisuals$transpose(
            this.client.player
               .networkHandler
               .getListedPlayerListEntries()
               .stream()
               .sorted(VIAFABRICPLUS_FIFO_ORDERING)
               .limit(this.viaFabricPlusVisuals$maxSlots)
               .toList()
         );
      }
   ...
   public void render(DrawContext context, int scaledWindowWidth, Scoreboard scoreboard, @Nullable ScoreboardObjective objective) {
      ...
      boolean bl = this.client.isInSingleplayer()
         || this.client.getNetworkHandler().getConnection().isEncrypted() && !this.viaFabricPlusVisuals$hideSkins;
      int q;
      if (objective != null) {
         if (objective.getRenderType() == RenderType.HEARTS) {
            q = 90;
      ...
      for (int w = 0; w < n; w++) {
         ...
         if (w < list.size()) {
            ...
            PlayerListHud.ScoreDisplayEntry scoreDisplayEntry = list2.get(w);
            ...
            context.drawTextWithShadow(
               this.client.textRenderer, scoreDisplayEntry.name, y, z, playerListEntry2.getGameMode() == GameMode.SPECTATOR ? -1862270977 : -1
            );
   ...
   private List<PlayerListEntry> viaFabricPlusVisuals$transpose(List<PlayerListEntry> list) {
      if (list.size() != this.viaFabricPlusVisuals$maxSlots) {
         this.viaFabricPlusVisuals$hideSkins = list.stream().noneMatch(entry -> entry.getProfile().getProperties().containsKey("textures"));
         return list;
      }

      List<PlayerListEntry> result = new ArrayList<>(list.size());
      int columns = this.viaFabricPlusVisuals$maxSlots / MAX_ROWS;
      boolean anyHasSkinData = false;

      for (int i = 0; i < this.viaFabricPlusVisuals$maxSlots; i++) {
         int row = i % MAX_ROWS;
         int column = i / MAX_ROWS;
         PlayerListEntry entry = list.get(row * columns + column);
         result.add(entry);
         anyHasSkinData |= entry.getProfile().getProperties().containsKey("textures");
      }

      this.viaFabricPlusVisuals$hideSkins = !anyHasSkinData;
      return result;
   }

   @Override
   public void viaFabricPlusVisuals$setMaxPlayers(int maxPlayers) {
      this.viaFabricPlusVisuals$maxSlots = Math.min(200, Math.max(MAX_ROWS, MathHelper.ceilDiv(maxPlayers, MAX_ROWS) * MAX_ROWS));
   }
   ...
   private void renderHearts(int y, int left, int right, UUID uuid, DrawContext context, int score) {
      ...
      if (i > 0) {
         ...
         if (k <= 3) {
            ...
            float g = score / 2.0F;
            Text text = Text.translatable("multiplayer.player.list.hp", new Object[]{g});
            Text text2;
            ...
            for (int l = 0; l < i; l++) {
               ...
               if (l * 2 + 1 < score) {
                  context.drawGuiTexture(
                     RenderLayer::getGuiTextured, l >= 10 ? ABSORBING_FULL_HEART_BLINKING_TEXTURE : FULL_HEART_TEXTURE, left + l * k, y, 9, 9
                  );
               }

               if (l * 2 + 1 == score) {
                  context.drawGuiTexture(
                     RenderLayer::getGuiTextured, l >= 10 ? ABSORBING_HALF_HEART_BLINKING_TEXTURE : HALF_HEART_TEXTURE, left + l * k, y, 9, 9
                  );
               }
            }
         }
