...
public class CommandBlockScreen extends AbstractCommandBlockScreen {
   ...
   protected void init() {
      ...
      this.modeButton = this.addDrawableChild(
         CyclingButtonWidget.<Type>builder(value -> {
               return switch (value) {
      ...
      if (DebugSettings.INSTANCE.hideModernCommandBlockScreenFeatures.isEnabled()) {
         ...
         this.conditionalModeButton.visible = false;
         this.redstoneTriggerButton.visible = false;
         this.setButtonsActive(false);
      }
   }
