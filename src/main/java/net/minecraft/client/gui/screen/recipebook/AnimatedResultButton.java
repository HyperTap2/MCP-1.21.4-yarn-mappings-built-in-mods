...
public class AnimatedResultButton extends ClickableWidget {
   ...
   public void showResultCollection(RecipeResultCollection resultCollection, boolean filteringCraftable, RecipeBookResults results, ContextParameterMap context) {
      ...
      List<NetworkRecipeId> list2 = list.stream().<NetworkRecipeId>map(RecipeDisplayEntry::id).filter(results.getRecipeBook()::isHighlighted).toList();
      if (!list2.isEmpty()) {
   ...
   public void appendClickableNarrations(NarrationMessageBuilder builder) {
      builder.put(NarrationPart.TITLE, Text.translatable("narration.recipe", new Object[]{this.getDisplayStack().getName()}));
      if (this.hasMultipleResults()) {
