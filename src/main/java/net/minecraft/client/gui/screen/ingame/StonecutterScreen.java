...
public class StonecutterScreen extends HandledScreen<StonecutterScreenHandler> {
   ...
   protected void drawMouseoverTooltip(DrawContext drawContext, int x, int y) {
      ...
      if (this.canCraft) {
         ...
         Grouping<StonecuttingRecipe> grouping = this.handler.getAvailableRecipes();
         ...
         for (int l = this.scrollOffset; l < k && l < grouping.size(); l++) {
            ...
            if (x >= n && x < n + 16 && y >= o && y < o + 18) {
               ...
               SlotDisplay slotDisplay = ((GroupEntry)grouping.entries().get(l)).recipe().optionDisplay();
   ...
   private void renderRecipeIcons(DrawContext context, int x, int y, int scrollOffset) {
      ...
      for (int i = this.scrollOffset; i < scrollOffset && i < grouping.size(); i++) {
         ...
         int l = j / 4;
         int m = y + l * 18 + 2;
         SlotDisplay slotDisplay = ((GroupEntry)grouping.entries().get(i)).recipe().optionDisplay();
