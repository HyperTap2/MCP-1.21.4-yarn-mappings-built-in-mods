...
public class DisconnectedScreen extends Screen {
   ...
   protected void init() {
      ...
      this.info
         .report()
         .ifPresent(
            path -> this.grid.add(ButtonWidget.builder(OPEN_REPORT_DIR_TEXT, button -> Util.getOperatingSystem().open(path.getParent())).width(200).build())
         );
   ...
   public Text getNarratedTitle() {
      return ScreenTexts.joinSentences(new Text[]{this.title, this.info.reason()});
   }
