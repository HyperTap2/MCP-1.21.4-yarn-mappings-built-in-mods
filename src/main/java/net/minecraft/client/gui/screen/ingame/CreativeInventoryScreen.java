...
public class CreativeInventoryScreen extends HandledScreen<CreativeInventoryScreen.CreativeScreenHandler> {
   ...
   public CreativeInventoryScreen(ClientPlayerEntity player, FeatureSet enabledFeatures, boolean operatorTabEnabled) {
      ...
      this.populateDisplay(
         player.networkHandler.getSearchManager(), enabledFeatures, this.shouldShowOperatorTab(player), player.getWorld().getRegistryManager()
      );
   ...
   private void updateDisplayParameters(FeatureSet enabledFeatures, boolean showOperatorTab, WrapperLookup registries) {
      ClientPlayNetworkHandler clientPlayNetworkHandler = this.client.getNetworkHandler();
      if (this.populateDisplay(
         clientPlayNetworkHandler != null ? clientPlayNetworkHandler.getSearchManager() : null, enabledFeatures, showOperatorTab, registries
      )) {
   ...
   private boolean populateDisplay(@Nullable SearchManager searchManager, FeatureSet enabledFeatures, boolean showOperatorTab, WrapperLookup registries) {
      ...
      if (actionType != SlotActionType.THROW || this.client.player.canDropItems()) {
         ...
         if (slot == null && selectedTab.getType() != Type.INVENTORY && actionType != SlotActionType.QUICK_CRAFT) {
            ...
            if (slot == this.deleteItemSlot && bl) {
               ...
               } else if (slot != null && PlayerInventory.isValidHotbarIndex(slot.getIndex()) && selectedTab.getType() != Type.INVENTORY) {
                  if (actionType == SlotActionType.THROW && !itemStack.isEmpty() && !this.handler.getCursorStack().isEmpty()) {
   ...
   protected void init() {
      ...
      if (VisualSettings.INSTANCE.replaceCreativeInventory.isEnabled()) {
         this.client.setScreen(GridItemSelectionScreen.INSTANCE);
      }
   }
   ...
   private void searchForTags(String id) {
      ...
      Registries.ITEM.streamTags().<TagKey>map(Named::getTag).filter(tag -> predicate.test(tag.id())).forEach(this.searchResultTags::add);
   }
   ...
   public List<Text> getTooltipFromItem(ItemStack stack) {
      ...
      Default default_ = this.client.options.advancedItemTooltips ? Default.ADVANCED : Default.BASIC;
      TooltipType tooltipType = bl ? default_.withCreative() : default_;
      List<Text> list = stack.getTooltip(TooltipContext.create(this.client.world), this.client.player, tooltipType);
      if (bl2 && bl) {
         return list;
      }
   ...
   protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
      ...
      context.drawTexture(
         RenderLayer::getGuiTextured, selectedTab.getTexture(), this.x, this.y, 0.0F, 0.0F, this.backgroundWidth, this.backgroundHeight, 256, 256
      );
