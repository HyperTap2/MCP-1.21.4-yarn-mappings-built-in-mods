...
public class MultiplayerScreen extends Screen {
   ...
   protected void init() {
      ...
      int buttonPosition = GeneralSettings.INSTANCE.multiplayerScreenButtonOrientation.getIndex();
      if (buttonPosition != 0) {
   ...
   private void directConnect(boolean confirmedAction) {
      if (confirmedAction) {
         ...
         if (serverInfo == null) {
            ...
            this.serverList.saveFile();
            this.selectedEntry.viaFabricPlus$passDirectConnectScreen(true);
            this.connect(this.selectedEntry);
         } else {
            serverInfo.viaFabricPlus$passDirectConnectScreen(true);
   ...
   private void connect(ServerInfo entry) {
      ProtocolVersion version = entry.viaFabricPlus$passedDirectConnectScreen()
         ? ProtocolTranslator.getTargetVersion()
         : entry.viaFabricPlus$forcedVersion();
      ConnectScreen.connect(this, this.client, ServerAddress.parse(BedrockSettings.replaceDefaultPort(entry.address, version)), entry, false, null);
   }
