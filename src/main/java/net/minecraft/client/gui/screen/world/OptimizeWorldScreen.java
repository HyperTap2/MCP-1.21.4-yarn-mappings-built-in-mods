...
public class OptimizeWorldScreen extends Screen {
   ...
   private static final ToIntFunction<RegistryKey<World>> DIMENSION_COLORS = (ToIntFunction<RegistryKey<World>>)Util.make(
   ...
   private OptimizeWorldScreen(
      BooleanConsumer callback, DataFixer dataFixer, Session storageSession, LevelInfo levelInfo, boolean eraseCache, DynamicRegistryManager registryManager
   ) {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      if (this.updater.getTotalChunkCount() > 0) {
         ...
         Text text = Text.translatable("optimizeWorld.progress.counter", new Object[]{o, this.updater.getTotalChunkCount()});
         Text text2 = Text.translatable("optimizeWorld.progress.percentage", new Object[]{MathHelper.floor(this.updater.getProgress() * 100.0F)});
