...
public class AbuseReportTypeScreen extends Screen {
   ...
   public Text getNarratedTitle() {
      return ScreenTexts.joinSentences(new Text[]{super.getNarratedTitle(), MESSAGE_TEXT});
   }
   ...
   protected void init() {
      ...
      ButtonWidget buttonWidget = this.layout
         .add(
            ButtonWidget.builder(
                  CHAT_TYPE_TEXT, button -> this.client.setScreen(new ChatReportScreen(this.parent, this.context, this.selectedPlayer.getUuid()))
               )
               .build()
         );
      if (!this.selectedPlayer.isReportable()) {
         ...
         buttonWidget.active = false;
         buttonWidget.setTooltip(
            Tooltip.of(Text.translatable("gui.socialInteractions.tooltip.report.no_messages", new Object[]{this.selectedPlayer.getName()}))
         );
      }
      ...
      this.layout
         .add(
            ButtonWidget.builder(
                  SKIN_TYPE_TEXT,
                  button -> this.client
                     .setScreen(new SkinReportScreen(this.parent, this.context, this.selectedPlayer.getUuid(), this.selectedPlayer.getSkinSupplier()))
               )
               .build()
         );
      this.layout
         .add(
            ButtonWidget.builder(
                  NAME_TYPE_TEXT,
                  button -> this.client
                     .setScreen(new UsernameReportScreen(this.parent, this.context, this.selectedPlayer.getUuid(), this.selectedPlayer.getName()))
               )
               .build()
         );
