...
public class JigsawBlockScreen extends Screen {
   ...
   private CyclingButtonWidget<Joint> jointRotationButton;
   ...
   protected void init() {
      ...
      this.jointRotationButton = this.addDrawableChild(
         CyclingButtonWidget.<Joint>builder(Joint::asText)
            .values(Joint.values())
      ...
      this.addDrawableChild(new SliderWidget(this.width / 2 - 154, 185, 100, 20, ScreenTexts.EMPTY, 0.0) {
         ...
         protected void updateMessage() {
            this.setMessage(Text.translatable("jigsaw_block.levels", new Object[]{JigsawBlockScreen.this.generationDepth}));
         }
      ...
      if (Boolean.TRUE.equals(DebugSettings.INSTANCE.hideModernJigsawScreenFeatures.getValue())) {
         if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20_2)) {
            this.selectionPriorityField.active = false;
            this.placementPriorityField.active = false;
         }

         if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_15_2)) {
            this.nameField.active = false;
            this.jointRotationButton.active = false;
            int jointButtonIndex = this.children().indexOf(this.jointRotationButton);
            ((ClickableWidget)this.children().get(jointButtonIndex + 1)).active = false;
            ((ClickableWidget)this.children().get(jointButtonIndex + 2)).active = false;
            ((ClickableWidget)this.children().get(jointButtonIndex + 3)).active = false;
         }
      }
   }
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      if (Boolean.TRUE.equals(DebugSettings.INSTANCE.hideModernJigsawScreenFeatures.getValue())
         ...
         this.nameField.setText(this.targetField.getText());
      }
