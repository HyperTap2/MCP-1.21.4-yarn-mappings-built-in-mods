...
public class EnchantmentScreen extends HandledScreen<EnchantmentScreenHandler> {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      for (int j = 0; j < 3; j++) {
         ...
         Optional<Reference<Enchantment>> optional = this.client
            .world
            .getRegistryManager()
         ...
         if (!optional.isEmpty()) {
            ...
            if (this.isPointWithinBounds(60, 14 + 19 * j, 108, 17, mouseX, mouseY) && k > 0 && l >= 0 && optional != null) {
               ...
               list.add(
                  Text.translatable("container.enchant.clue", new Object[]{Enchantment.getName((RegistryEntry)optional.get(), l)}).formatted(Formatting.WHITE)
               );
               if (!bl) {
                  list.add(ScreenTexts.EMPTY);
                  if (this.client.player.experienceLevel < k) {
                     list.add(
                        Text.translatable("container.enchant.level.requirement", new Object[]{this.handler.enchantmentPower[j]}).formatted(Formatting.RED)
                     );
                  } else {
