...
public class GameMenuScreen extends Screen {
   ...
   private void initWidgets() {
      boolean authenticLayout = VisualSettings.INSTANCE.changeGameMenuScreenLayout.getIndex() == 0;
      boolean betaLayout = authenticLayout
         && ViaFabricPlus.getImpl().getTargetVersion().olderThanOrEqualTo(LegacyProtocolVersion.b1_4tob1_4_1);
      GridWidget gridWidget = new GridWidget();
      gridWidget.getMainPositioner().margin(4, 4, 4, 0);
      GridWidget.Adder adder = gridWidget.createAdder(2);
      ButtonWidget returnToGameButton = ButtonWidget.builder(RETURN_TO_GAME_TEXT, button -> {
         this.client.setScreen(null);
         this.client.mouse.lockCursor();
      }).width(204).build();
      adder.add(returnToGameButton, 2, gridWidget.copyPositioner().marginTop(50));

      Text exitText = this.client.isInSingleplayer() ? RETURN_TO_MENU_TEXT : ScreenTexts.DISCONNECT;
      ButtonWidget disconnectButton = ButtonWidget.builder(exitText, button -> {
         button.active = false;
         this.client.getAbuseReportContext().tryShowDraftScreen(this.client, this, this::disconnect, true);
      }).width(204).build();
      this.exitButton = disconnectButton;

      if (betaLayout) {
         disconnectButton.setWidth(NORMAL_BUTTON_WIDTH);
         adder.add(disconnectButton);
      } else {
      ...
      if (serverLinks.isEmpty()) {
         if (!authenticLayout || ViaFabricPlus.getImpl().getTargetVersion().newerThan(ProtocolVersion.v1_13_2)) {
      ...
      ButtonWidget optionsButton = this.createButton(OPTIONS_TEXT, () -> new OptionsScreen(this, this.client.options));
      adder.add(optionsButton);
      ButtonWidget shareToLanButton = null;
      if (this.client.isIntegratedServerRunning() && !this.client.getServer().isRemote()) {
         if (!authenticLayout || ViaFabricPlus.getImpl().getTargetVersion().newerThan(LegacyProtocolVersion.r1_2_4tor1_2_5)) {
            shareToLanButton = this.createButton(SHARE_TO_LAN_TEXT, () -> new OpenToLanScreen(this));
            adder.add(shareToLanButton);
         }
      } else {
         if (ViaFabricPlus.getImpl().getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_19)
            && VisualSettings.INSTANCE.changeGameMenuScreenLayout.getIndex() != 2) {
            if (!authenticLayout) {
               adder.add(this.createButton(Text.translatable("gui.socialInteractions.title"), () -> new SocialInteractionsScreen(this)));
            } else if (ViaFabricPlus.getImpl().getTargetVersion().newerThan(LegacyProtocolVersion.r1_2_4tor1_2_5)) {
               shareToLanButton = this.createButton(SHARE_TO_LAN_TEXT, () -> new OpenToLanScreen(this));
               shareToLanButton.active = false;
               adder.add(shareToLanButton);
            }
         } else {
      ...
      if (!betaLayout) {
         adder.add(disconnectButton, 2);
      }

      gridWidget.refreshPositions();
      SimplePositioningWidget.setPos(gridWidget, 0, 0, this.width, this.height, 0.5F, 0.25F);
      if (authenticLayout && ViaFabricPlus.getImpl().getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_13_2)) {
         optionsButton.setY(optionsButton.getY() + 20);
         if (shareToLanButton != null) {
            shareToLanButton.setY(shareToLanButton.getY() + 20);
         }
         disconnectButton.setY(disconnectButton.getY() + 20);

         if (ViaFabricPlus.getImpl().getTargetVersion().olderThanOrEqualTo(LegacyProtocolVersion.r1_2_4tor1_2_5)) {
            optionsButton.setX(returnToGameButton.getX());
            optionsButton.setWidth(returnToGameButton.getWidth());
         }

         if (betaLayout) {
            optionsButton.setY(optionsButton.getY() + 20);
            disconnectButton.setY(returnToGameButton.getY() + 23);
         }
      }
   ...
   static void addFeedbackAndBugsButtons(Screen parentScreen, GridWidget.Adder gridAdder) {
      gridAdder.add(
         createUrlButton(parentScreen, SEND_FEEDBACK_TEXT, SharedConstants.getGameVersion().isStable() ? Urls.JAVA_FEEDBACK : Urls.SNAPSHOT_FEEDBACK)
      );
