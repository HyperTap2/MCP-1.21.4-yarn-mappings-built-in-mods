...
public class EditGameRulesScreen extends Screen {
   ...
   protected void init() {
      ...
      this.doneButton = directionalLayoutWidget.add(
         ButtonWidget.builder(ScreenTexts.DONE, button -> this.ruleSaver.accept(Optional.of(this.gameRules))).build()
      );
   ...
   public class BooleanRuleWidget extends EditGameRulesScreen.NamedRuleWidget {
      ...
      public BooleanRuleWidget(final Text name, final List<OrderedText> description, final String ruleName, final BooleanRule rule) {
   ...
   public class IntRuleWidget extends EditGameRulesScreen.NamedRuleWidget {
      ...
      public IntRuleWidget(final Text name, final List<OrderedText> description, final String ruleName, final IntRule rule) {
         super(description, name);
   ...
   public abstract class NamedRuleWidget extends EditGameRulesScreen.AbstractRuleWidget {
      ...
      protected void drawName(DrawContext context, int x, int y) {
         if (this.name.size() == 1) {
            context.drawTextWithShadow(EditGameRulesScreen.this.client.textRenderer, this.name.get(0), y, x + 5, -1);
         } else if (this.name.size() >= 2) {
            context.drawTextWithShadow(EditGameRulesScreen.this.client.textRenderer, this.name.get(0), y, x, -1);
            context.drawTextWithShadow(EditGameRulesScreen.this.client.textRenderer, this.name.get(1), y, x + 10, -1);
         }
      }
   }
   ...
   public class RuleCategoryWidget extends EditGameRulesScreen.AbstractRuleWidget {
      ...
      public void render(
         ...
         context.drawCenteredTextWithShadow(EditGameRulesScreen.this.client.textRenderer, this.name, x + entryWidth / 2, y + 5, -1);
      }
   ...
   public class RuleListWidget extends ElementListWidget<EditGameRulesScreen.AbstractRuleWidget> {
      ...
      public RuleListWidget(final GameRules gameRules) {
         ...
         final Map<Category, Map<Key<?>, EditGameRulesScreen.AbstractRuleWidget>> map = Maps.newHashMap();
         gameRules.accept(
            new Visitor() {
               ...
               private <T extends Rule<T>> void createRuleWidget(Key<T> key, EditGameRulesScreen.RuleWidgetFactory<T> widgetFactory) {
                  Text text = Text.translatable(key.getTranslationKey());
                  Text text2 = Text.literal(key.getName()).formatted(Formatting.YELLOW);
                  T rule = (T)gameRules.get(key);
                  String string = rule.serialize();
                  Text text3 = Text.translatable("editGamerule.default", new Object[]{Text.literal(string)}).formatted(Formatting.GRAY);
                  ...
                  map.computeIfAbsent(key.getCategory(), category -> Maps.newHashMap()).put(key, widgetFactory.create(text, list, string3, rule));
               }
            }
         );
         map.entrySet()
               ...
               entry -> {
                  this.addEntry(
                     EditGameRulesScreen.this.new RuleCategoryWidget(
                        Text.translatable(entry.getKey().getCategory()).formatted(new Formatting[]{Formatting.BOLD, Formatting.YELLOW})
                     )
                  );
                  entry.getValue()
                     .entrySet()
                     .stream()
                     .sorted(Map.Entry.comparingByKey(Comparator.comparing(Key::getName)))
   ...
   @FunctionalInterface
   interface RuleWidgetFactory<T extends Rule<T>> {
