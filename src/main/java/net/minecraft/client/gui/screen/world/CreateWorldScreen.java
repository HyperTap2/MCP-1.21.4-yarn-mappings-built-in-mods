...
public class CreateWorldScreen extends Screen {
   ...
   public static void show(MinecraftClient client, @Nullable Screen parent, CreateWorldCallback callback) {
      ...
      Function<LoadContextSupplierContext, WorldGenSettings> function = context -> new WorldGenSettings(
   ...
   private static void show(
      ...
      Function<LoadContextSupplierContext, WorldGenSettings> settingsSupplier,
      ...
      ServerConfig serverConfig = createServerConfig(resourcePackManager, DataConfiguration.SAFE_MODE);
      ...
      client.setScreen(new CreateWorldScreen(client, parent, completableFuture.join(), Optional.of(presetKey), OptionalLong.empty(), callback));
   }
   ...
   private void createLevel() {
      ...
      DimensionsConfig dimensionsConfig = generatorOptionsHolder.selectedDimensions().toConfig(generatorOptionsHolder.dimensionOptionsRegistry());
      CombinedDynamicRegistries<ServerDynamicRegistryType> combinedDynamicRegistries = generatorOptionsHolder.combinedDynamicRegistries()
         .with(ServerDynamicRegistryType.DIMENSIONS, new Immutable[]{dimensionsConfig.toDynamicRegistryManager()});
      ...
      LevelInfo levelInfo = this.createLevelInfo(dimensionsConfig.specialWorldProperty() == SpecialProperty.DEBUG);
   ...
   private boolean startServer(CombinedDynamicRegistries<ServerDynamicRegistryType> combinedDynamicRegistries, SaveProperties saveProperties) {
      ...
      Optional<Session> optional = createSession(this.client, string, this.dataPackTempDir);
      if (optional.isEmpty()) {
         ...
         this.client
            ...
            .startNewWorld(optional.get(), generatorOptionsHolder.dataPackContents(), combinedDynamicRegistries, saveProperties);
         return true;
      }
   }
   ...
   private LevelInfo createLevelInfo(boolean debugWorld) {
      ...
      if (debugWorld) {
         ...
         ((BooleanRule)gameRules.get(GameRules.DO_DAYLIGHT_CYCLE)).set(false, null);
         ...
         this.client
            .setScreen(
               new ExperimentsScreen(
                  ...
                  resourcePackManager -> this.applyDataPacks(resourcePackManager, false, this::openExperimentsScreen)
               )
            );
      }
   }
   ...
   private void applyDataPacks(ResourcePackManager dataPackManager, boolean fromPackScreen, Consumer<DataConfiguration> configurationSetter) {
      ...
      List<String> list2 = dataPackManager.getIds().stream().filter(name -> !list.contains(name)).collect(ImmutableList.toImmutableList());
   ...
   private void validateDataPacks(ResourcePackManager dataPackManager, DataConfiguration dataConfiguration, Consumer<DataConfiguration> configurationSetter) {
      ...
      ServerConfig serverConfig = createServerConfig(dataPackManager, dataConfiguration);
      SaveLoading.<WorldCreationSettings, GeneratorOptionsHolder>load(
            ...
            context -> {
               ...
               WorldGenSettings worldGenSettings = dataResult.flatMap(json -> WorldGenSettings.CODEC.parse(dynamicOps2, json))
                  .getOrThrow(error -> new IllegalStateException("Error parsing worldgen settings after loading data packs: " + error));
               return new LoadContext(new WorldCreationSettings(worldGenSettings, context.dataConfiguration()), context.dimensionsRegistryManager());
            },
            (resourceManager, dataPackContents, combinedDynamicRegistries, context) -> {
               WorldCreationSettings ctx = (WorldCreationSettings)context;
               resourceManager.close();
               return new GeneratorOptionsHolder(ctx.worldGenSettings(), combinedDynamicRegistries, dataPackContents, ctx.dataConfiguration());
            },
            Util.getMainWorkerExecutor(),
            this.client
         )
         .thenApply(g -> {
            GeneratorOptionsHolder generatorOptionsHolder = (GeneratorOptionsHolder)g;
            generatorOptionsHolder.initializeIndexedFeaturesLists();
            return generatorOptionsHolder;
         })
         .thenAcceptAsync(g -> this.worldCreator.setGeneratorOptionsHolder((GeneratorOptionsHolder)g), this.client)
         .handleAsync(
            (void_, throwable) -> {
               if (throwable != null) {
   ...
   private static ServerConfig createServerConfig(ResourcePackManager dataPackManager, DataConfiguration dataConfiguration) {
      DataPacks dataPacks = new DataPacks(dataPackManager, dataConfiguration, false, true);
      return new ServerConfig(dataPacks, RegistrationEnvironment.INTEGRATED, 2);
   }
   ...
   private static Optional<Session> createSession(MinecraftClient client, String worldDirectoryName, @Nullable Path dataPackTempDir) {
      try {
         Session session = client.getLevelStorage().createSessionWithoutSymlinkCheck(worldDirectoryName);
         ...
         LOGGER.warn("Failed to create access for {}", worldDirectoryName, exception2);
      }
   ...
   public static Path copyDataPack(Path srcFolder, MinecraftClient client) {
      ...
      try (Stream<Path> stream = Files.walk(srcFolder)) {
         ...
         LOGGER.warn("Failed to copy datapacks from world {}", srcFolder, exception);
         ...
         CreateWorldScreen.this.worldCreator
            .addListener(
               creator -> this.worldNameField
                  .setTooltip(
                     Tooltip.of(
                        Text.translatable("selectWorld.targetFolder", new Object[]{Text.literal(creator.getWorldDirectoryName()).formatted(Formatting.ITALIC)})
                     )
                  )
            );
   ...
   class MoreTab extends GridScreenTab {
      ...
      MoreTab() {
         ...
         adder.add(
            ButtonWidget.builder(
                  DATA_PACKS_TEXT,
                  button -> CreateWorldScreen.this.openPackScreen(CreateWorldScreen.this.worldCreator.getGeneratorOptionsHolder().dataConfiguration())
               )
               .width(210)
               .build()
         );
   ...
   class WorldTab extends GridScreenTab {
      ...
      WorldTab() {
         ...
         CyclingButtonWidget<WorldCreator.WorldType> cyclingButtonWidget = adder.add(
            CyclingButtonWidget.builder(WorldCreator.WorldType::getName)
         ...
         CreateWorldScreen.this.worldCreator
            .addListener(creator -> this.customizeButton.active = !creator.isDebug() && creator.getLevelScreenProvider() != null);
         ...
         builder.add(
               MAP_FEATURES_TEXT, CreateWorldScreen.this.worldCreator::shouldGenerateStructures, CreateWorldScreen.this.worldCreator::setGenerateStructures
            )
      ...
      private static MutableText getWorldTypeNarrationMessage(CyclingButtonWidget<WorldCreator.WorldType> worldTypeButton) {
         return worldTypeButton.getValue().isAmplified()
            ? ScreenTexts.joinSentences(new Text[]{worldTypeButton.getGenericNarrationMessage(), AMPLIFIED_GENERATOR_INFO_TEXT})
