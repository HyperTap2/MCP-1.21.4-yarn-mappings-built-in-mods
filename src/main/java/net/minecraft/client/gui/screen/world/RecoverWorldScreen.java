...
public class RecoverWorldScreen extends Screen {
   ...
   public RecoverWorldScreen(MinecraftClient client, BooleanConsumer callback, Session session) {
      super(TITLE_TEXT);
      this.callback = callback;
      this.message = Text.translatable("recover_world.message", new Object[]{Text.literal(session.getDirectoryName()).formatted(Formatting.GRAY)});
   ...
   private Text toText(Session session, boolean old, @Nullable Exception exception) {
      ...
      mutableText.append(Text.translatable("recover_world.state_entry", new Object[]{mutableText2.formatted(Formatting.GRAY)}));
      if (exception == null) {
   ...
   @Nullable
   private Exception getLoadingException(Session session, boolean old) {
      try {
         if (!old) {
   ...
   public Text getNarratedTitle() {
      return ScreenTexts.joinSentences(new Text[]{super.getNarratedTitle(), this.message});
   }
