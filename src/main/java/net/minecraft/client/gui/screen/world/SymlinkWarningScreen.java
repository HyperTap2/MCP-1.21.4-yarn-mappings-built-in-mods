...
public class SymlinkWarningScreen extends Screen {
   ...
   private static final Text WORLD_MESSAGE = Text.translatable("symlink_warning.message.world", new Object[]{Text.of(Urls.MINECRAFT_SYMLINKS)});
   private static final Text PACK_TITLE = Text.translatable("symlink_warning.title.pack").formatted(Formatting.BOLD);
   private static final Text PACK_MESSAGE = Text.translatable("symlink_warning.message.pack", new Object[]{Text.of(Urls.MINECRAFT_SYMLINKS)});
   ...
   protected void init() {
      ...
      adder2.add(
         ButtonWidget.builder(ScreenTexts.COPY_LINK_TO_CLIPBOARD, button -> this.client.keyboard.setClipboard(this.link.toString())).size(120, 20).build()
      );
