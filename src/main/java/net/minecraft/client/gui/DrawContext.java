...
public class DrawContext {
   ...
   public void fill(RenderLayer layer, int x1, int y1, int x2, int y2, int z, int color) {
      ...
      if (!SodiumExtraClientMod.isMixinEnabled("optimizations.draw_helpers.MixinGuiGraphics")) {
      ...
      try (MemoryStack stack = MemoryStack.stackPush()) {
         ...
         ColorVertex.put(buffer, matrix4f, x1, y1, z, abgr);
         ColorVertex.put(buffer + 16L, matrix4f, x1, y2, z, abgr);
         ColorVertex.put(buffer + 32L, matrix4f, x2, y2, z, abgr);
         ColorVertex.put(buffer + 48L, matrix4f, x2, y1, z, abgr);
         VertexBufferWriter.of(vertexConsumer).push(stack, buffer, 4, ColorVertex.FORMAT);
      }
   }
   ...
   private void fillGradient(VertexConsumer vertexConsumer, int startX, int startY, int endX, int endY, int z, int colorStart, int colorEnd) {
      ...
      if (!SodiumExtraClientMod.isMixinEnabled("optimizations.draw_helpers.MixinGuiGraphics")) {
         ...
         vertexConsumer.vertex(matrix4f, startX, endY, z).color(colorEnd);
         vertexConsumer.vertex(matrix4f, endX, endY, z).color(colorEnd);
         vertexConsumer.vertex(matrix4f, endX, startY, z).color(colorStart);
         return;
      }

      int startAbgr = ColorARGB.toABGR(colorStart);
      int endAbgr = ColorARGB.toABGR(colorEnd);
      try (MemoryStack stack = MemoryStack.stackPush()) {
         long buffer = stack.nmalloc(64);
         ColorVertex.put(buffer, matrix4f, startX, startY, z, startAbgr);
         ColorVertex.put(buffer + 16L, matrix4f, startX, endY, z, endAbgr);
         ColorVertex.put(buffer + 32L, matrix4f, endX, endY, z, endAbgr);
         ColorVertex.put(buffer + 48L, matrix4f, endX, startY, z, startAbgr);
         VertexBufferWriter.of(vertexConsumer).push(stack, buffer, 4, ColorVertex.FORMAT);
      }
   }
   ...
   public void fillWithLayer(RenderLayer layer, int startX, int startY, int endX, int endY, int z) {
      Matrix4f matrix4f = this.matrices.peek().getPositionMatrix();
      VertexConsumer vertexConsumer = this.vertexConsumers.getBuffer(layer);
      vertexConsumer.vertex(matrix4f, startX, startY, z);
      vertexConsumer.vertex(matrix4f, startX, endY, z);
      vertexConsumer.vertex(matrix4f, endX, endY, z);
      vertexConsumer.vertex(matrix4f, endX, startY, z);
   }
   ...
   public int drawText(TextRenderer textRenderer, @Nullable String text, int x, int y, int color, boolean shadow) {
      return text == null
         ? 0
         : textRenderer.draw(
            text, x, y, color, shadow, this.matrices.peek().getPositionMatrix(), this.vertexConsumers, TextRenderer.TextLayerType.NORMAL, 0, 15728880
         );
   }
   ...
   private void drawTexturedQuad(
      ...
      int abgr = ColorARGB.toABGR(color);
      try (MemoryStack stack = MemoryStack.stackPush()) {
         long buffer = stack.nmalloc(96);
         TextureColorVertex.write(buffer, matrix4f, x1, y1, 0.0F, abgr, u1, v1);
         TextureColorVertex.write(buffer + 24L, matrix4f, x1, y2, 0.0F, abgr, u1, v2);
         TextureColorVertex.write(buffer + 48L, matrix4f, x2, y2, 0.0F, abgr, u2, v2);
         TextureColorVertex.write(buffer + 72L, matrix4f, x2, y1, 0.0F, abgr, u2, v1);
         VertexBufferWriter.of(vertexConsumer).push(stack, buffer, 4, TextureColorVertex.FORMAT);
      }
   }
   ...
   private void drawItem(@Nullable LivingEntity entity, @Nullable World world, ItemStack stack, int x, int y, int seed, int z) {
      if (!stack.isEmpty()) {
         ...
         try {
            ...
            // The hard-merged accelerated item path does not yet preserve vanilla GUI
            // normal lighting for 3D block models. Keep GUI items on the vanilla vertex
            // path while retaining acceleration for world, hand, and entity rendering.
   ...
   public void drawItemTooltip(TextRenderer textRenderer, ItemStack stack, int x, int y) {
      this.drawTooltip(
         textRenderer, Screen.getTooltipFromItem(this.client, stack), stack.getTooltipData(), x, y, (Identifier)stack.get(DataComponentTypes.TOOLTIP_STYLE)
      );
   }
   ...
   public void drawTooltip(TextRenderer textRenderer, List<Text> text, Optional<TooltipData> data, int x, int y, @Nullable Identifier texture) {
      List<TooltipComponent> list = text.stream().<OrderedText>map(Text::asOrderedText).map(TooltipComponent::of).collect(Util.toArrayList());
   ...
   public void drawTooltip(TextRenderer textRenderer, List<Text> text, int x, int y, @Nullable Identifier texture) {
      this.drawTooltip(
         textRenderer, text.stream().<OrderedText>map(Text::asOrderedText).map(TooltipComponent::of).toList(), x, y, HoveredTooltipPositioner.INSTANCE, texture
      );
   }
   ...
   private void drawTooltip(
      ...
      if (!components.isEmpty()) {
         ...
         this.matrices.push();
         int p = 400;
         TooltipBackgroundRenderer.render(this, n, o, l, m, 400, texture);
   ...
   private void drawItemBar(ItemStack stack, int x, int y) {
      if (stack.isItemBarVisible()) {
         int i = x + 2;
         int j = y + 13;
         this.fill(RenderLayer.getGui(), i, j, i + 13, j + 2, 200, -16777216);
   ...
   private void drawStackCount(TextRenderer textRenderer, ItemStack stack, int x, int y, @Nullable String stackCountText) {
      int count = NegativeItemUtil.getCount(stack);
      if (count != 1 || stackCountText != null) {
         String string = stackCountText == null ? (count <= 0 ? Formatting.RED.toString() + count : String.valueOf(count)) : stackCountText;
         this.matrices.push();
   ...
   public void drawHoverEvent(TextRenderer textRenderer, @Nullable Style style, int x, int y) {
      if (style != null && style.getHoverEvent() != null) {
         ...
         ItemStackContent itemStackContent = (ItemStackContent)hoverEvent.getValue(Action.SHOW_ITEM);
         if (itemStackContent != null) {
            this.drawItemTooltip(textRenderer, itemStackContent.asStack(), x, y);
         } else {
            EntityContent entityContent = (EntityContent)hoverEvent.getValue(Action.SHOW_ENTITY);
            if (entityContent != null) {
               if (this.client.options.advancedItemTooltips) {
                  this.drawTooltip(textRenderer, entityContent.asTooltip(), x, y);
               }
            } else {
               Text text = (Text)hoverEvent.getValue(Action.SHOW_TEXT);
               if (text != null) {
