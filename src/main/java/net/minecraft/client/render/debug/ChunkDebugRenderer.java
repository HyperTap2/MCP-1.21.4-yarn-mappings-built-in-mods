...
public class ChunkDebugRenderer implements DebugRenderer.Renderer {
   ...
   public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
      ...
      if (this.client.debugChunkInfo || this.client.debugChunkOcclusion) {
         ...
         ObjectListIterator matrix4f2 = worldRenderer.getBuiltChunks().iterator();

         while (matrix4f2.hasNext()) {
            ChunkBuilder.BuiltChunk builtChunk = (ChunkBuilder.BuiltChunk)matrix4f2.next();
               ...
               if (this.client.debugChunkInfo) {
                  ...
                  for (int m = 0; m < DIRECTIONS.length; m++) {
                     if (chunkInfo.hasDirection(m)) {
                        ...
                        vertexConsumer.vertex(matrix4f, 8 - 16 * direction.getOffsetX(), 8 - 16 * direction.getOffsetY(), 8 - 16 * direction.getOffsetZ())
                           .color(j, k, l, 255)
                           .normal(direction.getOffsetX(), direction.getOffsetY(), direction.getOffsetZ());
                     }
                  }
               ...
               if (this.client.debugChunkOcclusion && builtChunk.getData().hasNonEmptyLayers()) {
                  ...
                  for (Direction direction2 : DIRECTIONS) {
                     for (Direction direction3 : DIRECTIONS) {
                        ...
                        if (!bl) {
                           i++;
                           vertexConsumer.vertex(matrix4f, 8 + 8 * direction2.getOffsetX(), 8 + 8 * direction2.getOffsetY(), 8 + 8 * direction2.getOffsetZ())
                              .color(255, 0, 0, 255)
                              .normal(direction2.getOffsetX(), direction2.getOffsetY(), direction2.getOffsetZ());
                           vertexConsumer.vertex(matrix4f, 8 + 8 * direction3.getOffsetX(), 8 + 8 * direction3.getOffsetY(), 8 + 8 * direction3.getOffsetZ())
                              .color(255, 0, 0, 255)
   ...
   private void addFace(VertexConsumer vertexConsumer, Matrix4f positionMatrix, Vector4f[] vertices, int i1, int i2, int i3, int i4, int r, int g, int b) {
      float f = 0.25F;
      vertexConsumer.vertex(positionMatrix, vertices[i1].x(), vertices[i1].y(), vertices[i1].z()).color(r, g, b, 0.25F);
      vertexConsumer.vertex(positionMatrix, vertices[i2].x(), vertices[i2].y(), vertices[i2].z()).color(r, g, b, 0.25F);
      vertexConsumer.vertex(positionMatrix, vertices[i3].x(), vertices[i3].y(), vertices[i3].z()).color(r, g, b, 0.25F);
      vertexConsumer.vertex(positionMatrix, vertices[i4].x(), vertices[i4].y(), vertices[i4].z()).color(r, g, b, 0.25F);
   }
}
