...
import net.minecraft.network.packet.s2c.custom.DebugHiveCustomPayload.HiveInfo;
...
public class BeeDebugRenderer implements DebugRenderer.Renderer {
   ...
   private final Map<UUID, Bee> bees = new HashMap<>();
   @Nullable
   ...
   public void addHive(HiveInfo hive, long time) {
   ...
   private void removeInvalidBees() {
      this.bees.entrySet().removeIf(bee -> this.client.world.getEntityById(bee.getValue().entityId()) == null);
   }

   private void removeOutdatedHives() {
      long l = this.client.world.getTime() - 20L;
      this.hives.entrySet().removeIf(hive -> hive.getValue().lastSeen() < l);
   }
   ...
   private void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
      ...
      this.hives.values().forEach(hive -> {
         if (blockPos.isWithinDistance(hive.info.pos(), 30.0)) {
            Set<UUID> set = map.get(hive.info.pos());
      ...
      this.getBeesByHive().forEach((hive, bees) -> {
         if (blockPos.isWithinDistance(hive, 30.0)) {
            this.drawHiveBees(matrices, vertexConsumers, hive, (List<String>)bees);
         }
      });
   }
   ...
   private void drawFlowers(MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
      ...
      map.forEach((flowerPos, bees) -> {
         Set<String> set = bees.stream().<String>map(NameGenerator::name).collect(Collectors.toSet());
         int i = 1;
   ...
   private void drawHiveInfo(MatrixStack matrices, VertexConsumerProvider vertexConsumers, HiveInfo hive, Collection<UUID> blacklistingBees) {
      int i = 0;
   ...
   private static void drawString(MatrixStack matrices, VertexConsumerProvider vertexConsumers, String string, HiveInfo hive, int line, int color) {
   ...
   private Set<String> getBeeNamesForHive(HiveInfo hive) {
      return this.getBeesForHive(hive.pos()).stream().<String>map(NameGenerator::name).collect(Collectors.toSet());
   }
   ...
   private Collection<UUID> getBeesForHive(BlockPos hivePos) {
      return this.bees.values().stream().filter(bee -> bee.isHiveAt(hivePos)).<UUID>map(Bee::uuid).collect(Collectors.toSet());
   }
   ...
   record Hive(HiveInfo info, long lastSeen) {
   }
}
