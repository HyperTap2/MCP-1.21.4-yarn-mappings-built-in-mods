...
public class BackgroundRenderer {
   ...
   private static final List<BackgroundRenderer.StatusEffectFogModifier> FOG_MODIFIERS = Lists.newArrayList(
      new BackgroundRenderer.StatusEffectFogModifier[]{new BackgroundRenderer.BlindnessFogModifier(), new BackgroundRenderer.DarknessFogModifier()}
   );
   ...
   public static Vector4f getFogColor(Camera camera, float tickDelta, ClientWorld world, int clampedViewDistance, float skyDarkness) {
      ...
      if (cameraSubmersionType == CameraSubmersionType.WATER) {
         ...
         Vec3d vec3d2 = CubicSampler.sampleColor(
            vec3d,
            (i, j, k) -> world.getDimensionEffects()
               .adjustFogColor(Vec3d.unpackRgb(((Biome)biomeAccess.getBiomeForNoiseGen(i, j, k).value()).getFogColor()), z)
         );
         r = (float)vec3d2.getX();
         s = (float)vec3d2.getY();
         t = (float)vec3d2.getZ();
         ...
         s += (x - s) * u;
         t += (y - t) * u;
         float f = world.getRainGradient(tickDelta);
         if (f > 0.0F) {
            float g = 1.0F - f * 0.5F;
            float h = 1.0F - f * 0.4F;
            r *= g;
      ...
      float u = ((float)camera.getPos().y - world.getBottomY()) * world.getLevelProperties().getHorizonShadingRatio();
      ...
      if (statusEffectFogModifier != null) {
         ...
         u = statusEffectFogModifier.applyColorModifier(livingEntity, livingEntity.getStatusEffect(statusEffectFogModifier.getStatusEffect()), u, tickDelta);
      }

      if (u < 1.0F && cameraSubmersionType != CameraSubmersionType.LAVA && cameraSubmersionType != CameraSubmersionType.POWDER_SNOW) {
         if (u < 0.0F) {
            u = 0.0F;
         }
      ...
      if (cameraSubmersionType == CameraSubmersionType.WATER) {
         if (entity instanceof ClientPlayerEntity) {
            w = ((ClientPlayerEntity)entity).getUnderwaterVisibility();
         } else {
            w = 1.0F;
         }
         ...
         w = GameRenderer.getNightVisionStrength(livingEntity2, tickDelta);
      } else {
         w = 0.0F;
      }

      if (r != 0.0F && s != 0.0F && t != 0.0F) {
         float x = Math.min(1.0F / r, Math.min(1.0F / s, 1.0F / t));
         r = r * (1.0F - w) + r * x * w;
         s = s * (1.0F - w) + s * x * w;
         t = t * (1.0F - w) + t * x * w;
      }
   ...
   private static BackgroundRenderer.StatusEffectFogModifier getFogModifier(Entity entity, float tickDelta) {
      return entity instanceof LivingEntity livingEntity
         ? FOG_MODIFIERS.stream().filter(modifier -> modifier.shouldApply(livingEntity, tickDelta)).findFirst().orElse(null)
         : null;
   }
   ...
   public static Fog applyFog(Camera camera, BackgroundRenderer.FogType fogType, Vector4f color, float viewDistance, boolean thickenFog, float tickDelta) {
      CapturedRenderingState.INSTANCE.setFogColor(color.x, color.y, color.z);
      if (camera.getSubmersionType() == CameraSubmersionType.WATER) {
         ...
         if (focusedEntity instanceof ClientPlayerEntity player) {
            density -= player.getUnderwaterVisibility() * player.getUnderwaterVisibility() * 0.03F;
      ...
      int fogStartPercent = SodiumExtraClientMod.isMixinEnabled("fog_falloff.MixinFogRenderer")
         ? SodiumExtraClientMod.options().renderSettings.fogStart
         : 100;
      fogData.fogStart *= fogStartPercent / 100.0F;
      if (SodiumExtraClientMod.isMixinEnabled("fog.MixinFogRenderer")) {
         ...
         int fogDistance = SodiumExtraClientMod.options().renderSettings.multiDimensionFogControl
            ? SodiumExtraClientMod.options().renderSettings.dimensionFogDistanceMap.get(entity.getWorld().getDimension().effects())
         ...
         if (fogDistance != 0
            ...
            && (thickenFog || fogType == BackgroundRenderer.FogType.FOG_TERRAIN)) {
            if (fogDistance == 33) {
               return Fog.DUMMY;
            }

            fogData.fogStart = fogDistance * 16 * (fogStartPercent / 100.0F);
            fogData.fogEnd = (fogDistance + 1) * 16;
         }
      }
