...
public class SpriteAtlasManager implements AutoCloseable {
   ...
   public SpriteAtlasManager(Map<Identifier, Identifier> loaders, TextureManager textureManager) {
      this.atlases = loaders.entrySet().stream().collect(Collectors.toMap(Entry::getKey, entry -> {
         ...
         return new SpriteAtlasManager.Atlas(spriteAtlasTexture, entry.getValue());
      }));
   }
      ...
      return this.atlases
         ...
         .collect(
            Collectors.toMap(
               Entry::getKey,
               entry -> {
                  SpriteAtlasManager.Atlas atlas = entry.getValue();
