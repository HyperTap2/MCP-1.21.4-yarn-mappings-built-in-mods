...
public class BakedQuadFactory {
   ...
   public static BakedQuad bake(
      ...
      if (settings.isUvLocked()) {
         modelElementTexture = uvLock(face.textureData(), direction, settings.getRotation());
      }

      float[] fs = new float[modelElementTexture.uvs.length];
      System.arraycopy(modelElementTexture.uvs, 0, fs, 0, fs.length);
      float f = sprite.getAnimationFrameDelta();
      ...
      int[] is = packVertexData(modelElementTexture, sprite, direction, getPositionMatrix(from, to), settings.getRotation(), rotation);
      ...
      return new BakedQuad(is, face.tintIndex(), direction2, sprite, shade, lightEmission);
   }
   ...
   private static int[] packVertexData(
      ...
      for (int i = 0; i < 4; i++) {
         packVertexData(is, i, direction, texture, positionMatrix, sprite, orientation, rotation);
      }

      return is;
   }

   private static float[] getPositionMatrix(Vector3f from, Vector3f to) {
   ...
   private static void packVertexData(
      ...
      rotateVertex(vector3f, rotation);
      transformVertex(vector3f, orientation);
      packVertexData(vertices, cornerIndex, vector3f, sprite, texture);
   }

   private static void packVertexData(int[] vertices, int cornerIndex, Vector3f pos, Sprite sprite, ModelElementTexture texture) {
      int i = cornerIndex * 8;
      ...
      vertices[i + 3] = -1;
      vertices[i + 4] = Float.floatToRawIntBits(sprite.getFrameU(texture.getU(cornerIndex) / 16.0F));
      vertices[i + 4 + 1] = Float.floatToRawIntBits(sprite.getFrameV(texture.getV(cornerIndex) / 16.0F));
   }
   ...
   private static void transformVertex(Vector3f vertex, Vector3f origin, Matrix4f transformationMatrix, Vector3f scale) {
   ...
   private static Direction decodeDirection(int[] rotationMatrix) {
      Vector3f vector3f = new Vector3f(
         Float.intBitsToFloat(rotationMatrix[0]), Float.intBitsToFloat(rotationMatrix[1]), Float.intBitsToFloat(rotationMatrix[2])
      );
      Vector3f vector3f2 = new Vector3f(
         Float.intBitsToFloat(rotationMatrix[8]), Float.intBitsToFloat(rotationMatrix[9]), Float.intBitsToFloat(rotationMatrix[10])
      );
