...
public class ModelVariantMap {
   ...
   public static ModelVariantMap fromJson(JsonElement json) {
      return (ModelVariantMap)GSON.fromJson(json, ModelVariantMap.class);
   }
      ...
      this.variantMap.forEach((key, model) -> {
         try {
            list.stream().filter(BlockPropertiesPredicate.parse(stateManager, key)).forEach(state -> {
               GroupableModel groupableModel = map.put(state, model);
               if (groupableModel != null && groupableModel != multipartUnbakedModel) {
                  String stringx = this.variantMap.entrySet().stream().filter(entry -> entry.getValue() == groupableModel).findFirst().get().getKey();
                  throw new RuntimeException("Overlapping definition with: " + stringx);
               }
            });
         } catch (Exception exception) {
            LOGGER.warn("Exception loading blockstate definition: '{}' for variant: '{}': {}", new Object[]{path, key, exception.getMessage()});
         }
      });
      return map;
   }
   ...
   public static class Deserializer implements JsonDeserializer<ModelVariantMap> {
      ...
      protected MultipartUnbakedModel.Serialized multipartFromJson(JsonDeserializationContext context, JsonObject object) {
         ...
         return (MultipartUnbakedModel.Serialized)context.deserialize(jsonArray, MultipartUnbakedModel.Serialized.class);
      }
   }
