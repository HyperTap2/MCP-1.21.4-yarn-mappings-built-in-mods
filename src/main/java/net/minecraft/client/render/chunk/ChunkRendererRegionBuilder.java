...
public class ChunkRendererRegionBuilder {
   ...
   private ChunkRendererRegionBuilder.ClientChunk computeClientChunk(World world, int chunkX, int chunkZ) {
      return (ChunkRendererRegionBuilder.ClientChunk)this.chunks
         .computeIfAbsent(
            ChunkPos.toLong(chunkX, chunkZ),
            chunkPos -> new ChunkRendererRegionBuilder.ClientChunk(world.getChunk(ChunkPos.getPackedX(chunkPos), ChunkPos.getPackedZ(chunkPos)))
         );
   }
