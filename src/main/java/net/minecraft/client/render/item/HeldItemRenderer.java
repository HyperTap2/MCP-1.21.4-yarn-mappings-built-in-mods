...
public class HeldItemRenderer {
   ...
   private void applyViaFabricPlusSwingOffset(MatrixStack matrices, Arm arm, float swingProgress) {
   ...
   private void renderFirstPersonItem(
      ...
      if (Iris.isPackInUseQuick()
         && (HandRenderer.INSTANCE.isRenderingSolid() == HandRenderer.INSTANCE.isHandTranslucent(hand))) {
         return;
      }
      ...
      if (!player.isUsingSpyglass()) {
         ...
         if (item.isEmpty()) {
            ...
            if (player.isUsingItem() && player.getItemUseTimeLeft() > 0 && player.getActiveHand() == hand) {
               switch (item.getUseAction()) {
                  ...
                  case DRINK:
                     ...
                     this.applyEquipOffset(matrices, arm, equipProgress);
                     this.applyViaFabricPlusSwingOffset(matrices, arm, swingProgress);
                     break;
                  case BLOCK:
                     this.applyEquipOffset(matrices, arm, equipProgress);
                     this.applyViaFabricPlusSwingOffset(matrices, arm, swingProgress);
                  ...
                  case SPEAR:
                     ...
                     matrices.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(l * 35.3F));
                     matrices.multiply(RotationAxis.POSITIVE_Z.rotationDegrees(l * -9.785F));
                     float v = item.getMaxUseTime(player) - (player.getItemUseTimeLeft() - tickDelta + 1.0F);
                     float v1 = v / 10.0F;
                     if (v1 > 1.0F) {
                        v1 = 1.0F;
                     }
                     ...
                     if (v1 > 0.1F) {
                        float g = MathHelper.sin((v - 0.1F) * 1.3F);
                        float h = v1 - 0.1F;
                        float j = g * h;
                        matrices.translate(j * 0.0F, j * 0.004F, j * 0.0F);
                     }

                     matrices.translate(0.0F, 0.0F, v1 * 0.2F);
                     matrices.scale(1.0F, 1.0F, 1.0F + v1 * 0.2F);
                     matrices.multiply(RotationAxis.NEGATIVE_Y.rotationDegrees(l * 45.0F));
                     break;
                  case BRUSH:
            ...
            if (VisualSettings.INSTANCE.tiltItemPositions.isEnabled() && !(item.getItem() instanceof BlockItem)) {
               matrices.scale(0.8819767F, 0.8819767F, 0.8819767F);
