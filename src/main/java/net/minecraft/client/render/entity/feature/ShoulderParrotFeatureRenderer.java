...
public class ShoulderParrotFeatureRenderer extends FeatureRenderer<PlayerEntityRenderState, PlayerEntityModel> {
   ...
   public void render(
      ...
      Variant variant = playerEntityRenderState.leftShoulderParrotVariant;
      if (variant != null) {
         this.render(matrixStack, vertexConsumerProvider, i, playerEntityRenderState, variant, f, g, true);
      }

      Variant variant2 = playerEntityRenderState.rightShoulderParrotVariant;
      if (variant2 != null) {
   ...
   private void render(
      ...
      this.model
         .render(matrices, vertexConsumers.getBuffer(this.model.getLayer(ParrotEntityRenderer.getTexture(parrotVariant))), light, OverlayTexture.DEFAULT_UV);
      matrices.pop();
   }
}
