...
public abstract class EntityRenderer<T extends Entity, S extends EntityRenderState> {
   private static final NamespacedId IRIS_NAME_TAG_ID = new NamespacedId("minecraft", "name_tag");
   ...
   public Box sodium$getCullingBox(T entity) {
      return this.getBoundingBox(entity);
   }

   public boolean entityCulling$canBeCulled(T entity) {
      return this.canBeCulled(entity);
   }

   public void entityCulling$renderLabelIfPresent(
      T entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light
   ) {
      S state = this.getAndUpdateRenderState(entity, tickDelta);
   ...
   private static void renderLeashSegment(
      ...
      int k = (int)MathHelper.lerp(j, leashedEntityBlockLight, leashHolderBlockLight);
      int l = (int)MathHelper.lerp(j, leashedEntitySkyLight, leashHolderSkyLight);
   ...
   protected void renderLabelIfPresent(S state, Text text, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
      ...
      if (vec3d != null) {
         ...
         try {
            ...
            if (entityIds != null) {
               CapturedRenderingState.INSTANCE.setCurrentEntity(entityIds.applyAsInt(IRIS_NAME_TAG_ID));
            }
            ...
            try {
               ...
               textRenderer.draw(
                  ...
                  matrix4f,
                  vertexConsumers,
                  bl ? TextRenderer.TextLayerType.SEE_THROUGH : TextRenderer.TextLayerType.NORMAL,
                  j,
                  light
               );
               if (bl) {
   ...
   public void updateRenderState(T entity, S state, float tickDelta) {
      ...
      if (entity.hasVehicle()
         ...
         double d = MathHelper.lerp(tickDelta, abstractMinecartEntity.lastRenderX, abstractMinecartEntity.getX());
         double e = MathHelper.lerp(tickDelta, abstractMinecartEntity.lastRenderY, abstractMinecartEntity.getY());
         double f = MathHelper.lerp(tickDelta, abstractMinecartEntity.lastRenderZ, abstractMinecartEntity.getZ());
