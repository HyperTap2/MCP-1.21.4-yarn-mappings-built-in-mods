...
public abstract class LivingEntityRenderer<T extends LivingEntity, S extends LivingEntityRenderState, M extends EntityModel<? super S>>
   ...
   public void render(S livingEntityRenderState, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
      if (SodiumExtraClientMod.isMixinEnabled("render.entity.MixinLivingEntityRenderer")
         && livingEntityRenderState instanceof ArmorStandEntityRenderState
         && !SodiumExtraClientMod.options().renderSettings.armorStand) {
         if (livingEntityRenderState.displayName != null) {
            this.renderLabelIfPresent(livingEntityRenderState, livingEntityRenderState.displayName, matrixStack, vertexConsumerProvider, i);
         }
      ...
      if (this.shouldRenderFeatures(livingEntityRenderState)) {
         for (FeatureRenderer<S, M> featureRenderer : this.features) {
            featureRenderer.render(
               matrixStack, vertexConsumerProvider, i, livingEntityRenderState, livingEntityRenderState.yawDegrees, livingEntityRenderState.pitch
            );
         }
      }
   ...
   protected boolean hasLabel(T livingEntity, double d) {
      ...
      if (livingEntity != clientPlayerEntity) {
         ...
         if (abstractTeam != null) {
            VisibilityRule visibilityRule = abstractTeam.getNameTagVisibilityRule();
      ...
      if (itemStack.getItem() instanceof BlockItem blockItem && blockItem.getBlock() instanceof AbstractSkullBlock abstractSkullBlock) {
         ...
         if (!ArmorFeatureRenderer.hasModel(itemStack, EquipmentSlot.HEAD)) {
            this.itemModelResolver
               .updateForLivingEntity(livingEntityRenderState.headItemRenderState, itemStack, ModelTransformationMode.HEAD, false, livingEntity);
         } else {
            livingEntityRenderState.headItemRenderState.clear();
         }
      }
