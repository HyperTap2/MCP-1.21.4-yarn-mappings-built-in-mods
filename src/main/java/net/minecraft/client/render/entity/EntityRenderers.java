...
public class EntityRenderers {
   ...
   private static final Map<SkinTextures.Model, EntityRendererFactory<AbstractClientPlayerEntity>> PLAYER_RENDERER_FACTORIES = Map.of(
      SkinTextures.Model.WIDE, context -> new PlayerEntityRenderer(context, false), SkinTextures.Model.SLIM, context -> new PlayerEntityRenderer(context, true)
   );
   ...
   public static Map<EntityType<?>, EntityRenderer<?, ?>> reloadEntityRenderers(EntityRendererFactory.Context ctx) {
      ...
      RENDERER_FACTORIES.forEach((entityType, factory) -> {
         try {
            ...
            throw new IllegalArgumentException("Failed to create model for " + Registries.ENTITY_TYPE.getId(entityType), exception);
         }
      });
   ...
   static {
      ...
      register(EntityType.DONKEY, context -> new AbstractDonkeyEntityRenderer(context, EntityModelLayers.DONKEY, EntityModelLayers.DONKEY_BABY, false));
      ...
      register(
         ...
         context -> new GlowSquidEntityRenderer(
            context,
            new SquidEntityModel(context.getPart(EntityModelLayers.GLOW_SQUID)),
            new SquidEntityModel(context.getPart(EntityModelLayers.GLOW_SQUID_BABY))
         )
      );
      ...
      register(EntityType.MULE, context -> new AbstractDonkeyEntityRenderer(context, EntityModelLayers.MULE, EntityModelLayers.MULE_BABY, true));
      ...
      register(
         ...
         context -> new UndeadHorseEntityRenderer(context, EntityModelLayers.SKELETON_HORSE, EntityModelLayers.SKELETON_HORSE_BABY, true)
      );
