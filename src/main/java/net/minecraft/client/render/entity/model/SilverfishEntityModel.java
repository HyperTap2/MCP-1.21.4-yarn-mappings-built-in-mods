...
public class SilverfishEntityModel extends EntityModel<EntityRenderState> {
   ...
   public static TexturedModelData getTexturedModelData() {
      ...
      for (int i = 0; i < 7; i++) {
         modelPartData.addChild(
            ...
            ModelTransform.pivot(0.0F, 24 - SEGMENT_LOCATIONS[i][1], f)
         );
         fs[i] = f;
         if (i < 6) {
   ...
   public void setAngles(EntityRenderState state) {
      ...
      for (int i = 0; i < this.body.length; i++) {
         ...
         this.body[i].pivotX = MathHelper.sin(state.age * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.2F * Math.abs(i - 2);
      }
