...
public class PandaEntityRenderer extends AgeableMobEntityRenderer<PandaEntity, PandaEntityRenderState, PandaEntityModel> {
   private static final Map<Gene, Identifier> TEXTURES = Util.make(Maps.newEnumMap(Gene.class), map -> {
      map.put(Gene.NORMAL, Identifier.ofVanilla("textures/entity/panda/panda.png"));
      map.put(Gene.LAZY, Identifier.ofVanilla("textures/entity/panda/lazy_panda.png"));
      map.put(Gene.WORRIED, Identifier.ofVanilla("textures/entity/panda/worried_panda.png"));
      map.put(Gene.PLAYFUL, Identifier.ofVanilla("textures/entity/panda/playful_panda.png"));
      map.put(Gene.BROWN, Identifier.ofVanilla("textures/entity/panda/brown_panda.png"));
      map.put(Gene.WEAK, Identifier.ofVanilla("textures/entity/panda/weak_panda.png"));
      map.put(Gene.AGGRESSIVE, Identifier.ofVanilla("textures/entity/panda/aggressive_panda.png"));
   });
   ...
   public Identifier getTexture(PandaEntityRenderState pandaEntityRenderState) {
      return TEXTURES.getOrDefault(pandaEntityRenderState.gene, TEXTURES.get(Gene.NORMAL));
   }
   ...
   protected void setupTransforms(PandaEntityRenderState pandaEntityRenderState, MatrixStack matrixStack, float f, float g) {
      ...
      if (h > 0.0F) {
         ...
         if (pandaEntityRenderState.scaredByThunderstorm) {
            float q = (float)(Math.cos(pandaEntityRenderState.age * 1.25F) * Math.PI * 0.05F);
