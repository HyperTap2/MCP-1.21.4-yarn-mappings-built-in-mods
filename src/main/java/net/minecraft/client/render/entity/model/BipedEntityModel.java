...
public class BipedEntityModel<T extends BipedEntityRenderState> extends EntityModel<T> implements ModelWithArms, ModelWithHead {
   ...
   public void setAngles(T bipedEntityRenderState) {
      ...
      if (VisualSettings.INSTANCE.oldWalkingAnimation.isEnabled()) {
         this.rightArm.pitch = MathHelper.cos(g * 0.6662F + (float)Math.PI) * 2.0F * h;
         this.rightArm.roll = (MathHelper.cos(g * 0.2312F) + 1.0F) * h;
         this.leftArm.pitch = MathHelper.cos(g * 0.6662F) * 2.0F * h;
         this.leftArm.roll = (MathHelper.cos(g * 0.2812F) - 1.0F) * h;
      }
         ...
         if (!bipedEntityRenderState.isUsingItem) {
            if (i < 14.0F) {
               ...
               this.leftArm.roll = MathHelper.lerpAngleRadians(
                  k, this.leftArm.roll, (float) Math.PI + 1.8707964F * this.method_2807(i) / this.method_2807(14.0F)
               );
   ...
   private void positionBlockingArm(ModelPart arm, boolean rightArm) {
      boolean lockRotation = VisualSettings.INSTANCE.lockBlockingArmRotation.isEnabled();
      float headPitch = lockRotation ? 0.0F : MathHelper.clamp(this.head.pitch, (float)(-Math.PI * 4.0 / 9.0), 0.43633232F);
      float headYaw = lockRotation ? 0.0F : MathHelper.clamp(this.head.yaw, (float)(-Math.PI / 6), (float)(Math.PI / 6));
      arm.pitch = arm.pitch * 0.5F - 0.9424779F + headPitch;
      arm.yaw = (rightArm ? -30.0F : 30.0F) * (float)(Math.PI / 180.0) + headYaw;
   }
   ...
   public enum ArmPose {
      ...
      ArmPose(final boolean twoHanded) {
