...
public class BlockModelRenderer {
   ...
   private final ThreadLocal<NonTerrainBlockRenderContext> continuityContexts;
   ...
   public BlockModelRenderer(BlockColors colors) {
      this.colors = colors;
      this.continuityContexts = ThreadLocal.withInitial(() -> new NonTerrainBlockRenderContext(colors));
   }
   ...
   public void render(
      ...
      if (!model.isVanillaAdapter()) {
         this.continuityContexts.get().renderModel(world, model, state, pos, matrices, vertexConsumer, cull, random, seed, overlay);
         return;
      }
   ...
   protected enum NeighborOrientation {
      ...
      NeighborOrientation(final Direction direction, final boolean flip) {
   ...
   enum Translation {
      ...
      private static final BlockModelRenderer.Translation[] VALUES = (BlockModelRenderer.Translation[])Util.make(
         new BlockModelRenderer.Translation[6], values -> {
            values[Direction.DOWN.getId()] = DOWN;
            values[Direction.UP.getId()] = UP;
            values[Direction.NORTH.getId()] = NORTH;
            values[Direction.SOUTH.getId()] = SOUTH;
      ...
      Translation(final int firstCorner, final int secondCorner, final int thirdCorner, final int fourthCorner) {
