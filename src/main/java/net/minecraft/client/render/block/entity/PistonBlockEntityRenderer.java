...
public class PistonBlockEntityRenderer implements BlockEntityRenderer<PistonBlockEntity> {
   ...
   public void render(PistonBlockEntity pistonBlockEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j) {
      if (SodiumExtraClientMod.isMixinEnabled("render.block.entity.MixinPistonBlockEntityRenderer")
      ...
      if (world != null) {
         ...
         if (!blockState.isAir()) {
            ContinuityFeatureStates continuityStates = ContinuityFeatureStates.get();
            continuityStates.getConnectedTexturesState().disable();
            continuityStates.getEmissiveTexturesState().disable();
            ...
            if (blockState.isOf(Blocks.PISTON_HEAD) && pistonBlockEntity.getProgress(f) <= 4.0F) {
               ...
               BlockState blockState2 = (BlockState)((BlockState)Blocks.PISTON_HEAD.getDefaultState().with(PistonHeadBlock.TYPE, pistonType))
            ...
            continuityStates.getConnectedTexturesState().enable();
            continuityStates.getEmissiveTexturesState().enable();
         }
      }
   }

   private void renderModel(
      BlockPos pos, BlockState state, MatrixStack matrices, VertexConsumerProvider vertexConsumers, World world, boolean cull, int overlay
   ) {
