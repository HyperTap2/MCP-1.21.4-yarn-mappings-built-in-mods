...
public class TexturedRenderLayers {
   ...
   public static final SpriteIdentifier SHULKER_TEXTURE_ID = createShulkerBoxTextureId(Identifier.ofVanilla("shulker"));
   public static final List<SpriteIdentifier> COLORED_SHULKER_BOXES_TEXTURES = Arrays.stream(DyeColor.values())
      .sorted(Comparator.comparingInt(DyeColor::getId))
      .map(TexturedRenderLayers::createShulkerBoxTextureId)
      .collect(ImmutableList.toImmutableList());
   public static final Map<WoodType, SpriteIdentifier> SIGN_TYPE_TEXTURES = WoodType.stream()
      .collect(Collectors.toMap(Function.identity(), TexturedRenderLayers::createSignTextureId));
   public static final Map<WoodType, SpriteIdentifier> HANGING_SIGN_TYPE_TEXTURES = WoodType.stream()
   ...
   public static final Map<RegistryKey<DecoratedPotPattern>, SpriteIdentifier> DECORATED_POT_PATTERN_TEXTURES = Registries.DECORATED_POT_PATTERN
      .streamEntries()
      .collect(Collectors.toMap(Reference::registryKey, pattern -> createDecoratedPotPatternTextureId(((DecoratedPotPattern)pattern.value()).assetId())));
   ...
   private static final SpriteIdentifier[] BED_TEXTURES = Arrays.stream(DyeColor.values())
      .sorted(Comparator.comparingInt(DyeColor::getId))
   ...
   static {
      ((BlendingStateHolder)ARMOR_TRIMS_RENDER_LAYER).setTransparencyType(TransparencyType.OPAQUE_DECAL);
      ((BlendingStateHolder)RenderLayer.getTextBackground()).setTransparencyType(TransparencyType.OPAQUE);
      ((BlendingStateHolder)RenderLayer.getTextBackgroundSeeThrough()).setTransparencyType(TransparencyType.OPAQUE);
      ((BlendingStateHolder)RenderLayer.getDragonRaysDepth()).setTransparencyType(TransparencyType.DECAL);
   }
   ...
   public static SpriteIdentifier createBedTextureId(DyeColor color) {
      return createBedTextureId(createColorId(color));
   }
   ...
   public static SpriteIdentifier createShulkerBoxTextureId(DyeColor color) {
      return createShulkerBoxTextureId(createShulkerId(color));
   }
   ...
   private static SpriteIdentifier createSignTextureId(WoodType type) {
      return createSignTextureId(Identifier.ofVanilla(type.name()));
   }
   ...
   private static SpriteIdentifier createHangingSignTextureId(WoodType type) {
      return createHangingSignTextureId(Identifier.ofVanilla(type.name()));
   }
   ...
   public static SpriteIdentifier getBannerPatternTextureId(RegistryEntry<BannerPattern> pattern) {
      return BANNER_PATTERN_TEXTURES.computeIfAbsent(((BannerPattern)pattern.value()).assetId(), id -> {
   ...
   public static SpriteIdentifier getShieldPatternTextureId(RegistryEntry<BannerPattern> pattern) {
      return SHIELD_PATTERN_TEXTURES.computeIfAbsent(((BannerPattern)pattern.value()).assetId(), id -> {
   ...
   public static SpriteIdentifier getDecoratedPotPatternTextureId(@Nullable RegistryKey<DecoratedPotPattern> potPatternKey) {
      return potPatternKey == null ? null : DECORATED_POT_PATTERN_TEXTURES.get(potPatternKey);
   }
