...
public class ChunkRenderingDataPreparer {
   ...
   public void addNeighbors(ChunkPos chunkPos) {
      ChunkRenderingDataPreparer.Events events = this.events.get();
      if (events != null) {
   ...
   private void updateTerrain(boolean cullChunks, Camera camera, Vec3d cameraPos, LongOpenHashSet activeSections) {
      ...
      this.terrainUpdateFuture = CompletableFuture.runAsync(() -> {
         ...
         Queue<ChunkRenderingDataPreparer.ChunkInfo> queue = Queues.newArrayDeque();
         this.method_52821(camera, queue);
         queue.forEach(info -> preparerState.storage.infoList.setInfo(info.chunk, info));
         this.method_52825(preparerState.storage, cameraPos, queue, cullChunks, builtChunk -> {}, longOpenHashSet);
   ...
   private void method_52823(ChunkRenderingDataPreparer.PreparerState preparerState) {
      ...
      while (longIterator.hasNext()) {
         ...
         List<ChunkBuilder.BuiltChunk> list = (List<ChunkBuilder.BuiltChunk>)preparerState.storage.field_45628.get(l);
            ...
            if (builtChunk2 != null && (!cullChunks || !chunkInfo.canCull(direction.getOpposite()))) {
               ...
               if (cullChunks && bl) {
                  ...
                  BlockPos blockPos4 = blockPos3.add(
                     (direction.getAxis() == Axis.X ? blockPos2.getX() <= blockPos3.getX() : blockPos2.getX() >= blockPos3.getX()) ? 0 : 16,
                     (direction.getAxis() == Axis.Y ? blockPos2.getY() <= blockPos3.getY() : blockPos2.getY() >= blockPos3.getY()) ? 0 : 16,
                     (direction.getAxis() == Axis.Z ? blockPos2.getZ() <= blockPos3.getZ() : blockPos2.getZ() >= blockPos3.getZ()) ? 0 : 16
                  );
                  ...
                  while (pos.subtract(vec3d).lengthSquared() > 3600.0) {
                     ...
                     HeightLimitView heightLimitView = this.builtChunkStorage.getWorld();
                     if (vec3d.y > heightLimitView.getTopYInclusive() || vec3d.y < heightLimitView.getBottomY()) {
                        break;
                     }
               ...
               if (chunkInfo2 != null) {
                  ...
                  ChunkRenderingDataPreparer.ChunkInfo chunkInfo3 = new ChunkRenderingDataPreparer.ChunkInfo(
                     builtChunk2, direction, chunkInfo.propagationLevel + 1
                  );
                  ...
                  if (builtChunk2.shouldBuild()) {
                     ...
                     renderableChunks.infoList.setInfo(builtChunk2, chunkInfo3);
                     ((List)renderableChunks.field_45628.computeIfAbsent(ChunkPos.toLong(builtChunk2.getOrigin()), lx -> new ArrayList())).add(builtChunk2);
                  }
               }
            }
         }
