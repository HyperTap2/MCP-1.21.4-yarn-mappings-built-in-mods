...
public class RenderLayers {
   private static final RenderLayer[] IRIS_BLOCK_LAYERS = Util.make(new RenderLayer[BlockRenderType.values().length], layers -> {
      for (int i = 0; i < layers.length; i++) {
         layers[i] = BlockMaterialMapping.convertBlockToRenderType(BlockRenderType.values()[i]);
      }
   });
   ...
   private static final Map<Fluid, RenderLayer> FLUIDS = Util.make(Maps.newHashMap(), map -> {
   ...
   public static RenderLayer getBlockLayer(BlockState state) {
      if (!CustomBlockLayers.isEmpty() && ContinuityConfig.INSTANCE.customBlockLayers.get()) {
         RenderLayer continuityLayer = CustomBlockLayers.getLayer(state);
         if (continuityLayer != null) {
            return continuityLayer;
         }
      }
      BlockRenderType irisType = WorldRenderingSettings.INSTANCE.getBlockTypeIds().get(state.getBlock());
      if (irisType != null) {
         return IRIS_BLOCK_LAYERS[irisType.ordinal()];
      }
   ...
   public static RenderLayer getMovingBlockLayer(BlockState state) {
      if (!CustomBlockLayers.isEmpty() && ContinuityConfig.INSTANCE.customBlockLayers.get()) {
         RenderLayer continuityLayer = CustomBlockLayers.getLayer(state);
         if (continuityLayer != null) {
            return continuityLayer == RenderLayer.getTranslucent() ? RenderLayer.getTranslucentMovingBlock() : continuityLayer;
         }
      }
      Block block = state.getBlock();
      if (block instanceof LeavesBlock) {
