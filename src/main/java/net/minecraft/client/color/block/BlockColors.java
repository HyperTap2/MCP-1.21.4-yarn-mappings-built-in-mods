...
public class BlockColors implements BlockColorsExtension {
   ...
   private final Reference2ReferenceMap<Block, BlockColorProvider> sodium$providers = new Reference2ReferenceOpenHashMap<>();
   private final ReferenceSet<Block> sodium$overriddenBlocks = new ReferenceOpenHashSet<>();
   ...
   public int getParticleColor(BlockState state, World world, BlockPos pos) {
      BlockColorProvider blockColorProvider = (BlockColorProvider)this.providers.get(Registries.BLOCK.getRawId(state.getBlock()));
   ...
   public void registerColorProvider(BlockColorProvider provider, Block... blocks) {
      for (Block block : blocks) {
         if (this.sodium$providers.put(block, provider) != null) {
            this.sodium$overriddenBlocks.add(block);
            SodiumClientMod.logger()
               .info("Block {} had its color provider replaced with {} and will not use per-vertex coloring", Registries.BLOCK.getId(block), provider);
         }
   ...
   public Reference2ReferenceMap<Block, BlockColorProvider> sodium$getProviders() {
      return Reference2ReferenceMaps.unmodifiable(this.sodium$providers);
   }

   @Override
   public ReferenceSet<Block> sodium$getOverridenVanillaBlocks() {
      return ReferenceSets.unmodifiable(this.sodium$overriddenBlocks);
   }
