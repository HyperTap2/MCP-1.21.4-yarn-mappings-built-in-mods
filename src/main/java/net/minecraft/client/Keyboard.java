...
public class Keyboard {
   ...
   private void addDebugMessage(Formatting formatting, Text text) {
      this.client
         ...
         .addMessage(
            Text.empty()
               .append(Text.translatable("debug.prefix").formatted(new Formatting[]{formatting, Formatting.BOLD}))
      ...
      switch (key) {
         ...
         case 74:
            if (Screen.hasShiftDown()) {
               ...
               this.debugLog("debug.reload_languages.switch.success");
            } else {
         ...
         case 78:
            if (!this.client.player.hasPermissionLevel(2)) {
               ...
               this.client
                  ...
                  .networkHandler
                  .sendCommand(
                     "gamemode " + ((GameMode)MoreObjects.firstNonNull(this.client.interactionManager.getPreviousGameMode(), GameMode.CREATIVE)).getName()
                  );
            }
         ...
         case 81:
            ...
            chatHud.addMessage(Text.translatable("debug.reload_languages.help"));
         ...
         case 83:
            ...
            Text text = Text.literal(path.relativize(path2).toString())
               ...
               .styled(style -> style.withClickEvent(new ClickEvent(Action.OPEN_FILE, path2.toFile().toString())));
   ...
   public void onKey(long window, int key, int scancode, int action, int modifiers) {
      if (window == this.client.getWindow().getHandle()) {
         ...
         if (screen != null && action != 0 && bl && key == 74) {
            if (Screen.hasShiftDown()) {
               Config config = Config.getInstance();
               LanguageReload.setLanguage(config.previousLanguage, config.previousFallbacks);
            } else {
         ...
         if (action != 0) {
            ...
            if (bl2) {
               if (Screen.hasControlDown() && key == 66 && this.client.getNarratorManager().isActive() && this.client.options.getNarratorHotkey().getValue()) {
   ...
   private void onChar(long window, int codePoint, int modifiers) {
      if (window == this.client.getWindow().getHandle()) {
         if (InputUtil.isKeyPressed(window, 292) && InputUtil.isKeyPressed(window, 74)) {
            return;
         }
         Screen screen = this.client.currentScreen;
   ...
   public void setup(long window) {
      InputUtil.setKeyboardCallbacks(
         window,
         (windowx, key, scancode, action, modifiers) -> this.executeScreenEvent(() -> this.onKey(windowx, key, scancode, action, modifiers)),
         (windowx, codePoint, modifiers) -> this.executeScreenEvent(() -> this.onChar(windowx, codePoint, modifiers))
      );
   }
