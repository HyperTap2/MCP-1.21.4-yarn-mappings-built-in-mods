...
public class SoundManager extends SinglePreparationResourceReloader<SoundManager.SoundList> {
   ...
   private static final Gson GSON = new GsonBuilder()
      .registerTypeHierarchyAdapter(Text.class, new Serializer(DynamicRegistryManager.EMPTY))
            ...
            try {
               for (Resource resource : resourceManager.getAllResources(Identifier.of(string, "sounds.json"))) {
                  ...
                  try (Reader reader = resource.getReader()) {
                     profiler.push("parse");
                     Map<String, SoundEntry> map = (Map<String, SoundEntry>)JsonHelper.deserialize(GSON, reader, TYPE);
                     profiler.swap("register");

                     for (Entry<String, SoundEntry> entry : map.entrySet()) {
                        soundList.register(Identifier.of(string, entry.getKey()), entry.getValue());
                     }

                     profiler.pop();
                  } catch (RuntimeException runtimeException) {
                     LOGGER.warn("Invalid {} in resourcepack: '{}'", new Object[]{"sounds.json", resource.getPackId(), runtimeException});
                  }
               ...
               case SOUND_EVENT:
                  soundContainer = new SoundContainer<Sound>() {
                     ...
                     public int getWeight() {
                        WeightedSoundSet weightedSoundSetx = SoundList.this.loadedSounds.get(identifier);
                     ...
                     public Sound getSound(Random random) {
                        ...
                        return new Sound(
                           ...
                           new MultipliedFloatSupplier(new FloatSupplier[]{soundx.getVolume(), sound.getVolume()}),
                           new MultipliedFloatSupplier(new FloatSupplier[]{soundx.getPitch(), sound.getPitch()}),
                           sound.getWeight(),
