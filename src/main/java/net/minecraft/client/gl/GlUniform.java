...
public class GlUniform extends Uniform implements AutoCloseable {
   ...
   public static int getUniformLocation(int program, CharSequence name) {
      ...
      if (location == -1 && name.equals("Sampler0")) {
         location = GlStateManager._glGetUniformLocation(program, "tex");
         if (location == -1) {
            location = GlStateManager._glGetUniformLocation(program, "gtexture");
            if (location == -1) {
               location = GlStateManager._glGetUniformLocation(program, "texture");
            }
         }
      }
   ...
   public void set(List<Float> values, int count) {
      ...
      if (values.size() == 1) {
         Arrays.fill(fs, values.getFirst().floatValue());
      } else {
