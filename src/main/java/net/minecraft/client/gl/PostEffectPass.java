...
public class PostEffectPass {
   ...
   public void render(FrameGraphBuilder builder, Map<Identifier, Handle<Framebuffer>> handles, Matrix4f projectionMatrix) {
      ...
      Handle<Framebuffer> handle = handles.computeIfPresent(this.outputTargetId, (id, handlex) -> renderPass.transfer((Handle<Framebuffer>)handlex));
      if (handle == null) {
      ...
      renderPass.setRenderer(() -> {
         ...
         bufferBuilder.vertex(framebuffer.textureWidth, 0.0F, 500.0F);
         bufferBuilder.vertex(framebuffer.textureWidth, framebuffer.textureHeight, 500.0F);
         bufferBuilder.vertex(0.0F, framebuffer.textureHeight, 500.0F);
