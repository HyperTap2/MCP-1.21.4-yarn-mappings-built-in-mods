...
import com.github.argon4w.acceleratedrendering.features.modelparts.AcceleratedModelGeometry;
...
public final class ModelPart {
   ...
   private final AcceleratedModelGeometry acceleratedRendering$geometry;
   ...
   public ModelPart(List<ModelPart.Cuboid> cuboids, Map<String, ModelPart> children) {
      ...
      this.acceleratedRendering$geometry = new AcceleratedModelGeometry(cuboids);
   }
   ...
   private void renderCuboids(MatrixStack.Entry entry, VertexConsumer vertexConsumer, int light, int overlay, int color) {
      ...
      if (AcceleratedRendering.isAvailable() && acceleratedContext && !this.acceleratedRendering$geometry.isEmpty()) {
         try {
            this.acceleratedRendering$geometry.render(entry, vertexConsumer, light, overlay, color);
            return;
         } catch (Throwable throwable) {
            AcceleratedRendering.disableAfterFailure("model geometry rendering failed", throwable);
         }
      }
