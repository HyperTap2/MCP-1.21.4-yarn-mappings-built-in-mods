...
public class TextSearchProvider<T> extends IdentifierSearchProvider<T> {
   ...
   protected List<T> search(String namespace, String path) {
      ...
      Iterator<T> iterator = new TextSearchableIterator<T>(list2.iterator(), list3.iterator(), this.lastIndexComparator);
      return ImmutableList.copyOf(new IdentifierSearchableIterator<T>(list.iterator(), iterator, this.lastIndexComparator));
   }
}
