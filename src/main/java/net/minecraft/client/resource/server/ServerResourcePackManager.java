...
public class ServerResourcePackManager {
   ...
   private void onDownload(Collection<ServerResourcePackManager.PackEntry> packs, DownloadResult result) {
   ...
   private boolean enqueueDownloads() {
      ...
      if (!list.isEmpty()) {
         Map<UUID, DownloadEntry> map = new HashMap<>();

         for (ServerResourcePackManager.PackEntry packEntry2 : list) {
            map.put(packEntry2.id, new DownloadEntry(packEntry2.url, packEntry2.hashCode));
         }
   ...
   private void applyDownloadedPacks() {
      ...
      if (bl) {
         ...
         for (ServerResourcePackManager.PackEntry packEntry : list2) {
            packEntry.status = ServerResourcePackManager.Status.PENDING;
         }
   ...
   enum DiscardReason {
      ...
      DiscardReason(@Nullable final PackStateChangeCallback.FinishState state) {
         this.state = state;
      }
   }
