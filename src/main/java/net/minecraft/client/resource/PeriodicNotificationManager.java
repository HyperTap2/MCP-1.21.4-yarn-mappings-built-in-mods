...
public class PeriodicNotificationManager
   ...
   protected Map<String, List<PeriodicNotificationManager.Entry>> prepare(ResourceManager resourceManager, Profiler profiler) {
      try (Reader reader = resourceManager.openAsReader(this.id)) {
         return (Map<String, List<PeriodicNotificationManager.Entry>>)CODEC.parse(JsonOps.INSTANCE, JsonParser.parseReader(reader)).result().orElseThrow();
      } catch (Exception exception) {
         LOGGER.warn("Failed to load {}", this.id, exception);
                  ...
                  this.client
                     .execute(
                        () -> SystemToast.add(
                           ...
                           Text.translatable(entry.title, new Object[]{n}),
                           Text.translatable(entry.message, new Object[]{n})
                        )
                     );
                  return;
               }
