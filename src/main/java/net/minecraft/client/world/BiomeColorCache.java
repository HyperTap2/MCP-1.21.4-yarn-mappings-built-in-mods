...
public class BiomeColorCache {
   ...
   public int getBiomeColor(BlockPos pos) {
      ...
      BiomeColorCache.Last last = this.last.get();
      ...
      try {
         ...
         for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
               ...
               BiomeColorCache.Colors colors = (BiomeColorCache.Colors)this.colors.remove(l);
               if (colors != null) {
   ...
   private BiomeColorCache.Colors getColorArray(int chunkX, int chunkZ) {
      ...
      try {
         BiomeColorCache.Colors colors = (BiomeColorCache.Colors)this.colors.get(l);
         if (colors != null) {
            return colors;
         }
      } finally {
      ...
      try {
         ...
         BiomeColorCache.Colors colors2 = new BiomeColorCache.Colors();
         if (this.colors.size() >= 256) {
            BiomeColorCache.Colors colors3 = (BiomeColorCache.Colors)this.colors.removeFirst();
            if (colors3 != null) {
   ...
   static class Colors {
      ...
      public int[] get(int y) {
         ...
         try {
            return (int[])this.colors.computeIfAbsent(y, yx -> this.createDefault());
         } finally {
            this.lock.writeLock().unlock();
         }
      }
