...
public class ClientChunkManager extends ChunkManager {
      ...
      if (this.chunks.isInRadius(pos.x, pos.z)) {
         ...
         if (positionEquals(worldChunk, pos.x, pos.z)) {
            ...
            ChunkTrackerHolder.get(this.world)
               .onChunkStatusRemoved(pos.x, pos.z, net.caffeinemc.mods.sodium.client.render.chunk.map.ChunkStatus.FLAG_HAS_BLOCK_DATA);
         }
      }
   }
   ...
   @Nullable
   public WorldChunk loadChunkFromPacket(int x, int z, PacketByteBuf buf, NbtCompound nbt, Consumer<BlockEntityVisitor> consumer) {
      if (!this.chunks.isInRadius(x, z)) {
         LOGGER.warn("Ignoring chunk since it's not in the view range: {}, {}", x, z);
         return null;
      }
   ...
   public List<WorldChunk> languageReload$getLoadedChunks() {
      List<WorldChunk> loaded = new ArrayList<>();
      AtomicReferenceArray<WorldChunk> chunks = this.chunks.chunks;
   ...
   final class ClientChunkMap {
      ...
      private void writePositions(String fileName) {
         try (FileOutputStream fileOutputStream = new FileOutputStream(fileName)) {
            ...
            ClientChunkManager.LOGGER.error("Failed to dump chunks to file {}", fileName, iOException);
         }
      }
   }
}
