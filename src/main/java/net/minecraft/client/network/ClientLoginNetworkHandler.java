...
public class ClientLoginNetworkHandler implements ClientLoginPacketListener {
   ...
   private Text joinServerSession(String serverId) {
      IClientConnection viaConnection = (IClientConnection)this.connection;
      if (viaConnection.viaFabricPlus$getTargetVersion() != null
         && viaConnection.viaFabricPlus$getTargetVersion().olderThanOrEqualTo(LegacyProtocolVersion.r1_6_4)
         && viaConnection.viaFabricPlus$getUserConnection() != null) {
         ProtocolMetadataStorage metadata = viaConnection.viaFabricPlus$getUserConnection().get(ProtocolMetadataStorage.class);
         if (metadata != null && !metadata.authenticate) {
            return null;
         }
      }
      ...
      try {
         ...
         return Text.translatable("disconnect.loginFailedInfo", new Object[]{authenticationException.getMessage()});
      }
   }
   ...
   public void onCompression(LoginCompressionS2CPacket packet) {
      if (!this.connection.isLocal()) {
         this.connection.setCompressionThreshold(
            ...
            ((IClientConnection)this.connection).viaFabricPlus$getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_17)
         );
      }
   }
