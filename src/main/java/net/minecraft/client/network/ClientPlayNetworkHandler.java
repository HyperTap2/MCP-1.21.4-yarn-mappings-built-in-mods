...
import net.minecraft.network.encryption.PlayerPublicKey.PublicKeyException;
...
public class ClientPlayNetworkHandler extends ClientCommonNetworkHandler implements ClientPlayPacketListener, TickablePacketListener {
   ...
   private final Set<PlayerListEntry> listedPlayerListEntries = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_19_1)
      ? new LinkedHashSet<>()
   ...
   private ClientRecipeManager recipeManager = new ClientRecipeManager(Map.of(), Grouping.empty());
   private final UUID sessionId = UUID.randomUUID();
   private Set<RegistryKey<World>> worldKeys;
   private final Immutable combinedDynamicRegistries;
   ...
   private Packer messagePacker = Packer.NONE;
   ...
   @Nullable
   public WorldLoadingState worldLoadingState;
   ...
   public ClientPlayNetworkHandler(MinecraftClient client, ClientConnection clientConnection, ClientConnectionState clientConnectionState) {
      ...
      BiomeHolderManager.setRegistryManager(this.combinedDynamicRegistries);
   ...
   public void onGameJoin(GameJoinS2CPacket packet) {
      ...
      if (this.serverInfo != null
         && !this.displayedUnsecureChatWarning
         && !(this.isSecureChatEnforced() || VisualSettings.INSTANCE.disableSecureChatWarning.isEnabled())) {
         SystemToast systemToast = SystemToast.create(
            this.client, SystemToast.Type.UNSECURE_SERVER_WARNING, UNSECURE_SERVER_TOAST_TITLE, UNSECURE_SERVER_TOAST_TEXT
         );
      ...
      if (this.client.player != null) {
         Iris.getUpdateChecker().getUpdateMessage().ifPresent(message -> this.client.player.sendMessage(message, false));
         Iris.getStoredError().ifPresent(error -> this.client.player.sendMessage(
            Text.translatable(error instanceof ShaderCompileException ? "iris.load.failure.shader" : "iris.load.failure.generic")
               .append(Text.literal("Copy Info").styled(style -> style
                  .withUnderline(true)
                  .withColor(Formatting.BLUE)
                  .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, error.getMessage()))
         ...
         if (Iris.loadedIncompatiblePack()) {
            this.client.inGameHud.setTitleTicks(10, 70, 140);
            Iris.logger.warn("Incompatible pack for DH!");
            this.client.player.sendMessage(Text.literal("This pack doesn't have DH support.").formatted(Formatting.BOLD, Formatting.RED), false);
            this.client.player.sendMessage(
               Text.literal("Distant Horizons (DH) chunks won't show up. This isn't a bug, get another shader.").formatted(Formatting.RED),
               false
            );
         }
      }
      ...
      ((IPlayerListHud)this.client.inGameHud.getPlayerListHud()).viaFabricPlusVisuals$setMaxPlayers(packet.maxPlayers());
   }
   ...
   public void onEntityPositionSync(EntityPositionSyncS2CPacket packet) {
      ...
      if (entity != null) {
         ...
         if (!viaFabricPlus$isLogicalSideForUpdatingMovement(entity)) {
   ...
   public void onEntityPosition(EntityPositionS2CPacket packet) {
      ...
      if (entity == null) {
         if (ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_21)
            && this.removedPlayerVehicleId.isPresent()
            && this.removedPlayerVehicleId.getAsInt() == packet.entityId()) {
   ...
   public void onEntitiesDestroy(EntitiesDestroyS2CPacket packet) {
      ...
      packet.getEntityIds().forEach(id -> {
         ...
         if (entity != null) {
            ...
            this.world.removeEntity(id, RemovalReason.DISCARDED);
         }
      });
   }
   ...
   public void onPlayerPositionLook(PlayerPositionLookS2CPacket packet) {
      ...
      TeleportConfirmC2SPacket teleportConfirm = new TeleportConfirmC2SPacket(packet.teleportId());
      Full position = new Full(playerEntity.getX(), playerEntity.getY(), playerEntity.getZ(), playerEntity.getYaw(), playerEntity.getPitch(), false, false);
      if (ProtocolTranslator.getTargetVersion().equalTo(ProtocolVersion.v1_21_2)) {
         this.connection.send(position);
         this.connection.send(teleportConfirm);
      } else {
         this.connection.send(teleportConfirm);
         this.connection.send(position);
      }
      if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_18)
         && this.client.currentScreen instanceof IDownloadingTerrainScreen downloadingTerrainScreen) {
         downloadingTerrainScreen.viaFabricPlus$setReady();
      }
   }
   ...
   private static boolean setPosition(PlayerPosition pos, Set<PositionFlag> flags, Entity entity, boolean bl) {
      ...
      if (bl && !bl2) {
         Vec3d newPosition = playerPosition2.position();
         boolean ignoreSmallChange = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_16_1)
            && Math.abs(entity.getX() - newPosition.getX()) < 0.03125
            && Math.abs(entity.getY() - newPosition.getY()) < 0.015625
            && Math.abs(entity.getZ() - newPosition.getZ()) < 0.03125;
         entity.updateTrackedPositionAndAngles(
            ignoreSmallChange ? entity.getX() : newPosition.getX(),
            ignoreSmallChange ? entity.getY() : newPosition.getY(),
            ignoreSmallChange ? entity.getZ() : newPosition.getZ(),
            playerPosition2.yaw(),
            playerPosition2.pitch(),
            ignoreSmallChange && ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_15_2) ? 0 : 3
         );
   ...
   private static boolean viaFabricPlus$isLogicalSideForUpdatingMovement(Entity entity) {
      ProtocolVersion version = ProtocolTranslator.getTargetVersion();
      if (version.newerThan(ProtocolVersion.v1_19_3) && !version.equals(BedrockProtocolVersion.bedrockLatest)) {
         return entity.isLogicalSideForUpdatingMovement();
      }
      return entity.getControllingPassenger() instanceof PlayerEntity player ? player.isMainPlayer() : !entity.getWorld().isClient;
   }
   ...
   public void onPlayerRotation(PlayerRotationS2CPacket packet) {
      ...
      this.connection.send(new LookAndOnGround(playerEntity.getYaw(), playerEntity.getPitch(), false, false));
   }
   ...
   private void unloadChunk(UnloadChunkS2CPacket packet) {
      ...
      ChunkTrackerHolder.get(this.world).onChunkStatusRemoved(packet.pos().x, packet.pos().z, ChunkStatus.FLAG_ALL);
   }
   ...
   public void onItemPickupAnimation(ItemPickupAnimationS2CPacket packet) {
      ...
      if (entity != null) {
         ...
         if (entity instanceof ItemEntity itemEntity) {
            ...
            if (itemStack.isEmpty()) {
               this.world.removeEntity(packet.getEntityId(), RemovalReason.DISCARDED);
            }
         ...
         for (int i : packet.getPassengerIds()) {
            ...
            if (entity2 != null) {
               ...
               if (entity2 == this.client.player) {
                  this.removedPlayerVehicleId = OptionalInt.empty();
                  if (!bl) {
                      if (entity instanceof AbstractBoatEntity && ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_18)) {
                     ...
                     Text text = Text.translatable("mount.onboard", new Object[]{this.client.options.sneakKey.getBoundKeyLocalizedText()});
   ...
   public void onPlayerRespawn(PlayerRespawnS2CPacket packet) {
      ...
      if (ProtocolTranslator.getTargetVersion().newerThanOrEqualTo(ProtocolVersion.v1_20_3) || bl) {
      ...
      if (packet.hasFlag((byte)2)) {
         List<SerializedEntry<?>> list = clientPlayerEntity.getDataTracker().getChangedEntries();
         if (list != null) {
   ...
   public void onSignEditorOpen(SignEditorOpenS2CPacket packet) {
      ...
      if (this.world.getBlockEntity(blockPos) instanceof SignBlockEntity signBlockEntity) {
         ...
         if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_21)) {
            SignBlockEntity emptySign = new SignBlockEntity(blockPos, this.world.getBlockState(blockPos));
            emptySign.setWorld(this.world);
            this.client.player.openEditSignScreen(emptySign, packet.isFront());
         } else {
   ...
   public void onGameStateChange(GameStateChangeS2CPacket packet) {
      ...
      if (reason == GameStateChangeS2CPacket.NO_RESPAWN_BLOCK) {
         ...
         if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20_5) && i == 0) {
            this.client.player.networkHandler.sendPacket(new ClientStatusC2SPacket(Mode.PERFORM_RESPAWN));
         ...
         if (f == 0.0F) {
            ...
            this.client.inGameHud.getChatHud().addMessage(Text.translatable("demo.help.jump", new Object[]{gameOptions.jumpKey.getBoundKeyLocalizedText()}));
         } else if (f == 103.0F) {
            this.client
               .inGameHud
               .getChatHud()
               .addMessage(Text.translatable("demo.help.inventory", new Object[]{gameOptions.inventoryKey.getBoundKeyLocalizedText()}));
         } else if (f == 104.0F) {
            this.client.inGameHud.getChatHud().addMessage(Text.translatable("demo.day.6", new Object[]{gameOptions.screenshotKey.getBoundKeyLocalizedText()}));
         }
   ...
   public void onStatistics(StatisticsS2CPacket packet) {
      ...
      if (this.client.currentScreen instanceof StatsScreen statsScreenx) {
   ...
   public void onRecipeBookAdd(RecipeBookAddS2CPacket packet) {
      ...
      for (net.minecraft.network.packet.s2c.play.RecipeBookAddS2CPacket.Entry entry : packet.entries()) {
   ...
   private <T> PendingTagLoad<T> startTagReload(
      RegistryKey<? extends Registry<? extends T>> registryRef, net.minecraft.registry.tag.TagPacketSerializer.Serialized serialized
   ) {
   ...
   public void onSynchronizeTags(SynchronizeTagsS2CPacket packet) {
      NetworkThreadUtils.forceMainThread(packet, this, this.client);
      List<PendingTagLoad<?>> list = new ArrayList<>(packet.getGroups().size());
   ...
   public void onPlayerList(PlayerListS2CPacket packet) {
      ...
      for (net.minecraft.network.packet.s2c.play.PlayerListS2CPacket.Entry entry : packet.getPlayerAdditionEntries()) {
         PlayerListEntry playerListEntry = new PlayerListEntry(Objects.requireNonNull(entry.profile()), this.isSecureChatEnforced());
   ...
   private void handlePlayerListAction(
      Action action, net.minecraft.network.packet.s2c.play.PlayerListS2CPacket.Entry receivedEntry, PlayerListEntry currentEntry
   ) {
      switch (action) {
         ...
         case UPDATE_GAME_MODE:
            if (currentEntry.getGameMode() != receivedEntry.gameMode()
               ...
               && this.client.player.getUuid().equals(receivedEntry.profileId())) {
   ...
   private void setPublicSession(net.minecraft.network.packet.s2c.play.PlayerListS2CPacket.Entry receivedEntry, PlayerListEntry currentEntry) {
      ...
      if (signatureVerifier == null) {
         ...
         net.minecraft.network.encryption.PublicPlayerSession.Serialized serialized = receivedEntry.chatSession();
         if (serialized != null) {
            try {
            ...
            } catch (PublicKeyException publicKeyException) {
               if (ProtocolTranslator.getTargetVersion().newerThanOrEqualTo(ProtocolVersion.v1_19_4)) {
                  LOGGER.error("Failed to validate profile key for player: '{}'", gameProfile.getName(), publicKeyException);
               }
               currentEntry.resetSession(this.isSecureChatEnforced());
            }
         } else {
   ...
   public void onOpenWrittenBook(OpenWrittenBookS2CPacket packet) {
      ...
      if (ProtocolTranslator.getTargetVersion().olderThan(ProtocolVersion.v1_20_5) && !itemStack.isOf(Items.WRITTEN_BOOK)) {
         return;
      }
   ...
   public void onScoreboardObjectiveUpdate(ScoreboardObjectiveUpdateS2CPacket packet) {
      ...
      if (packet.getMode() == 0) {
         this.scoreboard
            .addObjective(
               string, ScoreboardCriterion.DUMMY, packet.getDisplayName(), packet.getType(), false, (NumberFormat)packet.getNumberFormat().orElse(null)
            );
      } else {
   ...
   public void onTeam(TeamS2CPacket packet) {
      ...
      Optional<SerializableTeam> optional = packet.getTeam();
      optional.ifPresent(teamx -> {
         ...
         VisibilityRule visibilityRule = VisibilityRule.getRule(teamx.getNameTagVisibilityRule());
         if (visibilityRule != null) {
            team.setNameTagVisibilityRule(visibilityRule);
         }

         CollisionRule collisionRule = CollisionRule.getRule(teamx.getCollisionRule());
      ...
      Operation operation2 = packet.getPlayerListOperation();
      if (operation2 == Operation.ADD) {
         for (String string : packet.getPlayerNames()) {
            this.scoreboard.addScoreHolderToTeam(string, team);
         }
      } else if (operation2 == Operation.REMOVE) {
         for (String string : packet.getPlayerNames()) {
   ...
   public void onParticle(ParticleS2CPacket packet) {
      ...
      if (packet.getCount() == 0) {
         ...
         try {
            this.world
               .addParticle(packet.getParameters(), packet.shouldForceSpawn(), packet.isImportant(), packet.getX(), packet.getY(), packet.getZ(), d, e, f);
   ...
   public void onEntityAttributes(EntityAttributesS2CPacket packet) {
      ...
      if (entity != null) {
         ...
         for (net.minecraft.network.packet.s2c.play.EntityAttributesS2CPacket.Entry entry : packet.getEntries()) {
   ...
   private void readLightData(int x, int z, LightData data, boolean bl) {
      ...
      ChunkTrackerHolder.get(this.world).onChunkStatusAdded(x, z, ChunkStatus.FLAG_HAS_LIGHT_DATA);
   }
      ...
      for (int i = 0; i < provider.getHeight(); i++) {
         ...
         if (bl2 || bl3) {
            provider.enqueueSectionData(
               type, ChunkSectionPos.from(chunkX, j, chunkZ), bl2 ? new ChunkNibbleArray((byte[])nibbles.next().clone()) : new ChunkNibbleArray()
            );
            if (bl) {
   ...
   public void sendChatCommand(String command) {
      ...
      if (signedArgumentList.arguments().isEmpty() && ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_20_3)) {
   ...
   public boolean sendCommand(String command) {
      if (!SignedArgumentList.isNotEmpty(this.parse(command))) {
         if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20_3)) {
            this.sendPacket(
               new ChatCommandSignedC2SPacket(
                  command, Instant.now(), 0L, ArgumentSignatureDataMap.EMPTY, this.lastSeenMessagesCollector.collect().update()
               )
            );
         } else {
            this.sendPacket(new CommandExecutionC2SPacket(command));
