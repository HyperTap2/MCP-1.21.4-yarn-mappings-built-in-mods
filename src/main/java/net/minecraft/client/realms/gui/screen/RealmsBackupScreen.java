...
public class RealmsBackupScreen extends RealmsScreen {
      ...
      (new Thread("Realms-fetch-backups") {
            ...
            public void run() {
               ...
               try {
                              ...
                              RealmsBackupScreen.this.selectionList
                                 .replaceEntries(
                                    RealmsBackupScreen.this.backups
                                       .stream()
                                       .map(backup -> RealmsBackupScreen.this.new BackupObjectSelectionListEntry(backup))
                                       .toList()
                                 );
                           }
                        }
                  ...
                  RealmsBackupScreen.LOGGER.error("Couldn't request backups", realmsServiceException);
               }
            }
         })
         .start();
   ...
   class BackupObjectSelectionListEntry extends ElementListWidget.Entry<RealmsBackupScreen.BackupObjectSelectionListEntry> {
            ...
            this.changesButton = ButtonWidget.builder(
               ...
               .narrationSupplier(
                  textSupplier -> ScreenTexts.joinSentences(
                     new Text[]{Text.translatable("mco.backup.narration", new Object[]{this.getLastModifiedDate()}), (Text)textSupplier.get()}
                  )
               )
               .build();
         ...
         if (i != RealmsBackupScreen.this.backups.size() - 1) {
            ...
            for (String string : backup.metadata.keySet()) {
               if (!string.contains("uploaded") && backup2.metadata.containsKey(string)) {
                  if (!backup.metadata.get(string).equals(backup2.metadata.get(string))) {
      ...
      private void restore() {
         ...
         Text text2 = Text.translatable("mco.configure.world.restore.question.line1", new Object[]{this.getLastModifiedDate(), text});
         RealmsBackupScreen.this.client
            .setScreen(
               RealmsPopups.createContinuableWarningPopup(
                  ...
                  popup -> RealmsBackupScreen.this.client
                     .setScreen(
                        new RealmsLongRunningMcoTaskScreen(
                           RealmsBackupScreen.this.parent.getNewScreen(),
                           new RestoreTask(this.mBackup, RealmsBackupScreen.this.serverData.id, RealmsBackupScreen.this.parent)
                        )
                     )
               )
            );
      ...
      public void render(
         ...
         context.drawTextWithShadow(
            ...
            Text.translatable("mco.backup.entry", new Object[]{RealmsUtil.convertToAgePresentation(this.mBackup.lastModifiedDate)}),
            x,
            j,
            l
         );
