...
public class RealmsSubscriptionInfoScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addDrawableChild(
         ButtonWidget.builder(
               ...
               button -> ConfirmLinkScreen.open(
                  this, Urls.getExtendJavaRealmsUrl(this.serverData.remoteSubscriptionId, this.client.getSession().getUuidOrNull())
               )
            )
      ...
      if (this.serverData.expired) {
         ...
         this.addDrawableChild(
            new ScrollableTextWidget(
               ...
               200,
               46,
               Text.translatable("mco.snapshot.subscription.info", new Object[]{this.serverData.parentWorldName}),
               this.textRenderer
            )
         );
      } else {
   ...
   public Text getNarratedTitle() {
      return ScreenTexts.joinLines(new Text[]{SUBSCRIPTION_TITLE, SUBSCRIPTION_START_LABEL_TEXT, this.startDate, TIME_LEFT_LABEL_TEXT, this.daysLeft});
   }
      ...
      (new Thread("Realms-delete-realm") {
            ...
            public void run() {
               try {
                  ...
                  RealmsSubscriptionInfoScreen.LOGGER.error("Couldn't delete world", realmsServiceException);
               }

               RealmsSubscriptionInfoScreen.this.client
                  .execute(() -> RealmsSubscriptionInfoScreen.this.client.setScreen(RealmsSubscriptionInfoScreen.this.mainScreen));
            }
         })
         .start();
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawTextWithShadow(this.textRenderer, SUBSCRIPTION_START_LABEL_TEXT, i, row(0), -6250336);
      context.drawTextWithShadow(this.textRenderer, this.startDate, i, row(1), -1);
      if (this.type == Subscription.SubscriptionType.NORMAL) {
         context.drawTextWithShadow(this.textRenderer, TIME_LEFT_LABEL_TEXT, i, row(3), -6250336);
      } else if (this.type == Subscription.SubscriptionType.RECURRING) {
         context.drawTextWithShadow(this.textRenderer, DAYS_LEFT_LABEL_TEXT, i, row(3), -6250336);
      }

      context.drawTextWithShadow(this.textRenderer, this.daysLeft, i, row(4), -1);
   }
   ...
   private Text daysLeftPresentation(int daysLeft) {
      if (daysLeft < 0 && this.serverData.expired) {
         ...
         boolean bl2 = j > 0;
         if (bl && bl2) {
            return Text.translatable("mco.configure.world.subscription.remaining.months.days", new Object[]{i, j});
         } else if (bl) {
            return Text.translatable("mco.configure.world.subscription.remaining.months", new Object[]{i});
         } else {
            return bl2 ? Text.translatable("mco.configure.world.subscription.remaining.days", new Object[]{j}) : Text.empty();
         }
      }
   }
}
