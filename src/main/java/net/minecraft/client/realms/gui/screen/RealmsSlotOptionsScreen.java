...
public class RealmsSlotOptionsScreen extends RealmsScreen {
   ...
   public void init() {
      ...
      this.addDrawableChild(
         CyclingButtonWidget.<GameMode>builder(GameMode::getSimpleTranslatableName)
            ...
            .build(
               this.column1_x, row(3), this.column2_x, 20, Text.translatable("selectWorld.gameMode"), (button, gameModeIndex) -> this.gameMode = gameModeIndex
            )
      );
      this.spawnProtectionButton = this.addDrawableChild(
         new RealmsSlotOptionsScreen.SettingsSlider(i, row(3), this.column2_x, this.spawnProtection, 0.0F, 16.0F)
      );
      ...
      CyclingButtonWidget<Boolean> cyclingButtonWidget4 = this.addDrawableChild(
         CyclingButtonWidget.onOffBuilder(this.commandBlocks)
            .build(
               ...
               this.column2_x,
               20,
               Text.translatable("mco.configure.world.commandBlocks"),
               (button, commandBlocks) -> this.commandBlocks = commandBlocks
            )
      );
      if (this.worldType != RealmsServer.WorldType.NORMAL) {
   ...
   public Text getNarratedTitle() {
      return ScreenTexts.joinSentences(new Text[]{this.getTitle(), this.narrateLabels()});
   }
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawTextWithShadow(
         this.textRenderer, EDIT_SLOT_NAME, this.column1_x + this.column2_x / 2 - this.textRenderer.getWidth(EDIT_SLOT_NAME) / 2, row(0) - 5, -1
      );
         ...
         this.setMessage(
            ScreenTexts.composeGenericOptionText(
               ...
               (Text)(RealmsSlotOptionsScreen.this.spawnProtection == 0
                  ? ScreenTexts.OFF
                  : Text.literal(String.valueOf(RealmsSlotOptionsScreen.this.spawnProtection)))
            )
         );
      }
   }
