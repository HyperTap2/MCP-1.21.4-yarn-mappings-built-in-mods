      ...
      for (Entry<Integer, RealmsWorldOptions> entry : this.serverData.slots.entrySet()) {
         ...
         if (bl) {
            buttonWidget = ButtonWidget.builder(
               ...
               .build();
            buttonWidget.active = !this.serverData.slots.get(i).empty;
         } else {
            buttonWidget = ButtonWidget.builder(
                  ...
                  button -> this.client
                     .setScreen(
                        RealmsPopups.createInfoPopup(
                           this, Text.translatable("mco.configure.world.restore.download.question.line1"), popupScreen -> this.downloadWorld(i)
                        )
                     )
               )
               .dimensions(this.getFramePositionX(i), row(8), 80, 20)
