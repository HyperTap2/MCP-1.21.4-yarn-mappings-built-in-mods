...
public class RealmsSelectFileToUploadScreen extends RealmsScreen {
   ...
   private void loadLevelList() {
      LevelList levelList = this.client.getLevelStorage().getLevelList();
      this.levelList = this.client.getLevelStorage().loadSummaries(levelList).join()
         .stream()
   ...
   class WorldListEntry extends AlwaysSelectedEntryListWidget.Entry<RealmsSelectFileToUploadScreen.WorldListEntry> {
      ...
      public WorldListEntry(final LevelSummary summary) {
         ...
         this.nameAndLastPlayed = Text.translatable(
            "mco.upload.entry.id", new Object[]{summary.getName(), RealmsSelectFileToUploadScreen.getLastPlayed(summary)}
         );
      ...
      protected void renderItem(DrawContext context, int index, int x, int y) {
         ...
         context.drawTextWithShadow(RealmsSelectFileToUploadScreen.this.textRenderer, string, x + 2, y + 1, -1);
