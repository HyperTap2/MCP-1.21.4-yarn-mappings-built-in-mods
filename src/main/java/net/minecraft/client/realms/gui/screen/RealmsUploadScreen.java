...
public class RealmsUploadScreen extends RealmsScreen implements UploadProgressTracker {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      if (this.showDots) {
         context.drawTextWithShadow(
            this.textRenderer, DOTS[this.animTick / 10 % DOTS.length], this.width / 2 + this.textRenderer.getWidth(this.status) / 2 + 5, 50, -1
         );
      }
   ...
   private void drawProgressBar(DrawContext context) {
      ...
      context.drawCenteredTextWithShadow(this.textRenderer, Text.translatable("mco.upload.percent", new Object[]{this.progress}), this.width / 2, 84, -1);
   }
   ...
   private Text getNarration() {
      ...
      if (this.progress != null) {
         list.add(Text.translatable("mco.upload.percent", new Object[]{this.progress}));
      }
      ...
      realmsUploader.upload().handleAsync((v, throwable) -> {
         if (throwable != null) {
            ...
            if (throwable instanceof RealmsUploadException realmsUploadException) {
               ...
               this.status = Text.translatable("mco.upload.failed", new Object[]{throwable.getMessage()});
            }
         } else {
   ...
   public void updateProgressDisplay() {
      this.status = Text.translatable("mco.upload.uploading", new Object[]{this.selectedLevel.getDisplayName()});
   }
}
