   ...
   public void init() {
      ...
      directionalLayoutWidget.add(
         ButtonWidget.builder(
               Text.translatable("mco.configure.world.buttons.invite"),
   ...
   class InvitedObjectSelectionList extends ElementListWidget<RealmsPlayerScreen.InvitedObjectSelectionListEntry> {
      ...
      protected void renderHeader(DrawContext context, int x, int y) {
         ...
         context.drawTextWithShadow(
            RealmsPlayerScreen.this.textRenderer, text, x + this.getRowWidth() / 2 - RealmsPlayerScreen.this.textRenderer.getWidth(text) / 2, y, -1
         );
      }
   ...
   class InvitedObjectSelectionListEntry extends ElementListWidget.Entry<RealmsPlayerScreen.InvitedObjectSelectionListEntry> {
      ...
      public InvitedObjectSelectionListEntry(final PlayerInfo playerInfo) {
         ...
         this.opButton = TextIconButtonWidget.builder(NORMAL_TOOLTIP_TEXT, button -> this.op(i), false)
            ...
            .narration(
               supplier -> ScreenTexts.joinSentences(
                  new Text[]{
                     Text.translatable("mco.invited.player.narration", new Object[]{playerInfo.getName()}),
                     (Text)supplier.get(),
                     Text.translatable("narration.cycle_button.usage.focused", new Object[]{OPS_TOOLTIP_TEXT})
                  }
               )
            )
            .build();
            ...
            .narration(
               supplier -> ScreenTexts.joinSentences(
                  new Text[]{
                     Text.translatable("mco.invited.player.narration", new Object[]{playerInfo.getName()}),
                     (Text)supplier.get(),
                     Text.translatable("narration.cycle_button.usage.focused", new Object[]{NORMAL_TOOLTIP_TEXT})
                  }
               )
            )
            .build();
         this.uninviteButton = TextIconButtonWidget.builder(REMOVE_TOOLTIP_TEXT, button -> this.uninvite(i), false)
            ...
            .narration(
               supplier -> ScreenTexts.joinSentences(
                  new Text[]{Text.translatable("mco.invited.player.narration", new Object[]{playerInfo.getName()}), (Text)supplier.get()}
               )
            )
            .build();
