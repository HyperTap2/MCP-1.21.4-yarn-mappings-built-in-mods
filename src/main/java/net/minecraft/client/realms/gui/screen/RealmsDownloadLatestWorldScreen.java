...
public class RealmsDownloadLatestWorldScreen extends RealmsScreen {
   ...
   private void checkDownloadSize() {
      if (!this.finished && !this.checked) {
         ...
         if (this.getContentLength(this.worldDownload.downloadLink) >= 5368709120L) {
            Text text = Text.translatable("mco.download.confirmation.oversized", new Object[]{SizeUnit.getUserFriendlyString(5368709120L)});
   ...
   private Text getNarration() {
      ...
      if (this.progress != null) {
         list.add(Text.translatable("mco.download.percent", new Object[]{this.progress}));
         list.add(Text.translatable("mco.download.speed.narration", new Object[]{SizeUnit.getUserFriendlyString(this.bytesPerSecond)}));
      }
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      context.drawCenteredTextWithShadow(this.textRenderer, this.downloadTitle, this.width / 2, 20, -1);
      ...
      if (this.downloadError != null) {
         context.drawCenteredTextWithShadow(this.textRenderer, this.downloadError, this.width / 2, 110, -65536);
      }
   }
   ...
   private void drawDots(DrawContext context) {
      ...
      context.drawTextWithShadow(this.textRenderer, DOTS[this.dotIndex % DOTS.length], this.width / 2 + i / 2 + 5, 50, -1);
   }
   ...
   private void drawProgressBar(DrawContext context) {
      ...
      context.drawCenteredTextWithShadow(this.textRenderer, Text.translatable("mco.download.percent", new Object[]{this.progress}), this.width / 2, 84, -1);
   }
   ...
   private void drawDownloadSpeed0(DrawContext context, long bytesPerSecond) {
      if (bytesPerSecond > 0L) {
         ...
         context.drawTextWithShadow(
            this.textRenderer,
            Text.translatable("mco.download.speed", new Object[]{SizeUnit.getUserFriendlyString(bytesPerSecond)}),
            this.width / 2 + i / 2 + 15,
            84,
            -1
         );
      ...
      new Thread(() -> {
         try {
            try {
               ...
               this.status = Text.translatable("mco.download.downloading", new Object[]{this.worldName});
               FileDownload fileDownload = new FileDownload();
