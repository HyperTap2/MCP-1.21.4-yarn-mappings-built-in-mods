      ...
      (new Thread("Realms-connect-task") {
            ...
            public void run() {
               ...
               try {
                  ...
                  RealmsConnection.this.connection
                     .send(new LoginHelloC2SPacket(minecraftClient.getSession().getUsername(), minecraftClient.getSession().getUuidOrNull()));
                  minecraftClient.ensureAbuseReportContext(ReporterEnvironment.ofRealm(server));
                  minecraftClient.getQuickPlayLogger()
                     .setWorld(QuickPlayLogger.WorldType.REALMS, String.valueOf(server.id), Objects.requireNonNullElse(server.name, "unknown"));
                  ...
                  String stringx = exception.toString();
                  ...
                  DisconnectedRealmsScreen disconnectedRealmsScreen = new DisconnectedRealmsScreen(
                     RealmsConnection.this.onlineScreen, ScreenTexts.CONNECT_FAILED, Text.translatable("disconnect.genericReason", new Object[]{stringx})
                  );
