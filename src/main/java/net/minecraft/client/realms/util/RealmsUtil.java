...
public class RealmsUtil {
   ...
   public static Text convertToAgePresentation(long milliseconds) {
      if (milliseconds < 0L) {
         ...
         if (l < 60L) {
            return Text.translatable("mco.time.secondsAgo", new Object[]{l});
         } else if (l < 3600L) {
            long m = l / 60L;
            return Text.translatable("mco.time.minutesAgo", new Object[]{m});
         } else if (l < 86400L) {
            long m = l / 3600L;
            return Text.translatable("mco.time.hoursAgo", new Object[]{m});
         } else {
            long m = l / 86400L;
            return Text.translatable("mco.time.daysAgo", new Object[]{m});
         }
      }
   }
