...
public interface Oxidizable extends Degradable<Oxidizable.OxidationLevel> {
   ...
   static Block getUnaffectedOxidationBlock(Block block) {
      ...
      for (Block block3 = (Block)OXIDATION_LEVEL_DECREASES.get().get(block2); block3 != null; block3 = (Block)OXIDATION_LEVEL_DECREASES.get().get(block2)) {
         block2 = block3;
      }
   ...
   enum OxidationLevel implements StringIdentifiable {
      ...
      OxidationLevel(final String id) {
         this.id = id;
      }
