...
public final class SaplingGenerator {
   ...
   private RegistryKey<ConfiguredFeature<?, ?>> getSmallTreeFeature(Random random, boolean flowersNearby) {
      ...
      return flowersNearby && this.beesVariant.isPresent() ? this.beesVariant.get() : this.regularVariant.orElse(null);
   }

   @Nullable
   private RegistryKey<ConfiguredFeature<?, ?>> getMegaTreeFeature(Random random) {
      return this.rareMegaVariant.isPresent() && random.nextFloat() < this.rareChance ? this.rareMegaVariant.get() : this.megaVariant.orElse(null);
   }
