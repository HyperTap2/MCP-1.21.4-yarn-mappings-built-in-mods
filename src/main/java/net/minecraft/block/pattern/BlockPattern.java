...
public class BlockPattern implements BlockPatternExtended {
   private final Predicate<CachedBlockPosition>[][][] pattern;
   private final int depth;
   private final int height;
   private final int width;
   @Nullable
   private Block lithium$requiredBlock;
   ...
   @Nullable
   @VisibleForTesting
   public BlockPattern.Result testTransform(WorldView world, BlockPos frontTopLeft, Direction forwards, Direction up) {
   ...
   public BlockPattern.Result searchAround(WorldView world, BlockPos pos) {
      ...
      if (this.lithium$requiredBlock != null) {
         BlockPos maxCorner = pos.add(2 * i - 1, 2 * i - 1, 2 * i - 1);
         BlockPos minCorner = pos.add(-i, -i, -i);
         if (!BlockSearch.hasAtLeast(world, BlockBox.create(minCorner, maxCorner), this.lithium$requiredBlock, this.lithium$requiredBlockCount)) {
            return null;
         }
      }
