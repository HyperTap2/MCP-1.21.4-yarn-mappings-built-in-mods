...
public class RedstoneWireBlock extends Block {
   ...
   protected BlockState getStateForNeighborUpdate(
      ...
      return wireConnection.isConnected() == state.get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction)).isConnected() && !isFullyConnected(state)
         ...
         : this.getPlacementState(
            world, this.dotState.with(POWER, state.get(POWER)).with(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction), wireConnection), pos
         );
   }
   ...
   public int getStrongPower(World world, BlockPos pos) {
      return RedstoneWirePowerCalculations.getNeighborBlockSignal(this, this.redstoneController, world, pos);
   }
      ...
      if (this.wiresGivePower && direction != Direction.DOWN) {
         ...
         if (i == 0) {
            ...
            return direction != Direction.UP
                  && !this.getPlacementState(world, state, pos).get(DIRECTION_TO_WIRE_CONNECTION_PROPERTY.get(direction.getOpposite())).isConnected()
               ? 0
               : i;
         }
      } else {
   ...
   private static void addPoweredParticles(
      ...
      if (!(random.nextFloat() >= 0.2F * f)) {
         ...
         world.addParticle(new DustParticleEffect(color, 1.0F), pos.getX() + d, pos.getY() + e, pos.getZ() + i, 0.0, 0.0, 0.0);
      }
   }
