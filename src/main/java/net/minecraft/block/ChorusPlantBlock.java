...
public class ChorusPlantBlock extends ConnectingBlock {
   ...
   public ChorusPlantBlock(AbstractBlock.Settings settings) {
      ...
      this.setDefaultState(
         this.stateManager.getDefaultState().with(NORTH, false).with(EAST, false).with(SOUTH, false).with(WEST, false).with(UP, false).with(DOWN, false)
      );
   }
   ...
   public static BlockState withConnectionProperties(BlockView world, BlockPos pos, BlockState state) {
      ...
      return state.withIfExists(DOWN, blockState.isOf(block) || blockState.isOf(Blocks.CHORUS_FLOWER) || blockState.isOf(Blocks.END_STONE))
