      ...
      if (getPhase(state) != SculkSensorPhase.ACTIVE) {
         if (getPhase(state) == SculkSensorPhase.COOLDOWN) {
            ...
            if (!state.get(WATERLOGGED)) {
               world.playSound(
                  (PlayerEntity)null, pos, SoundEvents.BLOCK_SCULK_SENSOR_CLICKING_STOP, SoundCategory.BLOCKS, 1.0F, world.random.nextFloat() * 0.2F + 0.8F
               );
            }
         }
      } else {
   ...
   public static void tryResonate(@Nullable Entity sourceEntity, World world, BlockPos pos, int frequency) {
      for (Direction direction : Direction.values()) {
         ...
         if (blockState.isIn(BlockTags.VIBRATION_RESONATORS)) {
            ...
            world.playSound((PlayerEntity)null, blockPos, SoundEvents.BLOCK_AMETHYST_BLOCK_RESONATE, SoundCategory.BLOCKS, 1.0F, f);
         }
      }
   }
