...
import malte0811.ferritecore.FerriteCoreBlockStateCache;
...
public abstract class AbstractBlock implements ToggleableFeature {
   ...
   protected float calcBlockBreakingDelta(BlockState state, PlayerEntity player, BlockView world, BlockPos pos) {
      ...
      if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(LegacyProtocolVersion.r1_4_6tor1_4_7)) {
         return !player.canHarvest(state) ? 1.0F / f / 100.0F : player.getBlockBreakingSpeed(state) / f / 30.0F;
      }
   ...
   public final ItemStack viaFabricPlus$getPickStack(WorldView world, BlockPos pos, BlockState state, boolean includeData) {
      return this.getPickStack(world, pos, state, includeData);
   }
   ...
   public abstract static class AbstractBlockState extends State<Block, BlockState> implements BlockStateFlagHolder, BlockStatePathingCache {
      ...
      @Nullable
      private PathNodeType lithium$neighborPathNodeType;
      ...
      public void lithium$initializeFlags() {
         ...
         for (int i = 0; i < BlockStateFlags.FLAGS.length; i++) {
            if (BlockStateFlags.FLAGS[i].test(this.asBlockState())) {
               flags |= 1 << i;
            }
         }
      ...
      public void lithium$initializePathNodeTypeCache() {
         this.lithium$pathNodeType = null;
         this.lithium$neighborPathNodeType = null;
         BlockState state = this.asBlockState();
         SingleBlockBlockView view = SingleBlockBlockView.of(state, BlockPos.ORIGIN);

         try {
            this.lithium$pathNodeType = Objects.requireNonNull(LandPathNodeMaker.getCommonNodeType(view, BlockPos.ORIGIN));
         } catch (SingleBlockBlockView.SingleBlockViewException | ClassCastException | NullPointerException exception) {
            this.lithium$pathNodeType = null;
         }

         if (this.lithium$pathNodeType != null) {
            this.lithium$neighborPathNodeType = PathNodeCache.getNeighborPathNodeType(this.lithium$pathNodeType);
         }
      }
      ...
      public PathNodeType lithium$getNeighborPathNodeType() {
         return this.lithium$neighborPathNodeType;
      }
      ...
      private boolean shouldBeSolid() {
         ...
         if (this.owner.settings.forceNotSolid) {
            return false;
         }
      ...
      public float getAmbientOcclusionLightLevel(BlockView world, BlockPos pos) {
         float original = this.getBlock().getAmbientOcclusionLightLevel(this.asBlockState(), world, pos);
         float aoLevel = WorldRenderingSettings.INSTANCE.getAmbientOcclusionLevel();
         return 1.0F - aoLevel * (1.0F - original);
      }
      ...
      public float getHardness(BlockView world, BlockPos pos) {
         ...
         if ((block == Blocks.END_STONE_BRICKS
            ...
            && ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_14_4)) {
            return 0.8F;
         }
         ...
         if (block == Blocks.OBSIDIAN && ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(LegacyProtocolVersion.b1_8tob1_8_1)) {
            return 10.0F;
         }
      ...
      public boolean isToolRequired() {
         return this.getBlock() instanceof ShulkerBoxBlock && ProtocolTranslator.getTargetVersion().olderThan(ProtocolVersion.v1_14)
            || this.toolRequired;
      }
      ...
      static final class ShapeCache {
         ...
         ShapeCache(BlockState state) {
            Block block = state.getBlock();
            this.collisionShape = FerriteCoreBlockStateCache.deduplicateCollisionShape(
            ...
            this.solidSides = FerriteCoreBlockStateCache.deduplicateSolidSides(this.solidSides);
   ...
   public static class Settings {
      public static final Codec<AbstractBlock.Settings> CODEC = Codec.unit(() -> create());
