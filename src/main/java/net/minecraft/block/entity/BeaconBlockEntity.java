...
public class BeaconBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, Nameable {
   ...
   public static void tick(World world, BlockPos pos, BlockState state, BeaconBlockEntity blockEntity) {
      ...
      BeaconBlockEntity.BeamSegment beamSegment = blockEntity.field_19178.isEmpty() ? null : blockEntity.field_19178.get(blockEntity.field_19178.size() - 1);
   ...
   private static RegistryEntry<StatusEffect> readStatusEffect(NbtCompound nbt, String key) {
      if (nbt.contains(key, 8)) {
         ...
         return identifier == null ? null : Registries.STATUS_EFFECT.getEntry(identifier).map(BeaconBlockEntity::getEffectOrNull).orElse(null);
      } else {
         return null;
      }
   }
