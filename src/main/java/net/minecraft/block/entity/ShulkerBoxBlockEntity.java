...
public class ShulkerBoxBlockEntity extends LootableContainerBlockEntity implements SidedInventory, SleepingBlockEntity {
   ...
   private void updateAnimation(World world, BlockPos pos, BlockState state) {
      ...
      if (this.animationStage == ShulkerBoxBlockEntity.AnimationStage.CLOSED && this.animationProgress == 0.0F && this.prevAnimationProgress == 0.0F) {
   ...
   public void onOpen(PlayerEntity player) {
      if (!this.removed && !player.isSpectator()) {
         ...
         if (this.viewerCount == 1) {
            ...
            this.world
               .playSound(
                  (PlayerEntity)null, this.pos, SoundEvents.BLOCK_SHULKER_BOX_OPEN, SoundCategory.BLOCKS, 0.5F, this.world.random.nextFloat() * 0.1F + 0.9F
               );
         }
      }
   }
   ...
   public void onClose(PlayerEntity player) {
      if (!this.removed && !player.isSpectator()) {
         ...
         if (this.viewerCount <= 0) {
            ...
            this.world
               .playSound(
                  (PlayerEntity)null, this.pos, SoundEvents.BLOCK_SHULKER_BOX_CLOSE, SoundCategory.BLOCKS, 0.5F, this.world.random.nextFloat() * 0.1F + 0.9F
               );
         }
      }
   }
   ...
   @Override public void lithium$setTickWrapper(WorldChunk.WrappedBlockEntityTickInvoker wrapper) { this.lithium$tickWrapper = wrapper; }
