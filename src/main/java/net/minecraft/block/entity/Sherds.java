...
public record Sherds(Optional<Item> back, Optional<Item> left, Optional<Item> right, Optional<Item> front) {
   ...
   public NbtCompound toNbt(NbtCompound nbt) {
      ...
      nbt.put("sherds", (NbtElement)CODEC.encodeStart(NbtOps.INSTANCE, this).getOrThrow());
      return nbt;
   }
   ...
   public static Sherds fromNbt(@Nullable NbtCompound nbt) {
      return nbt != null && nbt.contains("sherds") ? CODEC.parse(NbtOps.INSTANCE, nbt.get("sherds")).result().orElse(DEFAULT) : DEFAULT;
   }
}
