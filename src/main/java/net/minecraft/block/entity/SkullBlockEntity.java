...
public class SkullBlockEntity extends BlockEntity {
   ...
   protected void writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
      ...
      if (this.owner != null) {
         nbt.put("profile", (NbtElement)ProfileComponent.CODEC.encodeStart(NbtOps.INSTANCE, this.owner).getOrThrow());
      }
   ...
   public float getPoweredTicks(float tickDelta) {
      return this.powered ? this.poweredTicks + tickDelta : this.poweredTicks;
   }
   ...
   public static CompletableFuture<Optional<GameProfile>> fetchProfileByName(String name) {
      ...
      return loadingCache != null && StringHelper.isValidPlayerName(name)
         ? (CompletableFuture)loadingCache.getUnchecked(name)
         : CompletableFuture.completedFuture(Optional.empty());
   }
