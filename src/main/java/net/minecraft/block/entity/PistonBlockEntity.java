...
public class PistonBlockEntity extends BlockEntity {
   ...
   public float getRenderOffsetX(float tickDelta) {
      return this.facing.getOffsetX() * this.getAmountExtended(this.getProgress(tickDelta));
   }

   public float getRenderOffsetY(float tickDelta) {
      return this.facing.getOffsetY() * this.getAmountExtended(this.getProgress(tickDelta));
   }

   public float getRenderOffsetZ(float tickDelta) {
      return this.facing.getOffsetZ() * this.getAmountExtended(this.getProgress(tickDelta));
   }
   ...
   public VoxelShape getCollisionShape(BlockView world, BlockPos pos) {
      ...
      float absoluteOffset = Math.abs(f);
      if (absoluteOffset == 0.0F || absoluteOffset == 0.5F || absoluteOffset == 1.0F) {
         if (!this.extending && this.source && this.pushedBlock.getBlock() instanceof PistonBlock) {
            return PISTON_BASE_WITH_MOVING_HEAD_SHAPES[getMergedShapeIndex(f, this.facing)];
         }
         VoxelShape blockShape = blockState.getCollisionShape(world, pos);
         return getOffsetAndSimplified(blockShape, absoluteOffset, f < 0.0F ? this.facing.getOpposite() : this.facing);
      }
   ...
   private static VoxelShape getOffsetAndSimplified(VoxelShape blockShape, float offset, Direction direction) {
      OffsetVoxelShapeCache cache = (OffsetVoxelShapeCache)blockShape;
      VoxelShape cached = cache.lithium$getOffsetSimplifiedShape(offset, direction);
      if (cached == null) {
         cached = blockShape
            .offset(direction.getOffsetX() * offset, direction.getOffsetY() * offset, direction.getOffsetZ() * offset)
            .simplify();
         cache.lithium$setShape(offset, direction, cached);
      }
      return cached;
   }
   ...
   private static VoxelShape[] precomputePistonBaseWithMovingHeadShapes() {
      ...
      VoxelShape[] mergedShapes = new VoxelShape[offsets.length * Direction.values().length];
      for (Direction facing : Direction.values()) {
         VoxelShape baseShape = Blocks.PISTON
            .getDefaultState()
            .with(PistonBlock.EXTENDED, true)
            .with(PistonBlock.FACING, facing)
            .getCollisionShape(null, null);
         for (float offset : offsets) {
            VoxelShape headShape = Blocks.PISTON_HEAD
               .getDefaultState()
               .with(PistonHeadBlock.FACING, facing)
               .with(PistonHeadBlock.SHORT, offset < 0.25F)
               .getCollisionShape(null, null);
            VoxelShape offsetHead = headShape.offset(
               facing.getOffsetX() * offset, facing.getOffsetY() * offset, facing.getOffsetZ() * offset
            );
            mergedShapes[getMergedShapeIndex(offset, facing)] = VoxelShapes.union(baseShape, offsetHead);
         }
      }
      return mergedShapes;
   }

   private static int getMergedShapeIndex(float offset, Direction direction) {
      if (offset != 0.0F && offset != 0.5F && offset != 1.0F) {
         throw new IllegalArgumentException("offset must be one of {0f, 0.5f, 1f}");
      }
      return (int)(2.0F * offset) + 3 * direction.getId();
   }
