...
public class RespawnAnchorBlock extends Block {
      ...
      if (!isNether(world)) {
         ...
         if (!world.isClient) {
            ...
            if (serverPlayerEntity.getSpawnPointDimension() != world.getRegistryKey() || !pos.equals(serverPlayerEntity.getSpawnPointPosition())) {
               ...
               world.playSound(
                  null, pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5, SoundEvents.BLOCK_RESPAWN_ANCHOR_SET_SPAWN, SoundCategory.BLOCKS, 1.0F, 1.0F
               );
   ...
   public static int getLightLevel(BlockState state, int maxLevel) {
      return MathHelper.floor((state.get(CHARGES) - 0) / 4.0F * maxLevel);
   }
