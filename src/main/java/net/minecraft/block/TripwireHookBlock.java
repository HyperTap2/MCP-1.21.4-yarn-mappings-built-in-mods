...
public class TripwireHookBlock extends Block {
      ...
      if (optional.isPresent()) {
         ...
         for (int k = 1; k < 42; k++) {
            ...
            if (!blockState2.isOf(Blocks.TRIPWIRE) && k != i) {
               ...
               boolean bl7 = !blockState2.get(TripwireBlock.DISARMED);
         ...
         if (j > 0) {
            ...
            Direction direction2 = direction.getOpposite();
            world.setBlockState(blockPos, blockState3.with(FACING, direction2), 3);
            ...
            for (int l = 1; l < j; l++) {
               ...
               if (blockState4 != null) {
                  ...
                  if (blockState5.isOf(Blocks.TRIPWIRE) || blockState5.isOf(Blocks.TRIPWIRE_HOOK)) {
                     world.setBlockState(blockPos2, blockState4.withIfExists(ATTACHED, bl5), 3);
                  }
               }
            }
         }
   ...
   private static void playSound(World world, BlockPos pos, boolean attached, boolean on, boolean detached, boolean off) {
      if (on && !off) {
         ...
         world.playSound(
            (PlayerEntity)null, pos, SoundEvents.BLOCK_TRIPWIRE_DETACH, SoundCategory.BLOCKS, 0.4F, 1.2F / (world.random.nextFloat() * 0.2F + 0.9F)
         );
