...
public class HoneyBlock extends TranslucentBlock {
   ...
   private static double method_65067(double d) {
      return ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_21) ? d : d / 0.98F + 0.08;
   }

   private static double method_65068(double d) {
      return ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_21) ? d : (d - 0.08) * 0.98F;
   }
   ...
   private boolean isSliding(BlockPos pos, Entity entity) {
      ...
      if (method_65067(entity.getVelocity().y) >= -0.08) {
         return false;
      }
   ...
   private static void addParticles(Entity entity, int count) {
      if (entity.getWorld().isClient) {
         ...
         for (int i = 0; i < count; i++) {
            entity.getWorld()
               .addParticle(new BlockStateParticleEffect(ParticleTypes.BLOCK, blockState), entity.getX(), entity.getY(), entity.getZ(), 0.0, 0.0, 0.0);
         }
      }
   }
}
