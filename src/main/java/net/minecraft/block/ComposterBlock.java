...
public class ComposterBlock extends Block implements InventoryProvider {
      ...
      shapes -> {
         for (int i = 0; i < 8; i++) {
            shapes[i] = VoxelShapes.combineAndSimplify(
               RAYCAST_SHAPE, Block.createCuboidShape(2.0, Math.max(2, 1 + i * 2), 2.0, 14.0, 16.0, 14.0), BooleanBiFunction.ONLY_FIRST
            );
         }
   ...
   public SidedInventory getInventory(BlockState state, WorldAccess world, BlockPos pos) {
      ...
      if (i == 8) {
         ...
         return i < 7 ? new ComposterBlock.ComposterInventory(state, world, pos) : new ComposterBlock.DummyInventory();
      }
   }

   static class ComposterInventory extends SimpleInventory implements SidedInventory, net.caffeinemc.mods.lithium.common.hopper.BlockStateOnlyInventory {
   ...
   static class DummyInventory extends SimpleInventory implements SidedInventory, net.caffeinemc.mods.lithium.common.hopper.BlockStateOnlyInventory {
   ...
   static class FullComposterInventory extends SimpleInventory implements SidedInventory, net.caffeinemc.mods.lithium.common.hopper.BlockStateOnlyInventory {
      private final BlockState state;
      private final WorldAccess world;
      private final BlockPos pos;
      private boolean dirty;
