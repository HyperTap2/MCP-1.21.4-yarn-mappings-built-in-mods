   ...
   static void registerDefaults() {
         ...
         public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            ...
            try {
               entityType.spawnFromItemStack(
                  pointer.world(), stack, null, pointer.pos().offset(direction), SpawnReason.DISPENSER, direction != Direction.UP, false
               );
      ...
      DispenserBlock.registerBehavior(Blocks.TNT, new ItemDispenserBehavior() {
         ...
         protected ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
            World world = pointer.world();
            BlockPos blockPos = pointer.pos().offset(pointer.state().get(DispenserBlock.FACING));
            TntEntity tntEntity = new TntEntity(world, blockPos.getX() + 0.5, blockPos.getY(), blockPos.getZ() + 0.5, null);
         ...
         new FallibleItemDispenserBehavior() {
            ...
            protected ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
               ...
               if (world.isAir(blockPos) && WitherSkullBlock.canDispense(world, blockPos, stack)) {
                  world.setBlockState(
                     blockPos, Blocks.WITHER_SKELETON_SKULL.getDefaultState().with(SkullBlock.ROTATION, RotationPropertyHelper.fromDirection(direction)), 3
                  );
                  ...
                  this.setSuccess(EquippableDispenserBehavior.dispenseS(pointer, stack));
               }
      ...
      DispenserBlock.registerBehavior(
         ...
         new ItemDispenserBehavior() {
            ...
            public ItemStack dispenseSilently(BlockPointer pointer, ItemStack stack) {
               ...
               serverWorld.playSound((PlayerEntity)null, blockPos, SoundEvents.ITEM_BOTTLE_EMPTY, SoundCategory.BLOCKS, 1.0F, 1.0F);
