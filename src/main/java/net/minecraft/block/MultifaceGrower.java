...
public class MultifaceGrower {
   ...
   public Optional<MultifaceGrower.GrowPos> grow(BlockState state, WorldAccess world, BlockPos pos, Random random) {
      return Direction.shuffle(random)
         .stream()
   ...
   public Optional<MultifaceGrower.GrowPos> getGrowPos(
      ...
      if (this.growChecker.canGrow(state) || this.growChecker.hasDirection(state, oldDirection) && !this.growChecker.hasDirection(state, newDirection)) {
         for (MultifaceGrower.GrowType growType : this.growChecker.getGrowTypes()) {
            MultifaceGrower.GrowPos growPos = growType.getGrowPos(pos, newDirection, oldDirection);
