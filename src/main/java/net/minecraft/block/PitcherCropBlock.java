...
public class PitcherCropBlock extends TallPlantBlock implements Fertilizable {
   ...
   private static final VoxelShape[] UPPER_OUTLINE_SHAPES = new VoxelShape[]{
      Block.createCuboidShape(3.0, 0.0, 3.0, 13.0, 11.0, 13.0), GROWN_UPPER_OUTLINE_SHAPE
   };
   ...
   public VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
      return state.get(HALF) == DoubleBlockHalf.UPPER
         ? UPPER_OUTLINE_SHAPES[Math.min(Math.abs(4 - (state.get(AGE) + 1)), UPPER_OUTLINE_SHAPES.length - 1)]
   ...
   public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state) {
      ...
      return lowerHalfContext == null ? false : this.canGrow(world, lowerHalfContext.pos, lowerHalfContext.state, lowerHalfContext.state.get(AGE) + 1);
   }
