...
public class DesertTempleGenerator extends ShiftableStructurePiece {
   ...
   public void generate(
      ...
      if (this.adjustToMinHeight(world, -random.nextInt(3))) {
         ...
         this.fillWithOutline(
            world, chunkBox, this.width - 5, 0, 0, this.width - 1, 9, 4, Blocks.SANDSTONE.getDefaultState(), Blocks.AIR.getDefaultState(), false
         );
         this.fillWithOutline(
         ...
         for (Direction direction : Direction.Type.HORIZONTAL) {
            if (!this.hasPlacedChest[direction.getHorizontalQuarterTurns()]) {
               int m = direction.getOffsetX() * 2;
               int n = direction.getOffsetZ() * 2;
               this.hasPlacedChest[direction.getHorizontalQuarterTurns()] = this.addChest(
                  world, chunkBox, random, 10 + m, -11, 10 + n, LootTables.DESERT_PYRAMID_CHEST
               );
            }
         }
