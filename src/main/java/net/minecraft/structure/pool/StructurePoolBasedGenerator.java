...
public class StructurePoolBasedGenerator {
   ...
   public static Optional<Structure.StructurePosition> generate(
      ...
      StructurePool structurePool2 = structurePool.getKey()
         .flatMap(key -> registry.getOptionalValue(aliasLookup.lookup((RegistryKey<StructurePool>)key)))
      ...
      int k = projectStartToHeightmap.isEmpty()
         ? blockPos2.getY()
         : pos.getY() + chunkGenerator.getHeightOnGround(i, j, projectStartToHeightmap.get(), heightLimitView, context.noiseConfig());
      ...
      if (method_65173(heightLimitView, dimensionPadding, poolStructurePiece.getBoundingBox())) {
         LOGGER.debug(
            ...
            new Object[]{structurePoolElement, poolStructurePiece.getBoundingBox(), dimensionPadding}
         );
         return Optional.empty();
      } else {
         int m = k + vec3i.getY();
                        ...
                        for (BlockRotation blockRotation2 : BlockRotation.randomRotationOrder(this.random)) {
                           ...
                           if (modifyBoundingBox && blockBox2.getBlockCountY() <= 16) {
                              m = list2.stream().mapToInt(jigsawInfo -> {
                                 ...
                                 Optional<RegistryEntry<StructurePool>> optional2 = optionalx.map(entry -> entry.value().getFallback());
                                 int ix = optionalx.<Integer>map(entry -> entry.value().getHighestY(this.structureTemplateManager)).orElse(0);
                                 int jx = optional2.<Integer>map(entry -> entry.value().getHighestY(this.structureTemplateManager)).orElse(0);
                              ...
                              if (JigsawBlock.attachmentMatches(jigsawBlockInfo, jigsawBlockInfo2)) {
                                 ...
                                 if (bl && bl3) {
                                    ...
                                    if (k == Integer.MIN_VALUE) {
                                       k = this.chunkGenerator
                                          .getHeightOnGround(blockPos2.getX(), blockPos2.getZ(), Heightmap.Type.WORLD_SURFACE_WG, world, noiseConfig);
                                    }
                                 ...
                                 if (!VoxelShapes.matchesAnywhere(
                                    ...
                                    mutableObject2.setValue(
                                       VoxelShapes.combine(
                                          (VoxelShape)mutableObject2.getValue(), VoxelShapes.cuboid(Box.from(blockBox4)), BooleanBiFunction.ONLY_FIRST
                                       )
                                    );
