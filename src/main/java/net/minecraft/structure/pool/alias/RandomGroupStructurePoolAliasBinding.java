...
record RandomGroupStructurePoolAliasBinding(DataPool<List<StructurePoolAliasBinding>> groups) implements StructurePoolAliasBinding {
   static MapCodec<RandomGroupStructurePoolAliasBinding> CODEC = RecordCodecBuilder.mapCodec(
      instance -> instance.group(
            DataPool.createCodecDataPool(Codec.list(StructurePoolAliasBinding.CODEC)).fieldOf("groups").forGetter(RandomGroupStructurePoolAliasBinding::groups)
         )
   ...
   public void forEach(Random random, BiConsumer<RegistryKey<StructurePool>, RegistryKey<StructurePool>> aliasConsumer) {
      this.groups.getOrEmpty(random).ifPresent(pool -> pool.data().forEach(binding -> binding.forEach(random, aliasConsumer)));
   }

   @Override
   public Stream<RegistryKey<StructurePool>> streamTargets() {
      return this.groups.getEntries().stream().flatMap(present -> present.data().stream()).flatMap(StructurePoolAliasBinding::streamTargets);
   }
