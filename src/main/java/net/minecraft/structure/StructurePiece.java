...
public abstract class StructurePiece {
   ...
   public StructurePiece(StructurePieceType type, NbtCompound nbt) {
      this(
         type,
         nbt.getInt("GD"),
         (BlockBox)BlockBox.CODEC.parse(NbtOps.INSTANCE, nbt.get("BB")).getOrThrow(error -> new IllegalArgumentException("Invalid boundingbox: " + error))
      );
      int i = nbt.getInt("O");
   ...
   protected void addBlockWithRandomThreshold(
      StructureWorldAccess world, BlockBox bounds, Random random, float threshold, int x, int y, int z, BlockState state
   ) {
