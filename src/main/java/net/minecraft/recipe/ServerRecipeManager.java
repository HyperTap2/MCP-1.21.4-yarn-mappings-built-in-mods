...
public class ServerRecipeManager extends SinglePreparationResourceReloader<PreparedRecipes> implements RecipeManager {
   ...
   private static final Map<RegistryKey<RecipePropertySet>, ServerRecipeManager.SoleIngredientGetter> SOLE_INGREDIENT_GETTERS = Map.of(
      ...
      recipe -> recipe instanceof SmithingRecipe smithingRecipe ? smithingRecipe.addition() : Optional.empty(),
      RecipePropertySet.SMITHING_BASE,
      recipe -> recipe instanceof SmithingRecipe smithingRecipe ? smithingRecipe.base() : Optional.empty(),
      RecipePropertySet.SMITHING_TEMPLATE,
      recipe -> recipe instanceof SmithingRecipe smithingRecipe ? smithingRecipe.template() : Optional.empty(),
   ...
   public void initialize(FeatureSet features) {
      ...
      List<ServerRecipeManager.PropertySetBuilder> list2 = SOLE_INGREDIENT_GETTERS.entrySet()
         .stream()
         .map(entry -> new ServerRecipeManager.PropertySetBuilder(entry.getKey(), entry.getValue()))
         .toList();
                     ...
                     if (isEnabled(features, stonecuttingRecipe.ingredient()) && stonecuttingRecipe.createResultDisplay().isEnabled(features)) {
                        list.add(
                           new CuttingRecipeDisplay.GroupEntry<>(
                              stonecuttingRecipe.ingredient(), new CuttingRecipeDisplay<>(stonecuttingRecipe.createResultDisplay(), Optional.of(recipeEntry))
                           )
                        );
                     }
                  }
   ...
   public ServerRecipeManager.ServerRecipe get(NetworkRecipeId id) {
      return this.recipes.get(id.index());
   }
