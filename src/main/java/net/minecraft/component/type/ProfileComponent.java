...
public record ProfileComponent(Optional<String> name, Optional<UUID> id, PropertyMap properties, GameProfile gameProfile) {
   ...
   public CompletableFuture<ProfileComponent> getFuture() {
      if (this.isCompleted()) {
         ...
         return this.id.isPresent() ? SkullBlockEntity.fetchProfileByUuid(this.id.get()).thenApply(optional -> {
         ...
         }) : SkullBlockEntity.fetchProfileByName(this.name.orElseThrow()).thenApply(profile -> {
