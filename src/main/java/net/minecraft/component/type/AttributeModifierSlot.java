...
public enum AttributeModifierSlot implements StringIdentifiable {
   ...
   public static final IntFunction<AttributeModifierSlot> ID_TO_VALUE = ValueLists.<AttributeModifierSlot>createIdToValueFunction(
   ...
   AttributeModifierSlot(final int id, final String name, final Predicate<EquipmentSlot> slotPredicate) {
      this.id = id;
      this.name = name;
      this.slotPredicate = slotPredicate;
   }

   AttributeModifierSlot(final int id, final String name, final EquipmentSlot slot) {
