...
public record FireworkExplosionComponent(FireworkExplosionComponent.Type shape, IntList colors, IntList fadeColors, boolean hasTrail, boolean hasTwinkle)
      ...
      if (!this.fadeColors.isEmpty()) {
         textConsumer.accept(
            appendColorsTooltipText(
               Text.translatable("item.minecraft.firework_star.fade_to").append(ScreenTexts.SPACE).formatted(Formatting.GRAY), this.fadeColors
            )
         );
      }
   ...
   private static Text getColorText(int color) {
      ...
      return dyeColor == null ? CUSTOM_COLOR_TEXT : Text.translatable("item.minecraft.firework_star." + dyeColor.getName());
   }
   ...
   public enum Type implements StringIdentifiable {
      ...
      public static final PacketCodec<ByteBuf, FireworkExplosionComponent.Type> PACKET_CODEC = PacketCodecs.indexed(
         BY_ID, FireworkExplosionComponent.Type::getId
      );
