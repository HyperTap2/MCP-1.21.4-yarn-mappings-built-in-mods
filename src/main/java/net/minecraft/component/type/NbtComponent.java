...
public final class NbtComponent {
   ...
   public static final Codec<NbtComponent> CODEC_WITH_ID = CODEC.validate(
      component -> component.getNbt().contains("id", 8) ? DataResult.success(component) : DataResult.error(() -> "Missing id for entity in: " + component)
   );
   @Deprecated
   ...
   public Identifier getId() {
      return !this.nbt.contains("id", 8) ? null : Identifier.tryParse(this.nbt.getString("id"));
   }
   ...
   public <T> DataResult<T> get(DynamicOps<NbtElement> ops, MapDecoder<T> decoder) {
      MapLike<NbtElement> mapLike = (MapLike<NbtElement>)ops.getMap(this.nbt).getOrThrow();
