...
public class MerchantScreenHandler extends ScreenHandler {
   ...
   public void switchTo(int recipeIndex) {
      if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_13_2)) {
         this.viaFabricPlus$switchToLegacy(recipeIndex);
         return;
      }
   ...
   private void viaFabricPlus$switchToLegacy(int recipeIndex) {
      if (recipeIndex < 0 || recipeIndex >= this.getRecipes().size()) {
         return;
      }
      ...
      if (!this.merchantInventory.getStack(0).isEmpty()) {
         int count = this.merchantInventory.getStack(0).getCount();
         interactionManager.clickSlot(this.syncId, 0, 0, SlotActionType.QUICK_MOVE, player);
      ...
      if (!this.merchantInventory.getStack(1).isEmpty()) {
         int count = this.merchantInventory.getStack(1).getCount();
         interactionManager.clickSlot(this.syncId, 1, 0, SlotActionType.QUICK_MOVE, player);
      ...
      if (this.merchantInventory.getStack(0).isEmpty() && this.merchantInventory.getStack(1).isEmpty()) {
         TradeOffer tradeOffer = this.getRecipes().get(recipeIndex);
         this.viaFabricPlus$autofillLegacy(interactionManager, player, 0, tradeOffer.getFirstBuyItem());
         tradeOffer.getSecondBuyItem().ifPresent(item -> this.viaFabricPlus$autofillLegacy(interactionManager, player, 1, item));
      }
   }

   private void viaFabricPlus$autofillLegacy(
      ClientPlayerInteractionManager interactionManager, ClientPlayerEntity player, int inputSlot, TradedItem stackNeeded
   ) {
      int slot;
      for (slot = 3; slot < 39; slot++) {
         ...
         if (!itemStack.isEmpty() && stackNeeded.matches(itemStack)) {
            ItemStack inputStack = this.merchantInventory.getStack(inputSlot);
            if (inputStack.isEmpty() || ItemStack.areItemsAndComponentsEqual(itemStack, inputStack)) {
               break;
            }
         }
      }
      ...
      if (slot != 39) {
         ...
         interactionManager.clickSlot(this.syncId, slot, 0, SlotActionType.PICKUP, player);
         interactionManager.clickSlot(this.syncId, slot, 0, SlotActionType.PICKUP_ALL, player);
         interactionManager.clickSlot(this.syncId, inputSlot, 0, SlotActionType.PICKUP, player);
         if (wasHoldingItem) {
            interactionManager.clickSlot(this.syncId, slot, 0, SlotActionType.PICKUP, player);
         }
      }
