...
public abstract class ChunkTicketManager {
   ...
   protected ChunkTicketManager(Executor workerExecutor, Executor mainThreadExecutor) {
      ...
      this.scheduler = new ThrottledChunkTaskScheduler(taskExecutor, workerExecutor, 4);
   ...
   private SortedArraySet<ChunkTicket<?>> getTicketSet(long position) {
      return (SortedArraySet<ChunkTicket<?>>)this.ticketsByPosition.computeIfAbsent(position, pos -> SortedArraySet.create(4));
   }
   ...
   public void handleChunkEnter(ChunkSectionPos pos, ServerPlayerEntity player) {
      ...
      ((ObjectSet)this.playersByChunkPos.computeIfAbsent(l, sectionPos -> new ObjectOpenHashSet())).add(player);
   ...
   private void dump(String path) {
      try (FileOutputStream fileOutputStream = new FileOutputStream(path)) {
         ...
         LOGGER.error("Failed to dump tickets to {}", path, iOException);
      }
   }
   ...
   public void removePersistentTickets() {
      ImmutableSet<ChunkTicketType<?>> immutableSet = ImmutableSet.of(ChunkTicketType.UNKNOWN);
      ObjectIterator<Entry<SortedArraySet<ChunkTicket<?>>>> objectIterator = this.ticketsByPosition.long2ObjectEntrySet().fastIterator();
   ...
   class DistanceFromNearestPlayerTracker extends ChunkPosDistanceLevelPropagator {
      ...
      private void dump(String path) {
         try (FileOutputStream fileOutputStream = new FileOutputStream(new File(path))) {
            ...
            ChunkTicketManager.LOGGER.error("Failed to dump chunks to {}", path, iOException);
         }
      }
   }
      ...
      private void updateTicket(long pos, int distance, boolean oldWithinViewDistance, boolean withinViewDistance) {
         if (oldWithinViewDistance != withinViewDistance) {
            ...
            if (withinViewDistance) {
               ChunkTicketManager.this.scheduler.add(() -> ChunkTicketManager.this.mainThreadExecutor.execute(() -> {
                  if (this.isWithinViewDistance(this.getLevel(pos))) {
                     ...
                     ChunkTicketManager.this.scheduler.remove(pos, () -> {}, false);
                  }
               }), pos, () -> distance);
            } else {
               ChunkTicketManager.this.scheduler
                  .remove(pos, () -> ChunkTicketManager.this.mainThreadExecutor.execute(() -> ChunkTicketManager.this.removeTicket(pos, chunkTicket)), true);
            }
         }
      }
      ...
      public void updateLevels() {
         ...
         if (!this.positionsAffected.isEmpty()) {
            ...
            while (longIterator.hasNext()) {
               ...
               if (i != j) {
                  ChunkTicketManager.this.scheduler.updateLevel(new ChunkPos(l), () -> this.distances.get(l), j, level -> {
