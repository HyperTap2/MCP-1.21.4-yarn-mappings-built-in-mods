...
public class ChunkHolder extends AbstractChunkHolder implements ChunkHolderExtended {
   ...
   private long lithium$lastRequestTime;

   @Override
   public boolean lithium$updateLastAccessTime(long time) {
      long previous = this.lithium$lastRequestTime;
      this.lithium$lastRequestTime = time;
   ...
   public void combinePostProcessingFuture(CompletableFuture<?> postProcessingFuture) {
      if (this.postProcessingFuture.isDone()) {
         ...
         this.postProcessingFuture = this.postProcessingFuture.thenCombine((CompletionStage<? extends Object>)postProcessingFuture, (object, object2) -> null);
      }
   }
   ...
   protected void combineSavingFuture(CompletableFuture<?> savingFuture) {
      if (this.savingFuture.isDone()) {
         ...
         this.savingFuture = this.savingFuture.thenCombine((CompletionStage<? extends Object>)savingFuture, (object, thenResult) -> null);
      }
   }
   ...
   protected void updateFutures(ServerChunkLoadingManager chunkLoadingManager, Executor executor) {
      ...
      if (bl && !bl2) {
         ChunkStatusTracker.onChunkInaccessible(chunkLoadingManager.world, this.pos);
