...
public class ServerChunkLoadingManager extends VersionedChunkStorage implements ChunkHolder.PlayersWatchingChunkProvider, ChunkLoadingManager {
   ...
   public ServerChunkLoadingManager(
      ...
      if (chunkGenerator instanceof NoiseChunkGenerator noiseChunkGenerator) {
         ...
         this.noiseConfig = NoiseConfig.create(
            ChunkGeneratorSettings.createMissingSettings(), dynamicRegistryManager.getOrThrow(RegistryKeys.NOISE_PARAMETERS), l
         );
      }
   ...
   private CompletableFuture<Chunk> loadChunk(ChunkPos pos) {
      ...
      return completableFuture.<Object, Optional<SerializedChunk>>thenCombine(
         ...
         .thenApplyAsync(nbt -> {
            ...
            if (nbt.isPresent()) {
               Chunk chunk = nbt.get().convert(this.world, this.pointOfInterestStorage, this.getStorageKey(), pos);
         ...
         .exceptionallyAsync(throwable -> this.recoverFromException(throwable, pos), this.mainThreadExecutor);
   }
   ...
   private boolean isLevelChunk(ChunkPos pos) {
      ...
      try {
         ...
         LOGGER.error("Failed to read chunk {}", pos, exception);
         this.markAsProtoChunk(pos);
         return false;
      }
   ...
   void forEachTickedChunk(Consumer<ChunkHolder> callback) {
      ...
      while (longIterator.hasNext()) {
         long l = longIterator.nextLong();
         ChunkHolder chunkHolder = (ChunkHolder)this.chunkHolders.get(l);
   ...
   public void updatePosition(ServerPlayerEntity player) {
      ...
      while (chunkSectionPos.hasNext()) {
         ServerChunkLoadingManager.EntityTracker entityTracker = (ServerChunkLoadingManager.EntityTracker)chunkSectionPos.next();
      ...
      if (bl3 || bl != bl2) {
         this.updateWatchedSection(player);
         if (!bl) {
            this.ticketManager.handleChunkLeave(chunkSectionPosx, player);
         }
      ...
      if (!(entity instanceof EnderDragonPart)) {
         ...
         if (i != 0) {
            ...
            if (entity instanceof ServerPlayerEntity serverPlayerEntity) {
               ...
               while (var7.hasNext()) {
                  ServerChunkLoadingManager.EntityTracker entityTracker2 = (ServerChunkLoadingManager.EntityTracker)var7.next();
   ...
   protected void unloadEntity(Entity entity) {
      if (entity instanceof ServerPlayerEntity serverPlayerEntity) {
         ...
         while (var3.hasNext()) {
            ServerChunkLoadingManager.EntityTracker entityTracker = (ServerChunkLoadingManager.EntityTracker)var3.next();
            entityTracker.stopTracking(serverPlayerEntity);
         }
      }

      ServerChunkLoadingManager.EntityTracker entityTracker2 = (ServerChunkLoadingManager.EntityTracker)this.entityTrackers.remove(entity.getId());
   ...
   public void sendToOtherNearbyPlayers(Entity entity, Packet<?> packet) {
      ServerChunkLoadingManager.EntityTracker entityTracker = (ServerChunkLoadingManager.EntityTracker)this.entityTrackers.get(entity.getId());
   ...
   class EntityTracker {
      ...
      private final Set<PlayerAssociatedNetworkHandler> listeners = new ReferenceOpenHashSet<>();
