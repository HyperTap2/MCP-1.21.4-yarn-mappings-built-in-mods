...
public record ServerLinks(List<ServerLinks.Entry> entries) {
   ...
   public enum Known {
      ...
      private static final IntFunction<ServerLinks.Known> FROM_ID = ValueLists.<ServerLinks.Known>createIdToValueFunction(
      ...
      Known(final int id, final String name) {
         this.id = id;
         this.name = name;
      }
   ...
   public record StringifiedEntry(Either<ServerLinks.Known, Text> type, String link) {
