...
public final class ServerMBean implements DynamicMBean {
   ...
   private ServerMBean(MinecraftServer server) {
      this.server = server;
      MBeanAttributeInfo[] mBeanAttributeInfos = this.entries.values().stream().map(ServerMBean.Entry::createInfo).toArray(MBeanAttributeInfo[]::new);
   ...
   public static void register(MinecraftServer server) {
      ...
      } catch (MalformedObjectNameException | InstanceAlreadyExistsException | MBeanRegistrationException | NotCompliantMBeanException jMException) {
