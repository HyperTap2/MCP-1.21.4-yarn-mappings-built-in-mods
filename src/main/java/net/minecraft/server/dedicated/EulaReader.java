   ...
   private void createEulaFile() {
      if (!SharedConstants.isDevelopment) {
         try (OutputStream outputStream = Files.newOutputStream(this.eulaFile)) {
            ...
            properties.store(outputStream, "By changing the setting below to TRUE you are indicating your agreement to our EULA (" + Urls.EULA + ").");
         } catch (Exception exception) {
            LOGGER.warn("Failed to save {}", this.eulaFile, exception);
         }
      }
   }
}
