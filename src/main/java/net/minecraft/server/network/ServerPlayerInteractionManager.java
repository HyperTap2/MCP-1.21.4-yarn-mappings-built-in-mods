...
public class ServerPlayerInteractionManager {
      ...
      if (!this.player.canInteractWithBlockAt(pos, 1.0)) {
         ...
         if (action == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK) {
            ...
            if (pos.equals(this.miningPos)) {
               ...
               BlockState blockState = this.world.getBlockState(pos);
               if (!blockState.isAir()) {
                  float g = blockState.calcBlockBreakingDelta(this.player, this.player.getWorld(), pos) * (j + 1);
                  if (g >= 0.7F) {
                     this.mining = false;
   ...
   public ActionResult interactItem(ServerPlayerEntity player, World world, ItemStack stack, Hand hand) {
      ...
      if (actionResult instanceof ActionResult.Fail && itemStack.getMaxUseTime(player) > 0 && !player.isUsingItem()) {
         return actionResult;
      }
