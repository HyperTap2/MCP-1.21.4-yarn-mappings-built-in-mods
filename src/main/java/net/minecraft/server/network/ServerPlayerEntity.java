...
public class ServerPlayerEntity extends PlayerEntity {
      ...
      if (world.getDimension().hasSkyLight() && world.getServer().getSaveProperties().getGameMode() != GameMode.ADVENTURE) {
         ...
         for (int p = 0; p < k; p++) {
            ...
            try {
               ...
               int v = p;
               int w = i;
               CrashReport crashReport = CrashReport.create(exception, "Searching for spawn");
               CrashReportSection crashReportSection = crashReport.addElement("Spawn Lookup");
               crashReportSection.add("Origin", basePos::toString);
               crashReportSection.add("Radius", () -> Integer.toString(w));
               crashReportSection.add("Candidate", () -> "[" + t + "," + u + "]");
               crashReportSection.add("Progress", () -> v + " out of " + k);
   ...
   public void writeCustomDataToNbt(NbtCompound nbt) {
      ...
      if (this.startRaidPos != null) {
         BlockPos.CODEC
            .encodeStart(NbtOps.INSTANCE, this.startRaidPos)
            .resultOrPartial(LOGGER::error)
   ...
   public void readRootVehicle(Optional<NbtCompound> nbt) {
      if (nbt.isPresent() && nbt.get().contains("RootVehicle", 10) && this.getWorld() instanceof ServerWorld serverWorld) {
         NbtCompound nbtCompound = nbt.get().getCompound("RootVehicle");
   ...
   public void readEnderPearls(Optional<NbtCompound> nbt) {
      if (nbt.isPresent() && nbt.get().contains("ender_pearls", 9) && nbt.get().get("ender_pearls") instanceof NbtList nbtList) {
         nbtList.forEach(
            enderPearlNbt -> {
               if (enderPearlNbt instanceof NbtCompound nbtCompound && nbtCompound.contains("ender_pearl_dimension")) {
                  Optional<RegistryKey<World>> optional = World.CODEC
                     .parse(NbtOps.INSTANCE, nbtCompound.get("ender_pearl_dimension"))
                     .resultOrPartial(LOGGER::error);
                  if (optional.isEmpty()) {
   ...
   private static Optional<ServerPlayerEntity.RespawnPos> findRespawnPosition(
      ...
      if (block instanceof RespawnAnchorBlock && (spawnForced || blockState.get(RespawnAnchorBlock.CHARGES) > 0) && RespawnAnchorBlock.isNether(world)) {
         ...
         if (!spawnForced && alive && optional.isPresent()) {
            world.setBlockState(pos, blockState.with(RespawnAnchorBlock.CHARGES, blockState.get(RespawnAnchorBlock.CHARGES) - 1), 3);
         }
         ...
         return bl && bl2
            ? Optional.of(new ServerPlayerEntity.RespawnPos(new Vec3d(pos.getX() + 0.5, pos.getY() + 0.1, pos.getZ() + 0.5), spawnAngle))
            : Optional.empty();
      }
   }
   ...
   public void unlockRecipes(List<RegistryKey<Recipe<?>>> recipes) {
      List<RecipeEntry<?>> list = recipes.stream()
         .flatMap(recipeKey -> this.server.getRecipeManager().get((RegistryKey<Recipe<?>>)recipeKey).stream())
   ...
   public void requestTeleport(double destX, double destY, double destZ) {
      this.networkHandler
         .requestTeleport(
            new PlayerPosition(new Vec3d(destX, destY, destZ), Vec3d.ZERO, 0.0F, 0.0F), PositionFlag.combine(PositionFlag.DELTA, PositionFlag.ROT)
         );
   }
   ...
   public SyncedClientOptions getClientOptions() {
      ...
      Arm arm = Arm.BY_ID.apply(this.getDataTracker().get(MAIN_ARM));
   ...
   public void setCameraEntity(@Nullable Entity entity) {
      ...
      if (entity2 != this.cameraEntity) {
         if (this.cameraEntity.getWorld() instanceof ServerWorld serverWorld) {
            this.teleport(
               serverWorld, this.cameraEntity.getX(), this.cameraEntity.getY(), this.cameraEntity.getZ(), Set.of(), this.getYaw(), this.getPitch(), false
            );
         }
      ...
      this.networkHandler
         .sendPacket(
            new PlaySoundS2CPacket(
               Registries.SOUND_EVENT.getEntry(sound), category, this.getX(), this.getY(), this.getZ(), volume, pitch, this.random.nextLong()
            )
         );
   }
