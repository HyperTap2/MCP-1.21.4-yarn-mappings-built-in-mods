      ...
      try {
         ...
         if (session.levelDatExists()) {
            ...
            try {
               ...
               LOGGER.warn("Failed to load world data from {}", levelSave.getLevelDatPath(), exception);
               LOGGER.info("Attempting to use fallback");

               try {
                  dynamic = session.readOldLevelProperties();
                  levelSummary = session.getLevelSummary(dynamic);
               } catch (IOException | NbtException | NbtCrashException exception2) {
                  LOGGER.error("Failed to load world data from {}", levelSave.getLevelDatOldPath(), exception2);
                  LOGGER.error(
         ...
         try {
            ...
            LOGGER.warn(
               "Failed to load datapacks, can't proceed with server load. You can either fix your datapacks or reset to vanilla with --safeMode", exception3
            );
            return;
         }
      ...
      try (WorldUpdater worldUpdater = new WorldUpdater(session, dataFixer, dynamicRegistryManager, eraseCache, recreateRegionFiles)) {
         ...
         while (!worldUpdater.isDone()) {
            ...
            if (i > 0) {
               ...
               LOGGER.info("{}% completed ({} / {} chunks)...", new Object[]{MathHelper.floor((float)j / i * 100.0F), j, i});
            }
