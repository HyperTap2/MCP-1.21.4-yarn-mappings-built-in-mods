...
public class TagCommand {
                     ...
                     .then(
                        CommandManager.literal("remove")
                           .then(
                              CommandManager.argument("name", StringArgumentType.word())
                                 .suggests(
                                    (context, builder) -> CommandSource.suggestMatching(getTags(EntityArgumentType.getEntities(context, "targets")), builder)
                                 )
                                 .executes(
   ...
   private static int executeAdd(ServerCommandSource source, Collection<? extends Entity> targets, String tag) throws CommandSyntaxException {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> Text.translatable("commands.tag.add.success.single", tag, targets.iterator().next().getDisplayName()), true);
      } else {
   ...
   private static int executeRemove(ServerCommandSource source, Collection<? extends Entity> targets, String tag) throws CommandSyntaxException {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> Text.translatable("commands.tag.remove.success.single", tag, targets.iterator().next().getDisplayName()), true);
      } else {
   ...
   private static int executeList(ServerCommandSource source, Collection<? extends Entity> targets) {
      ...
      if (targets.size() == 1) {
         ...
         if (set.isEmpty()) {
            ...
            source.sendFeedback(
               () -> Text.translatable("commands.tag.list.single.success", entity2.getDisplayName(), set.size(), Texts.joinOrdered(set)), false
            );
         }
      } else if (set.isEmpty()) {
