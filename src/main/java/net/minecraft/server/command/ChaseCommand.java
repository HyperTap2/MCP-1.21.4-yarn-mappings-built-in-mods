...
public class ChaseCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("chase")
                  .then(
                     ((LiteralArgumentBuilder)CommandManager.literal("follow")
                           .then(
                              ((RequiredArgumentBuilder)CommandManager.argument("host", StringArgumentType.string())
                                    .executes(
                                       context -> startClient((ServerCommandSource)context.getSource(), StringArgumentType.getString(context, "host"), 10000)
                                    ))
                                 .then(
                        ...
                        .executes(context -> startClient((ServerCommandSource)context.getSource(), "localhost", 10000))
                  ))
               .then(
                  ((LiteralArgumentBuilder)CommandManager.literal("lead")
                        .then(
                           ((RequiredArgumentBuilder)CommandManager.argument("bind_address", StringArgumentType.string())
                                 .executes(
                                    context -> startServer(
                                       (ServerCommandSource)context.getSource(), StringArgumentType.getString(context, "bind_address"), 10000
                                    )
                                 ))
                              .then(
                     ...
                     .executes(context -> startServer((ServerCommandSource)context.getSource(), "0.0.0.0", 10000))
               ))
            .then(CommandManager.literal("stop").executes(context -> stop((ServerCommandSource)context.getSource())))
      );
   }
   ...
   private static int startServer(ServerCommandSource source, String ip, int port) {
      ...
      try {
         server.start();
         source.sendFeedback(
            () -> Text.literal("Chase server is now running on port " + port + ". Clients can follow you using /chase follow <ip> <port>"), false
         );
