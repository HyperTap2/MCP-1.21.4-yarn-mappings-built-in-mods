   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal(
                           ...
                           .then(CommandManager.literal("query").executes(context -> executeQuery((ServerCommandSource)context.getSource()))))
                        .then(
                                 ...
                                 CommandManager.argument("rate", FloatArgumentType.floatArg(1.0F, 10000.0F))
                                    .suggests(
                                       (context, suggestionsBuilder) -> CommandSource.suggestMatching(
                                          new String[]{DEFAULT_TICK_RATE_STRING}, suggestionsBuilder
                                       )
                                    )
                                    .executes(context -> executeRate((ServerCommandSource)context.getSource(), FloatArgumentType.getFloat(context, "rate")))
                              )
                        ))
                     .then(
                        ((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("step")
                                 .executes(context -> executeStep((ServerCommandSource)context.getSource(), 1)))
                              .then(CommandManager.literal("stop").executes(context -> executeStopStep((ServerCommandSource)context.getSource()))))
                           .then(
                              CommandManager.argument("time", TimeArgumentType.time(1))
                                 ...
                                 .executes(context -> executeStep((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger(context, "time")))
                           )
                     ))
                  .then(
                     ((LiteralArgumentBuilder)CommandManager.literal("sprint")
                           .then(CommandManager.literal("stop").executes(context -> executeStopSprint((ServerCommandSource)context.getSource()))))
                        .then(
                           CommandManager.argument("time", TimeArgumentType.time(1))
                              .suggests((context, suggestionsBuilder) -> CommandSource.suggestMatching(new String[]{"60s", "1d", "3d"}, suggestionsBuilder))
                              .executes(context -> executeSprint((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger(context, "time")))
                        )
                  ))
               .then(CommandManager.literal("unfreeze").executes(context -> executeFreeze((ServerCommandSource)context.getSource(), false))))
            .then(CommandManager.literal("freeze").executes(context -> executeFreeze((ServerCommandSource)context.getSource(), true)))
      );
   }
