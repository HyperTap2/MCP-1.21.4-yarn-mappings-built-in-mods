...
public class LocateCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("locate")
               ...
               CommandManager.literal("poi")
                  .then(
                     CommandManager.argument(
                           "poi", RegistryEntryPredicateArgumentType.registryEntryPredicate(registryAccess, RegistryKeys.POINT_OF_INTEREST_TYPE)
                        )
                        .executes(
                           context -> executeLocatePoi(
                              (ServerCommandSource)context.getSource(),
                              RegistryEntryPredicateArgumentType.getRegistryEntryPredicate(context, "poi", RegistryKeys.POINT_OF_INTEREST_TYPE)
                           )
                        )
                  )
            )
   ...
   private static Optional<? extends RegistryEntryList.ListBacked<Structure>> getStructureListForPredicate(
      ...
      return (Optional<? extends RegistryEntryList.ListBacked<Structure>>)predicate.getKey()
         .map(key -> structureRegistry.getOptional(key).map(entry -> RegistryEntryList.of(entry)), structureRegistry::getOptional);
   }
   ...
   private static int executeLocatePoi(ServerCommandSource source, RegistryEntryPredicateArgumentType.EntryPredicate<PointOfInterestType> predicate) throws CommandSyntaxException {
      ...
      if (optional.isEmpty()) {
         ...
         return sendCoordinates(source, predicate, blockPos, optional.get().swap(), "commands.locate.poi.success", false, stopwatch.elapsed());
      }
   }
   ...
   public static int sendCoordinates(
      ...
      String string = (String)predicate.getEntry()
         .map(entry -> predicate.asString(), tag -> predicate.asString() + " (" + ((RegistryEntry)result.getSecond()).getIdAsString() + ")");
   ...
   public static int sendCoordinates(
      ...
      String string = (String)structure.getKey()
         .map(key -> key.getValue().toString(), key -> "#" + key.id() + " (" + ((RegistryEntry)result.getSecond()).getIdAsString() + ")");
      return sendCoordinates(source, currentPos, result, successMessage, includeY, string, timeTaken);
   }
