...
public class EffectCommand {
   ...
   private static int executeGive(
      ...
      if (targets.size() == 1) {
         source.sendFeedback(
            () -> Text.translatable("commands.effect.give.success.single", statusEffect2.getName(), targets.iterator().next().getDisplayName(), j / 20), true
         );
      } else {
   ...
   private static int executeClear(ServerCommandSource source, Collection<? extends Entity> targets, RegistryEntry<StatusEffect> statusEffect) throws CommandSyntaxException {
      ...
      if (targets.size() == 1) {
         source.sendFeedback(
            () -> Text.translatable("commands.effect.clear.specific.success.single", statusEffect2.getName(), targets.iterator().next().getDisplayName()), true
         );
      } else {
