...
public class ScheduleCommand {
   ...
   private static final SuggestionProvider<ServerCommandSource> SUGGESTION_PROVIDER = (context, builder) -> CommandSource.suggestMatching(
      ((ServerCommandSource)context.getSource()).getServer().getSaveProperties().getMainWorldProperties().getScheduledEvents().getEventNames(), builder
   );
                              ...
                              ((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument("time", TimeArgumentType.time())
                                       .executes(
                                          context -> execute(
                                             ...
                                             CommandFunctionArgumentType.getFunctionOrTag(context, "function"),
               ...
               CommandManager.literal("clear")
                  .then(
                     CommandManager.argument("function", StringArgumentType.greedyString())
                        ...
                        .executes(context -> clearEvent((ServerCommandSource)context.getSource(), StringArgumentType.getString(context, "function")))
                  )
            )
      );
   }
   ...
   private static int execute(
      ...
      Optional<CommandFunction<ServerCommandSource>> optional = ((Either)function.getSecond()).left();
