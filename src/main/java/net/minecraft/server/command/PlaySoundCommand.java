...
public class PlaySoundCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      RequiredArgumentBuilder<ServerCommandSource, Identifier> requiredArgumentBuilder = (RequiredArgumentBuilder<ServerCommandSource, Identifier>)CommandManager.argument(
            "sound", IdentifierArgumentType.identifier()
         )
         .suggests(SuggestionProviders.AVAILABLE_SOUNDS)
         .executes(
            context -> execute(
               (ServerCommandSource)context.getSource(),
               toList(((ServerCommandSource)context.getSource()).getPlayer()),
   ...
   private static LiteralArgumentBuilder<ServerCommandSource> makeArgumentsForCategory(SoundCategory category) {
      return (LiteralArgumentBuilder<ServerCommandSource>)((LiteralArgumentBuilder)CommandManager.literal(category.getName())
            .executes(
               context -> execute(
                  (ServerCommandSource)context.getSource(),
                  toList(((ServerCommandSource)context.getSource()).getPlayer()),
   ...
   private static int execute(
      ...
      if (targets.size() == 1) {
         source.sendFeedback(() -> Text.translatable("commands.playsound.success.single", Text.of(sound), targets.iterator().next().getDisplayName()), true);
      } else {
