...
public class ServerPackCommand {
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("serverpack")
                  .requires(source -> source.hasPermissionLevel(2)))
               .then(
                  CommandManager.literal("push")
                     .then(
                        ((RequiredArgumentBuilder)CommandManager.argument("url", StringArgumentType.string())
                              .then(
                           ...
                           .executes(
                              context -> executePush(
                                 (ServerCommandSource)context.getSource(), StringArgumentType.getString(context, "url"), Optional.empty(), Optional.empty()
                              )
                           )
                     )
               ))
               ...
               CommandManager.literal("pop")
                  .then(
                     CommandManager.argument("uuid", UuidArgumentType.uuid())
                        .executes(context -> executePop((ServerCommandSource)context.getSource(), UuidArgumentType.getUuid(context, "uuid")))
                  )
            )
      );
   }
