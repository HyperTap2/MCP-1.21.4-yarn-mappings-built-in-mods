...
public class FunctionCommand {
         ...
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("function").requires(source -> source.hasPermissionLevel(2)))
            .then(
               ((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument("name", CommandFunctionArgumentType.commandFunction())
                        ...
                        .executes(new FunctionCommand.Command() {
                           @Nullable
                           @Override
   ...
   private static <T extends AbstractServerCommandSource<T>> void enqueueFunction(
      ...
      try {
         ...
         throw INSTANTIATION_FAILURE_EXCEPTION.create(id, macroException.getMessageText());
      }
   }
   ...
   abstract static class Command extends ControlFlowAware.Helper<ServerCommandSource> implements ControlFlowAware.Command<ServerCommandSource> {
      ...
      public void executeInner(
         ...
         Collection<CommandFunction<ServerCommandSource>> collection = (Collection<CommandFunction<ServerCommandSource>>)pair.getSecond();
         ...
         if (collection.size() == 1) {
            serverCommandSource.sendFeedback(() -> Text.translatable("commands.function.scheduled.single", Text.of(collection.iterator().next().id())), true);
         } else {
            serverCommandSource.sendFeedback(
               () -> Text.translatable("commands.function.scheduled.multiple", Texts.join(collection.stream().map(CommandFunction::id).toList(), Text::of)),
               true
            );
         }
