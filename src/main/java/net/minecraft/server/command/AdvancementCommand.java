...
public class AdvancementCommand {
                                          ...
                                          CommandManager.literal("only")
                                             .then(
                                                ((RequiredArgumentBuilder)CommandManager.argument(
                                                         "advancement", RegistryKeyArgumentType.registryKey(RegistryKeys.ADVANCEMENT)
                                                      )
                                                      .executes(
                                                      ...
                                                      CommandManager.argument("criterion", StringArgumentType.greedyString())
                                                         .suggests(
                                                            (context, builder) -> CommandSource.suggestMatching(
                                                               RegistryKeyArgumentType.getAdvancementEntry(context, "advancement").value().criteria().keySet(),
                                                               builder
                                                            )
                                                         )
                                                         .executes(
      ...
      if (i == 0) {
            ...
            if (targets.size() == 1) {
               throw GENERIC_EXCEPTION.create(
                  Text.translatable(
                     ...
                     Advancement.getNameFromIdentity(selection.iterator().next()),
               ...
               throw GENERIC_EXCEPTION.create(
                  Text.translatable(
                     operation.getCommandPrefix() + ".one.to.many.failure", Advancement.getNameFromIdentity(selection.iterator().next()), targets.size()
                  )
               );
            }
         } else if (targets.size() == 1) {
            throw GENERIC_EXCEPTION.create(
               Text.translatable(operation.getCommandPrefix() + ".many.to.one.failure", selection.size(), targets.iterator().next().getDisplayName())
            );
         } else {
         ...
         if (selection.size() == 1) {
            if (targets.size() == 1) {
               ...
               source.sendFeedback(
                  () -> Text.translatable(
                     operation.getCommandPrefix() + ".one.to.many.success", Advancement.getNameFromIdentity(selection.iterator().next()), targets.size()
                  ),
                  true
               );
            }
            ...
            source.sendFeedback(
               () -> Text.translatable(operation.getCommandPrefix() + ".many.to.one.success", selection.size(), targets.iterator().next().getDisplayName()),
               true
            );
         } else {
   ...
   private static int executeCriterion(
      ...
      if (i == 0) {
         if (targets.size() == 1) {
            ...
            throw GENERIC_EXCEPTION.create(
               Text.translatable(
                  operation.getCommandPrefix() + ".criterion.to.many.failure", criterion, Advancement.getNameFromIdentity(advancement), targets.size()
               )
            );
         }
      } else {
   ...
   private static List<AdvancementEntry> select(
      ...
      if (selection.before) {
         for (PlacedAdvancement placedAdvancement2 = placedAdvancement.getParent();
            placedAdvancement2 != null;
            placedAdvancement2 = placedAdvancement2.getParent()
         ) {
