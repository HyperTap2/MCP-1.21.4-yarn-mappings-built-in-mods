...
public class CommandManager {
   ...
   public static <S> ParseResults<S> withCommandSource(ParseResults<S> parseResults, UnaryOperator<S> sourceMapper) {
      ...
      CommandContextBuilder<S> commandContextBuilder2 = commandContextBuilder.withSource(sourceMapper.apply((S)commandContextBuilder.getSource()));
   ...
   public void execute(ParseResults<ServerCommandSource> parseResults, String command) {
      ...
      try {
         if (contextChain != null) {
            callWithContext(
               serverCommandSource,
               context -> CommandExecutionContext.enqueueCommand(context, command, contextChain, serverCommandSource, ReturnValueConsumer.EMPTY)
            );
         }
      ...
      try {
         ...
         if (commandSyntaxException.getInput() != null && commandSyntaxException.getCursor() >= 0) {
            ...
            if (i < commandSyntaxException.getInput().length()) {
               Text text = Text.literal(commandSyntaxException.getInput().substring(i)).formatted(Formatting.RED, Formatting.UNDERLINE);
   ...
   public static void checkMissing() {
      ...
      commandDispatcher.findAmbiguities(
         (parent, child, sibling, inputs) -> LOGGER.warn(
            "Ambiguity between arguments {} and {} with inputs: {}", new Object[]{commandDispatcher.getPath(child), commandDispatcher.getPath(sibling), inputs}
         )
      );
      Set<ArgumentType<?>> set = ArgumentHelper.collectUsedArgumentTypes(rootCommandNode);
      Set<ArgumentType<?>> set2 = set.stream().filter(type -> !ArgumentTypes.has(type.getClass())).collect(Collectors.toSet());
      if (!set2.isEmpty()) {
   ...
   public enum RegistrationEnvironment {
      ...
      RegistrationEnvironment(final boolean integrated, final boolean dedicated) {
