...
public class LootCommand {
                  ...
                  CommandManager.literal("mine")
                     .then(
                        ((RequiredArgumentBuilder)((RequiredArgumentBuilder)((RequiredArgumentBuilder)CommandManager.argument(
                                    ...
                                    .executes(
                                       context -> executeMine(context, BlockPosArgumentType.getLoadedBlockPos(context, "pos"), ItemStack.EMPTY, constructor)
                                    ))
                                 .then(
                                    CommandManager.argument("tool", ItemStackArgumentType.itemStack(commandRegistryAccess))
                                       .executes(
   ...
   private static void sendDroppedFeedback(ServerCommandSource source, List<ItemStack> stacks, RegistryKey<LootTable> lootTable) {
      if (stacks.size() == 1) {
         ItemStack itemStack = stacks.get(0);
         source.sendFeedback(
            () -> Text.translatable("commands.drop.success.single_with_table", itemStack.getCount(), itemStack.toHoverableText(), Text.of(lootTable.getValue())),
            false
         );
      } else {
   ...
   private static int executeMine(CommandContext<ServerCommandSource> context, BlockPos pos, ItemStack stack, LootCommand.Target constructor) throws CommandSyntaxException {
      ...
      return constructor.accept(context, list, stacks -> sendDroppedFeedback(serverCommandSource, stacks, optional.get()));
   }
