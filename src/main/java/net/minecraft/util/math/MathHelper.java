...
public class MathHelper {
   ...
   private static float[] SINE_TABLE = Util.make(new float[65536], sineTable -> {
      for (int ix = 0; ix < sineTable.length; ix++) {
         sineTable[ix] = (float)Math.sin(ix * Math.PI * 2.0 / 65536.0);
      }
   });
   private static final int[] LITHIUM_SINE_TABLE = createCompactSineTable();
   private static final float LITHIUM_SINE_MIDPOINT = SINE_TABLE[32768];
   static {
      for (int i = 0; i < 65536; i++) {
         float expected = SINE_TABLE[i];
         float actual = lithium$lookupSine(i);
         if (expected != actual) {
            throw new IllegalStateException("Compact sine table mismatch at " + i);
         }
      }
      SINE_TABLE = null;
   }
   ...
   public static float sin(float value) {
      return lithium$lookupSine((int)(value * 10430.378F) & 65535);
   }

   public static float cos(float value) {
      return lithium$lookupSine((int)(value * 10430.378F + 16384.0F) & 65535);
   }

   private static int[] createCompactSineTable() {
      int[] compact = new int[16385];
      for (int i = 0; i < compact.length; i++) {
         compact[i] = Float.floatToRawIntBits(SINE_TABLE[i]);
      }
      return compact;
   }

   private static float lithium$lookupSine(int index) {
      if (index == 32768) {
         return LITHIUM_SINE_MIDPOINT;
      }
      int negation = (index & 32768) << 16;
      int mask = index << 17 >> 31;
      int position = (32769 & mask) + (index ^ mask) & 32767;
      return Float.intBitsToFloat(LITHIUM_SINE_TABLE[position] ^ negation);
   }
