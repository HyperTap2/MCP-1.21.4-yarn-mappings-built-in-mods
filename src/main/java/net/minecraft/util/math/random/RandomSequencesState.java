...
public class RandomSequencesState extends PersistentState {
   ...
   public NbtCompound writeNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
      ...
      this.sequences
         .forEach(
            (id, sequence) -> nbtCompound.put(id.toString(), (NbtElement)RandomSequence.CODEC.encodeStart(NbtOps.INSTANCE, sequence).result().orElseThrow())
         );
   ...
   public static RandomSequencesState fromNbt(long seed, NbtCompound nbt) {
      ...
      for (String string : nbtCompound.getKeys()) {
         try {
            RandomSequence randomSequence = (RandomSequence)((Pair)RandomSequence.CODEC.decode(NbtOps.INSTANCE, nbtCompound.get(string)).result().get())
               .getFirst();
            randomSequencesState.sequences.put(Identifier.of(string), randomSequence);
         } catch (Exception exception) {
            LOGGER.error("Failed to load random sequence {}", string, exception);
         }
      }
