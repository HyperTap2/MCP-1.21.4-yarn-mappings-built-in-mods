...
public enum DirectionTransformation implements StringIdentifiable {
   ...
   private static final DirectionTransformation[][] COMBINATIONS = Util.make(
      ...
      directionTransformations -> {
         Map<Pair<AxisTransformation, BooleanList>, DirectionTransformation> map = Arrays.stream(values())
            .collect(
               Collectors.toMap(
         ...
         for (DirectionTransformation directionTransformation : values()) {
            for (DirectionTransformation directionTransformation2 : values()) {
               ...
               directionTransformations[directionTransformation.ordinal()][directionTransformation2.ordinal()] = map.get(
   ...
   private static final DirectionTransformation[] INVERSES = Arrays.stream(values())
      .map(
         directionTransformation -> Arrays.stream(values())
   ...
   DirectionTransformation(final String name, final AxisTransformation axisTransformation, final boolean flipX, final boolean flipY, final boolean flipZ) {
      this.name = name;
      this.flipX = flipX;
      this.flipY = flipY;
      this.flipZ = flipZ;
