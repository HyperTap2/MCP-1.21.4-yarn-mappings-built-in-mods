...
public abstract class VoxelShape implements OffsetVoxelShapeCache {
   protected final VoxelSet voxels;
   @Nullable
   private VoxelShape[] shapeCache;
   @Nullable
   private volatile VoxelShape[] offsetAndSimplifiedShapes;
   ...
   protected int getCoordIndex(Direction.Axis axis, double coord) {
      DoubleList points = this.getPointPositions(axis);
      ...
      while (length > 0) {
         ...
         if (index <= size && !(coord < points.getDouble(index))) {
            start = index + 1;
            length -= half + 1;
         } else {
            length = half;
   ...
   private VoxelShape getUncachedFace(Direction facing) {
      ...
      if (this.isSquare(axis)) {
         ...
         if (slicedVoxelShape.isEmpty()) {
            ...
            return slicedVoxelShape.isCube() ? VoxelShapes.fullCube() : slicedVoxelShape;
         }
      }
   }
   ...
   protected double calculateMaxDistance(AxisCycleDirection axisCycle, Box box, double maxDist) {
      ...
      if (maxDist > 0.0) {
         double boxMax = box.getMax(axis);
         int maxIndex = this.getCoordIndex(axis, boxMax - 1.0E-7);
         int shapeMax = this.voxels.getSize(axis);
         for (int x = maxIndex + 1; x < shapeMax; x++) {
            minY = minY == Integer.MIN_VALUE ? Math.max(0, this.getCoordIndex(axis2, box.getMin(axis2) + 1.0E-7)) : minY;
            maxY = maxY == Integer.MIN_VALUE
               ? Math.min(this.voxels.getSize(axis2), this.getCoordIndex(axis2, box.getMax(axis2) - 1.0E-7) + 1)
               : maxY;
            for (int y = minY; y < maxY; y++) {
               minZ = minZ == Integer.MIN_VALUE ? Math.max(0, this.getCoordIndex(axis3, box.getMin(axis3) + 1.0E-7)) : minZ;
               maxZ = maxZ == Integer.MIN_VALUE
                  ? Math.min(this.voxels.getSize(axis3), this.getCoordIndex(axis3, box.getMax(axis3) - 1.0E-7) + 1)
                  : maxZ;
               for (int z = minZ; z < maxZ; z++) {
                  if (this.voxels.inBoundsAndContains(axisCycleDirection, x, y, z)) {
                     double distance = this.getPointPosition(axis, x) - boxMax;
                     if (distance >= -1.0E-7) {
                        maxDist = Math.min(maxDist, distance);
                     }
                     return maxDist;
                  }
               }
         ...
         double boxMin = box.getMin(axis);
         int minIndex = this.getCoordIndex(axis, boxMin + 1.0E-7);
         for (int x = minIndex - 1; x >= 0; x--) {
            minY = minY == Integer.MIN_VALUE ? Math.max(0, this.getCoordIndex(axis2, box.getMin(axis2) + 1.0E-7)) : minY;
            maxY = maxY == Integer.MIN_VALUE
               ? Math.min(this.voxels.getSize(axis2), this.getCoordIndex(axis2, box.getMax(axis2) - 1.0E-7) + 1)
               : maxY;
            for (int y = minY; y < maxY; y++) {
               ...
               for (int z = minZ; z < maxZ; z++) {
                  if (this.voxels.inBoundsAndContains(axisCycleDirection, x, y, z)) {
                     double distance = this.getPointPosition(axis, x + 1) - boxMin;
                     if (distance <= 1.0E-7) {
   ...
   @Override
   public void lithium$setShape(float offset, Direction direction, VoxelShape shape) {
      if (shape == null) {
         throw new IllegalArgumentException("shape must not be null");
      }
      int index = getOffsetShapeIndex(offset, direction);
      VoxelShape[] shapes = this.offsetAndSimplifiedShapes;
      shapes = shapes == null ? new VoxelShape[13] : shapes.clone();
      shapes[index] = shape;
      this.offsetAndSimplifiedShapes = shapes;
   }
   ...
   public VoxelShape lithium$getOffsetSimplifiedShape(float offset, Direction direction) {
      VoxelShape[] shapes = this.offsetAndSimplifiedShapes;
      return shapes == null ? null : shapes[getOffsetShapeIndex(offset, direction)];
   }

   private static int getOffsetShapeIndex(float offset, Direction direction) {
      if (offset != 0.0F && offset != 0.5F && offset != 1.0F) {
         throw new IllegalArgumentException("offset must be one of {0f, 0.5f, 1f}");
      }
      return offset == 0.0F ? 0 : (int)(2.0F * offset) + 2 * direction.getId();
   }
