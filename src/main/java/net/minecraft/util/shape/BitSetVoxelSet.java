...
public final class BitSetVoxelSet extends VoxelSet {
   ...
   public void lithium$replaceStorage(BitSet replacement) {
      this.storage.clear();
      this.storage.or(replacement);
      this.minX = this.sizeX;
      this.minY = this.sizeY;
      this.minZ = this.sizeZ;
      this.maxX = 0;
      this.maxY = 0;
      this.maxZ = 0;

      for (int index = this.storage.nextSetBit(0); index >= 0; index = this.storage.nextSetBit(index + 1)) {
         int x = index / (this.sizeY * this.sizeZ);
         int remainder = index - x * this.sizeY * this.sizeZ;
         int y = remainder / this.sizeZ;
         int z = remainder - y * this.sizeZ;
         this.minX = Math.min(this.minX, x);
         this.minY = Math.min(this.minY, y);
         this.minZ = Math.min(this.minZ, z);
         this.maxX = Math.max(this.maxX, x + 1);
