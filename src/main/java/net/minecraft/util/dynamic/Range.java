            ...
            range -> {
               if (range.minInclusive().compareTo(minInclusive) < 0) {
                  return DataResult.error(
                     () -> "Range limit too low, expected at least " + minInclusive + " [" + range.minInclusive() + "-" + range.maxInclusive() + "]"
                  );
               } else {
                  return range.maxInclusive().compareTo(maxInclusive) > 0
                     ? DataResult.error(
                        () -> "Range limit too high, expected at most " + maxInclusive + " [" + range.minInclusive() + "-" + range.maxInclusive() + "]"
                     )
