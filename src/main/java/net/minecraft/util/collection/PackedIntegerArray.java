...
public class PackedIntegerArray implements PaletteStorage, BitStorageExtension, CompactingPackedIntegerArray {
   ...
   private int getStorageIndex(int index) {
      ...
      return (int)(index * l + m >> 32 >> this.indexShift);
   }
   ...
   public int swap(int index, int value) {
      ...
      this.data[i] = l & ~(this.maxValue << j) | (value & this.maxValue) << j;
      return k;
   }
   ...
   @Override
   public <T> void lithium$compact(Palette<T> sourcePalette, Palette<T> destinationPalette, short[] output) {
      ...
      short[] mappings = new short[(int)(this.maxValue + 1L)];
      int index = 0;
      for (long word : this.data) {
         for (int element = 0; element < this.elementsPerLong; element++) {
            int value = (int)(word & this.maxValue);
            int remapped = mappings[value];
            if (remapped == 0) {
               remapped = destinationPalette.index(sourcePalette.get(value)) + 1;
               mappings[value] = (short)remapped;
            }
            output[index] = (short)(remapped - 1);
            word >>= this.elementBits;
            if (++index >= this.size) {
               return;
            }
         }
   ...
   public <T> void sodium$unpack(T[] out, Palette<T> palette) {
      ...
      for (long word : this.data) {
         for (int element = 0; element < this.elementsPerLong; element++) {
            out[index] = Objects.requireNonNull(palette.get((int)(word & this.maxValue)), "Palette does not contain entry for value in storage");
            word >>= this.elementBits;
            if (++index >= this.size) {
               return;
            }
