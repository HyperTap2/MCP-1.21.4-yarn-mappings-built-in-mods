...
public record JfrProfile(
   ...
   public List<Pair<ChunkStatus, LongRunningSampleStatistics<ChunkGenerationSample>>> getChunkGenerationSampleStatistics() {
      ...
      return map.entrySet()
         .stream()
         .map(entry -> Pair.of(entry.getKey(), LongRunningSampleStatistics.fromSamples(entry.getValue())))
         .sorted(
            Comparator.<Pair<ChunkStatus, LongRunningSampleStatistics<ChunkGenerationSample>>, Duration>comparing(
                  pair -> ((LongRunningSampleStatistics)pair.getSecond()).totalDuration()
               )
               .reversed()
         )
         .toList();
