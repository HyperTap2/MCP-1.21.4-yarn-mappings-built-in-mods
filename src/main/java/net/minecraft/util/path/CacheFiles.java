...
public class CacheFiles {
   ...
   public static void clear(Path directory, int maxRetained) {
      try {
         ...
         for (int j = 0; j < i; j++) {
            CacheFiles.CacheEntry cacheEntry = list2.get(j);
            ...
            try {
               ...
               LOGGER.warn("Failed to delete cache file {}", path, iOException);
            }
         }
         ...
         for (Path path2 : set) {
            try {
               ...
               LOGGER.warn("Failed to delete empty(?) cache directory {}", path2, iOException2);
            }
         }
      } catch (IOException | UncheckedIOException exception) {
         LOGGER.error("Failed to vacuum cache dir {}", directory, exception);
      }
   }
