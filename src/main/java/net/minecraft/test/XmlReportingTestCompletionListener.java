...
public class XmlReportingTestCompletionListener implements TestCompletionListener {
   ...
   private Element addTestCase(GameTestState test, String name) {
      ...
      element.setAttribute("time", String.valueOf(test.getElapsedMilliseconds() / 1000.0));
   ...
   public void onTestFailed(GameTestState test) {
      ...
      element.setAttribute("message", "(" + test.getOrigin().toShortString() + ") " + string2);
   ...
   public void onStopped() {
      this.stopwatch.stop();
      this.testSuiteElement.setAttribute("time", String.valueOf(this.stopwatch.elapsed(TimeUnit.MILLISECONDS) / 1000.0));
