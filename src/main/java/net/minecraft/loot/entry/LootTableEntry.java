...
public class LootTableEntry extends LeafEntry {
   public static final MapCodec<LootTableEntry> CODEC = RecordCodecBuilder.mapCodec(
      instance -> instance.group(
            Codec.either(RegistryKey.createCodec(RegistryKeys.LOOT_TABLE), LootTable.CODEC).fieldOf("value").forGetter(entry -> entry.value)
         )
   ...
   private LootTableEntry(
      Either<RegistryKey<LootTable>, LootTable> value, int weight, int quality, List<LootCondition> conditions, List<LootFunction> functions
   ) {
   ...
   public void generateLoot(Consumer<ItemStack> lootConsumer, LootContext context) {
      ((LootTable)this.value.map(key -> context.getLookup().getOptionalEntry(key).map(RegistryEntry::value).orElse(LootTable.EMPTY), table -> table))
