...
public class SetCustomModelDataLootFunction extends ConditionalLootFunction {
   private static final Codec<LootNumberProvider> COLOR_CODEC = Codec.<LootNumberProvider, Integer>either(
      LootNumberProviderTypes.CODEC, Codecs.RGB
   ).xmap(
      either -> either.map(p -> p, rgb -> new ConstantLootNumberProvider(rgb)),
      provider -> Either.left(provider)
   );
