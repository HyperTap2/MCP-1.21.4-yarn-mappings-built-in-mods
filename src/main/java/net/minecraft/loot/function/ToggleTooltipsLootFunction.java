...
public class ToggleTooltipsLootFunction extends ConditionalLootFunction {
   private static final Map<ComponentType<?>, ToggleTooltipsLootFunction.Toggle<?>> TOGGLES = Stream.of(
      ...
      .collect(Collectors.toMap(ToggleTooltipsLootFunction.Toggle::type, toggle -> (ToggleTooltipsLootFunction.Toggle<?>)toggle));
   private static final Codec<ToggleTooltipsLootFunction.Toggle<?>> TOGGLE_CODEC = Registries.DATA_COMPONENT_TYPE
      .getCodec()
      .comapFlatMap(
         componentType -> {
            ToggleTooltipsLootFunction.Toggle<?> toggle = TOGGLES.get(componentType);
            return toggle != null
   ...
   record Toggle<T>(ComponentType<T> type, ToggleTooltipsLootFunction.TooltipSetter<T> setter) {
