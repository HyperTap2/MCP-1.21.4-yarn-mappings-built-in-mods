...
public class LootFunctionTypes {
   ...
   public static final LootFunctionType<EnchantWithLevelsLootFunction> ENCHANT_WITH_LEVELS = register(
      "enchant_with_levels", EnchantWithLevelsLootFunction.CODEC
   );
   ...
   public static BiFunction<ItemStack, LootContext, ItemStack> join(List<? extends BiFunction<ItemStack, LootContext, ItemStack>> terms) {
      ...
      return switch (list.size()) {
         ...
         case 2 -> {
            ...
            yield (stack, context) -> biFunction2.apply(biFunction.apply(stack, context), context);
         }
         default -> (stack, context) -> {
            for (BiFunction<ItemStack, LootContext, ItemStack> biFunctionx : list) {
               stack = biFunctionx.apply(stack, context);
            }
