...
public class SpawnRestriction {
   ...
   public static <T extends Entity> boolean canSpawn(EntityType<T> type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
      SpawnRestriction.Entry entry = RESTRICTIONS.get(type);
      return entry == null || ((SpawnPredicate<T>)(Object)entry.predicate).test(type, world, spawnReason, pos, random);
   }
   ...
   static {
      ...
      register(EntityType.GLOW_SQUID, SpawnLocationTypes.IN_WATER, Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, (EntityType<GlowSquidEntity> type, ServerWorldAccess world, SpawnReason reason, BlockPos pos, Random random) -> GlowSquidEntity.canSpawn((EntityType<? extends LivingEntity>)(EntityType<?>)type, world, reason, pos, random));
