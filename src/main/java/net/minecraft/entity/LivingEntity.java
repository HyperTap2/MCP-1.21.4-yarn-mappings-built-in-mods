...
public abstract class LivingEntity extends Entity implements Attackable, EquipmentEntity, EquipmentEntity.EquipmentTrackingEntity,
   EquipmentEntity.TickableEnchantmentTrackingEntity, ChangeSubscriber.CountChangeSubscriber<ItemStack> {
   ...
   private boolean lithium$equipmentChanged = true;
   private boolean lithium$maybeHasTickableEnchantments = this instanceof PlayerEntity;
   ...
   protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
      ...
      if (this.getWorld() instanceof ServerWorld serverWorld && onGround && this.fallDistance > 0.0F) {
         ...
         if (this.fallDistance > d && !state.isAir()) {
            double e = this.getX();
            double f = this.getY();
            double g = this.getZ();
            ...
            float k = MathHelper.ceil(this.fallDistance - d);
            double l = Math.min(0.2F + k / 15.0F, 2.5);
            int m = (int)(150.0 * l);
            serverWorld.spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, state), e, f, g, m, 0.0, 0.0, 0.0, 0.15F);
         }
      }
   ...
   public void baseTick() {
      ...
      if (this.lithium$maybeHasTickableEnchantments && this.getWorld() instanceof ServerWorld serverWorld) {
            ...
            if (bl2) {
               ...
               if (this.getAir() == -20) {
                  ...
                  for (int i = 0; i < 8; i++) {
                     ...
                     this.getWorld().addParticle(ParticleTypes.BUBBLE, this.getX() + e, this.getY() + f, this.getZ() + g, vec3d.x, vec3d.y, vec3d.z);
                  }
   ...
   protected void addPowderSnowSlowIfNeeded() {
      ...
      if (i > 0) {
         EntityAttributeInstance entityAttributeInstance = this.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED);
         if (entityAttributeInstance != null && !this.getLandingBlockState().isAir()) {
            float f = -0.05F * this.getFreezingScale();
            entityAttributeInstance.addTemporaryModifier(
               new EntityAttributeModifier(POWDER_SNOW_SPEED_MODIFIER_ID, f, EntityAttributeModifier.Operation.ADD_VALUE)
            );
         }
      }
   }
   ...
   public void onEquipStack(EquipmentSlot slot, ItemStack oldStack, ItemStack newStack) {
      if (!this.getWorld().isClient() && !this.isSpectator()) {
         ...
         if (!bl && !ItemStack.areItemsAndComponentsEqual(oldStack, newStack)) {
         ...
         if (!bl && !ItemStack.areItemsAndComponentsEqual(oldStack, newStack) && !this.firstUpdate) {
            ...
            if (!this.isSilent() && equippableComponent != null && slot == equippableComponent.slot()) {
               this.getWorld()
                  .playSound(
                     null, this.getX(), this.getY(), this.getZ(), equippableComponent.equipSound(), this.getSoundCategory(), 1.0F, 1.0F, this.random.nextLong()
                  );
            }
   ...
   protected void tickStatusEffects() {
      ...
      try {
         while (iterator.hasNext()) {
            RegistryEntry<StatusEffect> registryEntry = iterator.next();
      ...
      if (!list.isEmpty()) {
         ...
         int i = this.isInvisible() ? 15 : ProtocolTranslator.getTargetVersion().olderThan(ProtocolVersion.v1_20_5) ? 2 : 4;
         int j = bl ? 5 : 1;
         if (this.random.nextInt(i * j) == 0) {
            this.getWorld()
               .addParticle(Util.getRandom(list, this.random), this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 1.0, 1.0, 1.0);
         }
      }
   }
   ...
   protected void dropExperience(ServerWorld world, @Nullable Entity attacker) {
      if (!this.isExperienceDroppingDisabled()
         && (
            this.shouldAlwaysDropExperience()
               || this.playerHitTimer > 0 && this.shouldDropExperience() && world.getGameRules().getBoolean(GameRules.DO_MOB_LOOT)
         )) {
      ...
      if (this.isSpectator()) {
         ...
         if (blockState.isIn(BlockTags.CLIMBABLE)) {
            ...
            if (ProtocolTranslator.getTargetVersion().olderThan(LegacyProtocolVersion.b1_5tob1_5_2)) {
               ...
               if (upperState.isIn(BlockTags.CLIMBABLE)
                  || upperState.getBlock() instanceof TrapdoorBlock && this.canEnterTrapdoor(upperPos, upperState)) {
                  this.climbingPos = Optional.of(upperPos);
                  return true;
               }
            }
   ...
   protected int computeFallDamage(float fallDistance, float damageMultiplier) {
      ...
      return MathHelper.ceil(g * damageMultiplier * this.getAttributeValue(EntityAttributes.FALL_DAMAGE_MULTIPLIER));
   }
      ...
      switch (status) {
         ...
         case 46:
            ...
            for (int j = 0; j < 128; j++) {
               ...
               double e = MathHelper.lerp(d, this.prevX, this.getX()) + (this.random.nextDouble() - 0.5) * this.getWidth() * 2.0;
               double k = MathHelper.lerp(d, this.prevY, this.getY()) + this.random.nextDouble() * this.getHeight();
               double l = MathHelper.lerp(d, this.prevZ, this.getZ()) + (this.random.nextDouble() - 0.5) * this.getWidth() * 2.0;
   ...
   private void travelMidAir(Vec3d movementInput) {
      ...
      if (statusEffectInstance != null) {
         ...
         || this.getWorld().isChunkLoaded(blockPos)
            && (
               ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_13_2)
                  || this.getWorld().getChunkManager().isChunkLoaded(blockPos.getX() >> 4, blockPos.getZ() >> 4)
            )) {
   ...
   private void travelInFluid(Vec3d movementInput) {
      ...
      if (this.isTouchingWater()) {
         boolean swimSprinting = ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_12_2) && this.isSprinting();
         float f = swimSprinting ? 0.9F : this.getBaseWaterMovementSpeedMultiplier();
         float g = 0.02F;
         ...
         Vec3d vec3d = this.getVelocity();
         if (ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_13_2) && this.horizontalCollision && this.isClimbing()) {
         ...
         if (ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_15_2)
            && this.getFluidHeight(FluidTags.LAVA) <= this.getSwimHeight()) {
   ...
   private Vec3d calcGlidingVelocity(Vec3d oldVelocity) {
      ...
      double glidingCosine = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_18)
         ? MathHelper.cos(f)
         : Math.cos(f);
      double h = MathHelper.square(glidingCosine);
   ...
   public Vec3d applyFluidMovingSpeed(double gravity, boolean falling, Vec3d motion) {
      if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_12_2) && !this.hasNoGravity()) {
         return new Vec3d(motion.x, motion.y - 0.02, motion.z);
      }

      falling = ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_13_2) && falling;
   ...
   public void tick() {
      ...
      if (f > 0.0025000002F) {
         ...
         float m = VisualSettings.INSTANCE.sidewaysBackwardsRunning.isEnabled()
            ? 0.0F
            : MathHelper.abs(MathHelper.wrapDegrees(this.getYaw()) - l);
         if (95.0F < m && m < 265.0F) {
            g = l - 180.0F;
         } else {
            g = l;
   ...
   private void sendEquipmentChanges() {
      ...
      if (map != null) {
         ...
         if (!map.isEmpty()) {
            ...
            if (!(this instanceof PlayerEntity)) {
               this.lithium$equipmentChanged = false;
            }
         }
      }
   }
   ...
   private Map<EquipmentSlot, ItemStack> getEquipmentChanges() {
      if (!this.lithium$equipmentChanged) {
         return null;
      }
   ...
   public void lithium$onEquipmentReplaced(ItemStack oldStack, ItemStack newStack) {
      this.lithium$updateHasTickableEnchantments(oldStack, newStack);
      this.lithium$onEquipmentChanged();
      if (!oldStack.isEmpty()) {
         ((ChangePublisher<ItemStack>)oldStack).lithium$unsubscribeWithData(this, 0);
      }
      if (!newStack.isEmpty()) {
         ((ChangePublisher<ItemStack>)newStack).lithium$subscribe(this, 0);
      }
   }

   @Override
   public void lithium$onEquipmentChanged() {
      this.lithium$equipmentChanged = true;
   }
   ...
   public void lithium$notifyCount(ItemStack publisher, int subscriberData, int newCount) {
      if (newCount == 0) {
         ((ChangePublisher<ItemStack>)publisher).lithium$unsubscribeWithData(this, subscriberData);
      }
   ...
   public void lithium$updateHasTickableEnchantments(ItemStack oldStack, ItemStack newStack) {
      if (!this.lithium$maybeHasTickableEnchantments) {
         this.lithium$maybeHasTickableEnchantments = lithium$stackHasTickableEnchantment(newStack);
      }
   }
   ...
   public void lithium$notifyAfterEnchantmentChange(ItemStack publisher, int subscriberData) {
      if (!this.lithium$maybeHasTickableEnchantments) {
         this.lithium$maybeHasTickableEnchantments = lithium$stackHasTickableEnchantment(publisher);
      }
   }

   private static boolean lithium$stackHasTickableEnchantment(ItemStack stack) {
      if (stack.isEmpty()) {
         return false;
      }
      ...
      for (RegistryEntry<Enchantment> enchantment : enchantments.getEnchantments()) {
         if (!enchantment.value().getEffect(EnchantmentEffectComponentTypes.TICK).isEmpty()) {
            return true;
         }
      }
      return false;
   ...
   protected final void lithium$trackDirectEquipmentReplacement(ItemStack oldStack, ItemStack newStack) {
      if (!this.getWorld().isClient() && oldStack != newStack) {
         this.lithium$onEquipmentReplaced(oldStack, newStack);
      }
   }
   ...
   protected float turnHead(float bodyRotation, float headRotation) {
      ...
      if (VisualSettings.INSTANCE.changeBodyRotationInterpolation.isEnabled()) {
         float clampedRotation = MathHelper.clamp(g, -75.0F, 75.0F);
         this.bodyYaw = this.getYaw() - clampedRotation;
         if (Math.abs(clampedRotation) > 50.0F) {
            this.bodyYaw += clampedRotation * 0.2F;
         }
      } else if (Math.abs(g) > h) {
   ...
   public void tickMovement() {
      ...
      double e = vec3d.y;
      double f = vec3d.z;
      double velocityZero = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_8) ? 0.005 : 0.003;
      if (Math.abs(vec3d.x) < velocityZero) {
         d = 0.0;
      }

      if (Math.abs(vec3d.y) < velocityZero) {
         e = 0.0;
      }

      if (Math.abs(vec3d.z) < velocityZero) {
         f = 0.0;
      }
      ...
      if (this.jumping && this.shouldSwimInFluids()) {
         ...
         if (this.isInLava()) {
            ...
            if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_12_2) && g > 0.0) {
               g = 1.0;
            }
         }
      ...
      if (this.hasStatusEffect(StatusEffects.SLOW_FALLING)
         || ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_12_2) && this.hasStatusEffect(StatusEffects.LEVITATION)) {
         this.onLanding();
      }
      ...
      if (this.getWorld() instanceof ServerWorld serverWorld && this.age % 40 == 0 && this.isFrozen() && this.canFreeze()) {
      ...
      this.tickCramming();
      profiler.pop();
      if (this.getWorld() instanceof ServerWorld serverWorld && this.hurtByWater() && this.isWet()) {
   ...
   protected void tickGliding() {
      ...
      if (!this.getWorld().isClient) {
         if (!this.isGliding() && this.glidingTicks == 0) {
            return;
         }
         if (!this.canGlide()) {
   ...
   protected boolean canGlide() {
      boolean vehicleBlocksGliding = ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_14_4) && this.hasVehicle();
      boolean levitationBlocksGliding = ProtocolTranslator.getTargetVersion().newerThan(ProtocolVersion.v1_15_2)
         && this.hasStatusEffect(StatusEffects.LEVITATION);
      if (!this.isOnGround() && !vehicleBlocksGliding && !levitationBlocksGliding) {
         for (EquipmentSlot equipmentSlot : EquipmentSlot.VALUES) {
   ...
   protected void tickCramming() {
      ...
      if (!(this.getWorld() instanceof ServerWorld serverWorld)) {
         ...
         List<Entity> list = WorldHelper.getPushableEntities(
            this.getWorld(), this, this.getBoundingBox(), EntityPredicates.canBePushedBy(this)
         );
         if (!list.isEmpty()) {
   ...
   @Override
   public void lithium$setClimbingMobCachingSectionUpdateBehavior(boolean listening) {
      this.lithium$updateClimbingMobCachingSectionOnChange = listening;
   }
   ...
   private void lithium$updateClimbingMobCachingSection(@Nullable BlockState newState) {
      SectionedEntityCache<Entity> cache = WorldHelper.getEntityCacheOrNull(this.getWorld());
      if (cache == null) {
         return;
      }
      EntityTrackingSection<Entity> section = cache.findTrackingSection(ChunkSectionPos.from(this).asLong());
      if (section != null) {
         ((ClimbingMobCachingSection)section).lithium$onEntityModifiedCachedBlock((FeetBlockCachingEntity)this, newState);
      } else {
   ...
   private void tickActiveItemStack() {
      if (this.isUsingItem()) {
         ...
         boolean sameActiveStack = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_14_3)
            ? stackInHand == this.activeItemStack
            : ItemStack.areItemsEqual(stackInHand, this.activeItemStack);
         if (sameActiveStack) {
   ...
   public ItemStack getBlockingItem() {
      if (this.isUsingItem() && !this.activeItemStack.isEmpty()) {
         ...
         if (item.getUseAction(this.activeItemStack) != UseAction.BLOCK) {
            ...
            int blockUseDelay = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_8) ? 0 : 5;
            return item.getMaxUseTime(this.activeItemStack, this) - this.itemUseTimeLeft < blockUseDelay ? null : this.activeItemStack;
         }
      } else {
         return null;
      }
   ...
   public void sleep(BlockPos pos) {
      ...
      if (blockState.getBlock() instanceof BedBlock) {
         this.getWorld().setBlockState(pos, blockState.with(BedBlock.OCCUPIED, true), 3);
      }
   ...
   public void wakeUp() {
      this.getSleepingPosition().filter(this.getWorld()::isChunkLoaded).ifPresent(pos -> {
         ...
         if (blockState.getBlock() instanceof BedBlock) {
            ...
            this.getWorld().setBlockState(pos, blockState.with(BedBlock.OCCUPIED, false), 3);
            Vec3d vec3dx = BedBlock.findWakeUpPosition(this.getType(), this.getWorld(), pos, direction, this.getYaw()).orElseGet(() -> {
   ...
   public Vec3d getPassengerRidingPos(Entity passenger) {
      Vec3d attachmentPos = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20)
         ...
         : this.getPassengerAttachmentPos(passenger, this.getDimensions(this.getPose()), this.getScale() * this.getScaleFactor());
      return this.getPos().add(attachmentPos);
   }

   protected void lerpHeadYaw(int headTrackingIncrements, double serverHeadYaw) {
      this.headYaw = (float)MathHelper.lerpAngleDegrees(1.0 / headTrackingIncrements, this.headYaw, serverHeadYaw);
   }
