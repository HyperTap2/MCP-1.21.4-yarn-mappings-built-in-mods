...
public class AreaEffectCloudEntity extends Entity implements Ownable {
      ...
      if (!bl || !this.random.nextBoolean()) {
         ...
         for (int j = 0; j < i; j++) {
            ...
            if (particleEffect.getType() == ParticleTypes.ENTITY_EFFECT) {
               ...
               this.getWorld()
                  .addImportantParticle(particleEffect, d, e, l, (0.5 - this.random.nextDouble()) * 0.15, 0.01F, (0.5 - this.random.nextDouble()) * 0.15);
            }
         }
      }
   }
   ...
   private void serverTick(ServerWorld world) {
      if (this.age >= this.waitTime + this.duration) {
         ...
         if (!bl2) {
            ...
            if (this.age % 5 == 0) {
               this.affectedEntities.entrySet().removeIf(entity -> this.age >= entity.getValue());
   ...
   protected void writeCustomDataToNbt(NbtCompound nbt) {
      ...
      RegistryOps<NbtElement> registryOps = this.getRegistryManager().getOps(NbtOps.INSTANCE);
      nbt.put("Particle", (NbtElement)ParticleTypes.TYPE_CODEC.encodeStart(registryOps, this.getParticleType()).getOrThrow());
      if (this.ownerUuid != null) {
         nbt.putUuid("Owner", this.ownerUuid);
      }

      if (!this.potionContentsComponent.equals(PotionContentsComponent.DEFAULT)) {
         NbtElement nbtElement = (NbtElement)PotionContentsComponent.CODEC.encodeStart(registryOps, this.potionContentsComponent).getOrThrow();
