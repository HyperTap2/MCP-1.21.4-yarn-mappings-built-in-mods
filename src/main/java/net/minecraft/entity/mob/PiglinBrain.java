...
public class PiglinBrain {
   ...
   private static void addIdleActivities(Brain<PiglinEntity> brain) {
      brain.setTaskList(
         ...
         (ImmutableList)ImmutableList.of(
            ...
            UpdateAttackTargetTask.create((world, target) -> target.isAdult(), PiglinBrain::getPreferredTarget),
   ...
   private static void addCelebrateActivities(Brain<PiglinEntity> brain) {
      brain.setTaskList(
         ...
         (ImmutableList)ImmutableList.of(
            ...
            LookAtMobTask.create(PiglinBrain::isGoldHoldingPlayer, 14.0F),
            UpdateAttackTargetTask.create((world, target) -> target.isAdult(), PiglinBrain::getPreferredTarget),
            TaskTriggerer.runIf(piglin -> !((PiglinEntity)piglin).isDancing(), WalkTowardsFuzzyPosTask.create(MemoryModuleType.CELEBRATE_LOCATION, 2, 1.0F)),
            TaskTriggerer.runIf(PiglinEntity::isDancing, WalkTowardsFuzzyPosTask.create(MemoryModuleType.CELEBRATE_LOCATION, 4, 0.6F)),
            new RandomTask(
               (ImmutableList)ImmutableList.of(
                  Pair.of(LookAtMobTask.create(EntityType.PIGLIN, 8.0F), 1), Pair.of(StrollTask.create(0.6F, 2, 1), 1), Pair.of(new WaitTask(10, 20), 1)
               )
            )
         ),
   ...
   private static void addAvoidActivities(Brain<PiglinEntity> brain) {
      brain.setTaskList(
         ...
         (ImmutableList)ImmutableList.of(
            ...
            makeRandomFollowTask(),
            makeRandomWanderTask(),
            ForgetTask.create(PiglinBrain::shouldRunAwayFromHoglins, MemoryModuleType.AVOID_TARGET)
         ),
      ...
      brain.setTaskList(
         ...
         (ImmutableList)ImmutableList.of(
            ...
            TaskTriggerer.runIf(
               ...
               Tasks.pickRandomly((ImmutableList)ImmutableList.builder().addAll(makeFollowTasks()).add(Pair.of(TaskTriggerer.predicate(piglin -> true), 1)).build())
            ),
            RidingTask.create(8, PiglinBrain::canRide)
         ),
   ...
   private static RandomTask<LivingEntity> makeRandomFollowTask() {
      return new RandomTask<LivingEntity>((ImmutableList)ImmutableList.builder().addAll(makeFollowTasks()).add(Pair.of(new WaitTask(30, 60), 1)).build());
   }
   ...
   private static void doBarter(PiglinEntity piglin, List<ItemStack> items) {
      ...
      if (optional.isPresent()) {
         dropBarteredItem(piglin, optional.get(), items);
      } else {
   ...
   private static Optional<? extends LivingEntity> getPreferredTarget(ServerWorld world, PiglinEntity piglin) {
      ...
      return optional3.isPresent() && Sensor.testAttackableTargetPredicate(world, piglin, optional3.get()) ? optional3 : Optional.empty();
   }
   ...
   private static boolean hasTargetToAvoid(PiglinEntity piglin) {
      ...
      return !brain.hasMemoryModule(MemoryModuleType.AVOID_TARGET)
         ? false
         : brain.getOptionalRegisteredMemory(MemoryModuleType.AVOID_TARGET).get().isInRange(piglin, 12.0);
   }
   ...
   protected static void angerNearbyPiglins(ServerWorld world, AbstractPiglinEntity piglin) {
      getNearbyPiglins(piglin)
         .forEach(nearbyPiglin -> getNearestDetectedPlayer(nearbyPiglin).ifPresent(player -> becomeAngryWith(world, nearbyPiglin, player)));
   }
   ...
   private static void becomeAngryWithPlayer(ServerWorld world, AbstractPiglinEntity piglin, LivingEntity target) {
      ...
      if (optional.isPresent()) {
         becomeAngryWith(world, piglin, optional.get());
      } else {
         becomeAngryWith(world, piglin, target);
      }
   }
   ...
   private static void runAwayFromClosestTarget(PiglinEntity piglin, LivingEntity target) {
      ...
      livingEntity = TargetUtil.getCloserEntity(piglin, brain.getOptionalRegisteredMemory(MemoryModuleType.AVOID_TARGET), livingEntity);
