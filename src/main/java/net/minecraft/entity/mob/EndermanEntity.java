...
public class EndermanEntity extends HostileEntity implements Angerable {
   ...
   boolean isPlayerStaring(PlayerEntity player) {
      return !LivingEntity.NOT_WEARING_GAZE_DISGUISE_PREDICATE.test(player) ? false : this.isEntityLookingAtMe(player, 0.025, true, false, this.getEyeY());
   }
   ...
   static class TeleportTowardsPlayerGoal extends ActiveTargetGoal<PlayerEntity> {
      ...
      @Override
      public boolean canStart() {
         this.targetPlayer = getServerWorld(this.enderman)
            .getClosestPlayer(this.staringPlayerPredicate.setBaseMaxDistance(this.getFollowRange()), this.enderman);
