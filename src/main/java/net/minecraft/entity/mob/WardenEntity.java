...
public class WardenEntity extends HostileEntity implements Vibrations {
      ...
      if (this.getWorld().isClient()) {
         if (this.age % this.getHeartRate() == 0) {
            ...
            if (!this.isSilent()) {
               this.getWorld()
                  .playSound(
                     this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_WARDEN_HEARTBEAT, this.getSoundCategory(), 5.0F, this.getSoundPitch(), false
                  );
            }
         }
   ...
   public float getTendrilAlpha(float tickDelta) {
      return MathHelper.lerp(tickDelta, (float)this.lastTendrilAlpha, (float)this.tendrilAlpha) / 10.0F;
   }

   public float getHeartAlpha(float tickDelta) {
   ...
   class VibrationCallback implements Vibrations.Callback {
      ...
      @Override
      public void accept(
         ServerWorld world, BlockPos pos, RegistryEntry<GameEvent> event, @Nullable Entity sourceEntity, @Nullable Entity entity, float distance
      ) {
