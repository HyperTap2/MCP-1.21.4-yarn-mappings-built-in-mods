...
public class ShulkerEntity extends GolemEntity implements VariantHolder<Optional<DyeColor>>, Monster {
   ...
   public void setVariant(Optional<DyeColor> optional) {
      this.dataTracker.set(COLOR, optional.<Byte>map(color -> (byte)color.getId()).orElse((byte)16));
   }
                     ...
                     ShulkerEntity.this.getWorld()
                        .spawnEntity(
                           new ShulkerBulletEntity(
                              ShulkerEntity.this.getWorld(), ShulkerEntity.this, livingEntity, ShulkerEntity.this.getAttachedFace().getAxis()
                           )
                        );
