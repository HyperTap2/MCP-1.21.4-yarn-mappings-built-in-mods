...
public class SlimeEntity extends MobEntity implements Monster {
   ...
   protected void initGoals() {
      ...
      this.targetSelector
         .add(1, new ActiveTargetGoal<>(this, PlayerEntity.class, 10, true, false, (target, world) -> Math.abs(target.getY() - this.getY()) <= 4.0));
   ...
   public void tick() {
      ...
      if (this.isOnGround() && !this.onGroundLastTick) {
         ...
         for (int i = 0; i < f * 16.0F; i++) {
            ...
            this.getWorld().addParticle(this.getParticles(), this.getX() + k, this.getY(), this.getZ() + l, 0.0, 0.0, 0.0);
         }
   ...
   public void remove(Entity.RemovalReason reason) {
      ...
      if (!this.getWorld().isClient && i > 1 && this.isDead()) {
         ...
         for (int l = 0; l < k; l++) {
            ...
            float m = (l / 2 - 0.5F) * g;
            this.convertTo(
               this.getType(), new EntityConversionContext(EntityConversionType.SPLIT_ON_DEATH, false, false, team), SpawnReason.TRIGGERED, newSlime -> {
