...
public class EvokerEntity extends SpellcastingIllagerEntity {
   ...
   public boolean isInSameTeam(Entity other) {
      if (other == this) {
         ...
         return other instanceof VexEntity vexEntity && vexEntity.getOwner() != null ? this.isInSameTeam(vexEntity.getOwner()) : false;
      }
   }
      ...
      protected void castSpell() {
         ...
         if (EvokerEntity.this.squaredDistanceTo(livingEntity) < 9.0) {
            for (int i = 0; i < 5; i++) {
               ...
               this.conjureFangs(EvokerEntity.this.getX() + MathHelper.cos(g) * 1.5, EvokerEntity.this.getZ() + MathHelper.sin(g) * 1.5, d, e, g, 0);
            }

            for (int i = 0; i < 8; i++) {
               float g = f + i * (float) Math.PI * 2.0F / 8.0F + (float) (Math.PI * 2.0 / 5.0);
               this.conjureFangs(EvokerEntity.this.getX() + MathHelper.cos(g) * 2.5, EvokerEntity.this.getZ() + MathHelper.sin(g) * 2.5, d, e, g, 3);
            }
         } else {
            for (int i = 0; i < 16; i++) {
               double h = 1.25 * (i + 1);
               int j = 1 * i;
               this.conjureFangs(EvokerEntity.this.getX() + MathHelper.cos(f) * h, EvokerEntity.this.getZ() + MathHelper.sin(f) * h, d, e, f, j);
            }
         }
      }
      ...
      private void conjureFangs(double x, double z, double maxY, double y, float yaw, int warmup) {
         ...
         if (bl) {
            EvokerEntity.this.getWorld()
               .spawnEntity(new EvokerFangsEntity(EvokerEntity.this.getWorld(), x, blockPos.getY() + d, z, yaw, warmup, EvokerEntity.this));
            EvokerEntity.this.getWorld().emitGameEvent(GameEvent.ENTITY_PLACE, new Vec3d(x, blockPos.getY() + d, z), GameEvent.Emitter.of(EvokerEntity.this));
         }
      }
      ...
      public void tick() {
         if (EvokerEntity.this.getTarget() != null) {
            EvokerEntity.this.getLookControl()
               .lookAt(EvokerEntity.this.getTarget(), EvokerEntity.this.getMaxHeadRotation(), EvokerEntity.this.getMaxLookPitchChange());
         } else if (EvokerEntity.this.getWololoTarget() != null) {
            EvokerEntity.this.getLookControl()
               .lookAt(EvokerEntity.this.getWololoTarget(), EvokerEntity.this.getMaxHeadRotation(), EvokerEntity.this.getMaxLookPitchChange());
         }
      }
   }
