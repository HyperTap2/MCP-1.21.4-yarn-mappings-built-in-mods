...
public class WardenBrain {
   ...
   private static void addCoreActivities(Brain<WardenEntity> brain) {
      brain.setTaskList(
         Activity.CORE,
         0,
         (ImmutableList)ImmutableList.of(new StayAboveWaterTask(0.8F), LookAtDisturbanceTask.create(), new UpdateLookControlTask(45, 90), new MoveToTargetTask())
      );
   }

   private static void addEmergeActivities(Brain<WardenEntity> brain) {
      brain.setTaskList(Activity.EMERGE, 5, (ImmutableList)ImmutableList.of(new EmergeTask(EMERGE_DURATION)), MemoryModuleType.IS_EMERGING);
   }
   ...
   private static void addInvestigateActivities(Brain<WardenEntity> brain) {
      brain.setTaskList(
         ...
         5,
         (ImmutableList)ImmutableList.of(
            FindRoarTargetTask.create(WardenEntity::getPrimeSuspect), WalkTowardsFuzzyPosTask.create(MemoryModuleType.DISTURBANCE_LOCATION, 2, 0.7F)
         ),
         MemoryModuleType.DISTURBANCE_LOCATION
      );
   }
   ...
   private static void addSniffActivities(Brain<WardenEntity> brain) {
      brain.setTaskList(
         Activity.SNIFF,
         5,
         (ImmutableList)ImmutableList.of(FindRoarTargetTask.create(WardenEntity::getPrimeSuspect), new SniffTask(SNIFF_DURATION)),
         MemoryModuleType.IS_SNIFFING
      );
   }
