...
public class CreakingBrain {
   ...
   static void addCoreTasks(Brain<CreakingEntity> brain) {
      brain.setTaskList(Activity.CORE, 0, (ImmutableList)ImmutableList.of(new StayAboveWaterTask<CreakingEntity>(0.8F) {
         protected boolean shouldRun(ServerWorld serverWorld, CreakingEntity creakingEntity) {
            return creakingEntity.isUnrooted() && super.shouldRun(serverWorld, creakingEntity);
         }
      ...
      brain.setTaskList(
         ...
         (ImmutableList)ImmutableList.of(
            UpdateAttackTargetTask.create(
               (world, creaking) -> ((CreakingEntity)creaking).isActive(),
            ...
            new RandomTask(
               ImmutableList.of(Pair.of(StrollTask.create(0.3F), 2), Pair.of(GoToLookTargetTask.create(0.3F, 3), 2), Pair.of(new WaitTask(30, 60), 1))
            )
         )
      );
   }
   ...
   static void addFightTasks(Brain<CreakingEntity> brain) {
      brain.setTaskList(
         Activity.FIGHT,
         10,
         (ImmutableList)ImmutableList.of(RangedApproachTask.create(1.0F), MeleeAttackTask.create(CreakingEntity::isUnrooted, 40), ForgetAttackTargetTask.create()),
         MemoryModuleType.ATTACK_TARGET
      );
   }
