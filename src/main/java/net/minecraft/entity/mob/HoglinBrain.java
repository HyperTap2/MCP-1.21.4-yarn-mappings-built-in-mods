...
public class HoglinBrain {
   ...
   private static void addIdleTasks(Brain<HoglinEntity> brain) {
      brain.setTaskList(
         ...
         ImmutableList.of(
            ...
            TaskTriggerer.runIf(
               HoglinEntity::isAdult, GoToRememberedPositionTask.createEntityBased(MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLIN, 0.4F, 8, false)
            ),
   ...
   private static void avoidEnemy(HoglinEntity hoglin, LivingEntity target) {
      ...
      Brain<HoglinEntity> brain = hoglin.getBrain();
      livingEntity = TargetUtil.getCloserEntity(hoglin, brain.getOptionalRegisteredMemory(MemoryModuleType.AVOID_TARGET), livingEntity);
   ...
   private static boolean hasMoreHoglinsAround(HoglinEntity hoglin) {
      ...
      int j = hoglin.getBrain().getOptionalRegisteredMemory(MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT).orElse(0) + 1;
      return i > j;
   }
