...
public abstract class SpellcastingIllagerEntity extends IllagerEntity {
   ...
   protected enum Spell {
      ...
      private static final IntFunction<SpellcastingIllagerEntity.Spell> BY_ID = ValueLists.<SpellcastingIllagerEntity.Spell>createIdToValueFunction(
         spell -> spell.id, values(), ValueLists.OutOfBoundsHandling.ZERO
      );
      final int id;
      final double[] particleVelocity;

      Spell(final int id, final double particleVelocityX, final double particleVelocityY, final double particleVelocityZ) {
         this.id = id;
