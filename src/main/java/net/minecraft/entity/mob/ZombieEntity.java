...
public class ZombieEntity extends HostileEntity {
   ...
   protected void convertInWater() {
      ...
      if (!this.isSilent()) {
         this.getWorld().syncWorldEvent(null, 1040, this.getBlockPos(), 0);
      }
   }
      ...
      ZombieVillagerEntity zombieVillagerEntity = villager.convertTo(
         ...
         zombieVillager -> {
            zombieVillager.initialize(
               world, world.getLocalDifficulty(zombieVillager.getBlockPos()), SpawnReason.CONVERSION, new ZombieEntity.ZombieData(false, true)
            );
         ...
         for (int l = 0; l < 50; l++) {
            ...
            if (SpawnRestriction.isSpawnPosAllowed(entityType, world, blockPos)
               ...
               if (!world.isPlayerInRange(m, n, o, 7.0)
                  ...
                  zombieEntity.getAttributeInstance(EntityAttributes.SPAWN_REINFORCEMENTS)
         ...
         if (zombieData.baby) {
            ...
            if (zombieData.tryChickenJockey) {
               if (random.nextFloat() < 0.05) {
                  List<ChickenEntity> list = world.getEntitiesByClass(
   ...
   protected void applyAttributeModifiers(float chanceMultiplier) {
      ...
      if (d > 1.0) {
         this.getAttributeInstance(EntityAttributes.FOLLOW_RANGE)
            .overwritePersistentModifier(
               new EntityAttributeModifier(ZOMBIE_RANDOM_SPAWN_BONUS_MODIFIER_ID, d, EntityAttributeModifier.Operation.ADD_MULTIPLIED_TOTAL)
            );
      }
         ...
         this.getAttributeInstance(EntityAttributes.MAX_HEALTH)
            .overwritePersistentModifier(
               new EntityAttributeModifier(
                  LEADER_ZOMBIE_BONUS_MODIFIER_ID, this.random.nextDouble() * 3.0 + 1.0, EntityAttributeModifier.Operation.ADD_MULTIPLIED_TOTAL
               )
            );
