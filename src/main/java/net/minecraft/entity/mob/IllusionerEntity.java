...
public class IllusionerEntity extends SpellcastingIllagerEntity implements RangedAttackMob {
   ...
   public void tickMovement() {
      ...
      if (this.getWorld().isClient && this.isInvisible()) {
         ...
         if (this.hurtTime == 1 || this.age % 1200 == 0) {
            ...
            for (int j = 0; j < 16; j++) {
               this.getWorld().addParticle(ParticleTypes.CLOUD, this.getParticleX(0.5), this.getRandomBodyY(), this.getBodyZ(0.5), 0.0, 0.0, 0.0);
            }

            this.getWorld()
               .playSound(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ILLUSIONER_MIRROR_MOVE, this.getSoundCategory(), 1.0F, 1.0F, false);
   ...
   public Vec3d[] getMirrorCopyOffsets(float tickDelta) {
      ...
      double d = (this.mirrorSpellTimer - tickDelta) / 3.0F;
      d = Math.pow(d, 0.25);
