...
public abstract class RaiderEntity extends PatrolEntity {
   ...
   static final Predicate<ItemEntity> OBTAINABLE_OMINOUS_BANNER_PREDICATE = itemEntity -> !itemEntity.cannotPickup()
      && itemEntity.isAlive()
      && ItemStack.areEqual(itemEntity.getStack(), lithium$getOminousBanner(itemEntity));
   @Nullable
   protected Raid raid;
   private int wave;
   ...
   public boolean isCaptain() {
      ...
      boolean bl = !itemStack.isEmpty()
         && ItemStack.areEqual(itemStack, lithium$getOminousBanner(this));
   ...
   static class AttackHomeGoal extends Goal {
      ...
      private boolean tryFindHome() {
         ...
         Optional<BlockPos> optional = serverWorld.getPointOfInterestStorage()
            .getPosition(
               new SinglePointOfInterestTypeFilter(POIRegistryEntries.HOME_ENTRY),
               this::canLootHome,
   ...
   public class PickUpBannerAsLeaderGoal<T extends RaiderEntity> extends Goal {
      ...
      public boolean canStart() {
         ...
         for (ItemEntity itemEntity : this.actor
            .getWorld()
            .getEntitiesByClass(ItemEntity.class, this.actor.getBoundingBox().expand(d, 8.0, d), RaiderEntity.OBTAINABLE_OMINOUS_BANNER_PREDICATE)) {
      ...
      private boolean shouldStop() {
         ...
         if (this.actor.getRaid().isFinished()) {
            return true;
         }

         if (!this.actor.canLead()) {
            return true;
         }

         if (ItemStack.areEqual(
            this.actor.getEquippedStack(EquipmentSlot.HEAD), lithium$getOminousBanner(this.actor)
         )) {
            return true;
         }
   ...
   private static ItemStack lithium$getOminousBanner(Entity entity) {
      return ((LithiumData)entity.getWorld())
         .lithium$getData()
         .ominousBanner(entity.getRegistryManager().getOrThrow(RegistryKeys.BANNER_PATTERN));
   }
}
