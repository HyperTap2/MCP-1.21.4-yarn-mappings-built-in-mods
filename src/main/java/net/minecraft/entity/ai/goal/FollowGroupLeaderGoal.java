   ...
   public boolean canStart() {
      if (this.fish.hasOtherFishInGroup()) {
         ...
         List<? extends SchoolingFishEntity> list = this.fish
            .getWorld()
            .getEntitiesByClass((Class<? extends SchoolingFishEntity>)this.fish.getClass(), this.fish.getBoundingBox().expand(8.0, 8.0, 8.0), predicate);
         SchoolingFishEntity schoolingFishEntity = (SchoolingFishEntity)DataFixUtils.orElse(
            list.stream().filter(SchoolingFishEntity::canHaveMoreFishInGroup).findAny(), this.fish
         );
