...
public abstract class MoveToTargetPosGoal extends Goal {
   ...
   protected void startMovingToTarget() {
      this.mob.getNavigation().startMovingTo(this.targetPos.getX() + 0.5, this.targetPos.getY() + 1, this.targetPos.getZ() + 0.5, this.speed);
   }
   ...
   public void tick() {
      ...
      if (!blockPos.isWithinDistance(this.mob.getPos(), this.getDesiredDistanceToTarget())) {
         ...
         if (this.shouldResetPath()) {
            this.mob.getNavigation().startMovingTo(blockPos.getX() + 0.5, blockPos.getY(), blockPos.getZ() + 0.5, this.speed);
         }
      } else {
         this.reached = true;
         this.tryingTime--;
