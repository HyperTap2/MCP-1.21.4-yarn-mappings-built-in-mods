...
public class FormCaravanGoal extends Goal {
   ...
   public boolean canStart() {
      if (!this.llama.isLeashed() && !this.llama.isFollowing()) {
         List<LlamaEntity> list = this.llama.getWorld().getEntitiesByClass(
            LlamaEntity.class, this.llama.getBoundingBox().expand(9.0, 4.0, 9.0), entity -> entity != this.llama
         );
         ...
         if (!llamaEntity.isLeashed() && !this.canFollow(llamaEntity, 1)) {
            return false;
         }
