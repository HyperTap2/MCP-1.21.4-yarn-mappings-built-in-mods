...
public class TaskTriggerer<E extends LivingEntity, M> implements App<TaskTriggerer.K1<E>, M> {
   ...
   public static <E extends LivingEntity> SingleTickTask<E> task(
      ...
      final TaskTriggerer.TaskFunction<E, TaskRunnable<E>> taskFunction = getFunction(creator.apply(newContext()));
      return new SingleTickTask<E>() {
         ...
         public boolean trigger(ServerWorld serverWorld, E livingEntity, long l) {
            ...
            return taskRunnable != null && taskRunnable.trigger(serverWorld, livingEntity, l);
         }
      ...
      QueryMemory(MemoryQuery<F, Value> query) {
         super(new TaskTriggerer.TaskFunction<E, MemoryQueryResult<F, Value>>() {
            public MemoryQueryResult<F, Value> run(ServerWorld serverWorld, E livingEntity, long l) {
               ...
               MemoryModuleType<Value> memoryType = (MemoryModuleType<Value>)query.memory();
               Optional<Value> optional = brain.getOptionalMemory(memoryType);
   ...
   public static final class TaskContext<E extends LivingEntity> implements Applicative<TaskTriggerer.K1<E>, TaskTriggerer.TaskContext.Mu<E>> {
      ...
      public <Value> TaskTriggerer<E, MemoryQueryResult<com.mojang.datafixers.kinds.IdF.Mu, Value>> queryMemoryValue(MemoryModuleType<Value> type) {
         return new TaskTriggerer.QueryMemory<>(new MemoryQuery.Present<>(type));
      }

      public <Value> TaskTriggerer<E, MemoryQueryResult<com.mojang.datafixers.kinds.Const.Mu<Unit>, Value>> queryMemoryAbsent(MemoryModuleType<Value> type) {
         ...
         return app2 -> {
            ...
            return TaskTriggerer.of(new TaskTriggerer.TaskFunction<E, R>() {
               ...
               public R run(ServerWorld world, E entity, long time) {
                  ...
                  Function<A, R> function = taskFunction2.run(world, entity, time);
