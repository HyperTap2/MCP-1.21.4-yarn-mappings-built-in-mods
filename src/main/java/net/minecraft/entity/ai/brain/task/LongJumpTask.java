...
public class LongJumpTask<E extends MobEntity> extends MultiTickTask<E> {
   ...
   protected List<LongJumpTask.Target> potentialTargets = Lists.newArrayList();
   ...
   protected boolean shouldKeepRunning(ServerWorld serverWorld, MobEntity mobEntity, long l) {
      boolean bl = this.startPos.isPresent()
         && this.startPos.get().equals(mobEntity.getPos())
   ...
   protected void run(ServerWorld serverWorld, E mobEntity, long l) {
      ...
      if (this.horizontalRange >= 0 && this.horizontalRange < 128 && this.verticalRange >= 0 && this.verticalRange < 128) {
         this.potentialTargets = LongJumpChoiceList.forCenter(blockPos, (byte)this.horizontalRange, (byte)this.verticalRange);
         return;
      }
      int i = blockPos.getX();
      int j = blockPos.getY();
      int k = blockPos.getZ();
      this.potentialTargets = BlockPos.stream(
   ...
   protected Optional<LongJumpTask.Target> removeRandomTarget(ServerWorld world) {
      if (this.potentialTargets instanceof LongJumpChoiceList choices) {
         return Optional.ofNullable(choices.removeRandomWeightedByDistanceSq(world.random));
      }
