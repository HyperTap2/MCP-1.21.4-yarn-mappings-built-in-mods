...
public class BreezeJumpTask extends MultiTickTask<BreezeEntity> {
   ...
   private static final ObjectArrayList<Integer> POSSIBLE_JUMP_ANGLES = new ObjectArrayList(Lists.newArrayList(new Integer[]{40, 55, 60, 75, 80}));
   ...
   public static boolean shouldJump(ServerWorld world, BreezeEntity breeze) {
      ...
      if (isTargetOutOfRange(breeze, livingEntity)) {
         breeze.getBrain().forget(MemoryModuleType.ATTACK_TARGET);
         return false;
      }

      if (isTargetTooClose(breeze, livingEntity)) {
         return false;
      }
