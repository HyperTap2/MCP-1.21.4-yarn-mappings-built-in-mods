            ...
            .apply(
               ...
               (potentialJobSite, jobSite, mobs, walkTarget, lookTarget) -> (world, entity, time) -> {
                  ...
                  context.<List<LivingEntity>>getValue(mobs)
                     ...
                     .filter(villager -> canUseJobSite(optional.get(), villager, blockPos))
                     .findFirst()
                     .ifPresent(villager -> {
                        walkTarget.forget();
                        lookTarget.forget();
   ...
   private static boolean canUseJobSite(RegistryEntry<PointOfInterestType> poiType, VillagerEntity villager, BlockPos pos) {
      ...
      if (bl) {
         ...
         if (villagerProfession.heldWorkstation().test(poiType)) {
            return optional.isEmpty() ? canReachJobSite(villager, pos, poiType.value()) : optional.get().pos().equals(pos);
         } else {
            return false;
         }
      }
