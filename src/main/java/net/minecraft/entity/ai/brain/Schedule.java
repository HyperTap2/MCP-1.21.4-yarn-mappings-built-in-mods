...
public class Schedule {
   ...
   protected List<ScheduleRule> getOtherRules(Activity activity) {
      return this.scheduleRules.entrySet().stream().filter(rule -> rule.getKey() != activity).map(Entry::getValue).collect(Collectors.toList());
   }
   ...
   public Activity getActivityForTime(int time) {
      return this.scheduleRules
         .entrySet()
         .stream()
         .max(Comparator.comparingDouble(rule -> rule.getValue().getPriority(time)))
         .map(Entry::getKey)
