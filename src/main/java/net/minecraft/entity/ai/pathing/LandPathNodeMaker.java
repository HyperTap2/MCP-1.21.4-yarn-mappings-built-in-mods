...
public class LandPathNodeMaker extends PathNodeMaker {
   ...
   public int getSuccessors(PathNode[] successors, PathNode node) {
      ...
      for (Direction direction : Direction.Type.HORIZONTAL) {
         ...
         if (this.isValidDiagonalSuccessor(
            node, this.successors[direction.getHorizontalQuarterTurns()], this.successors[direction2.getHorizontalQuarterTurns()]
         )) {
            PathNode pathNode2 = this.getPathNode(
               node.x + direction.getOffsetX() + direction2.getOffsetX(),
               node.y,
               node.z + direction.getOffsetZ() + direction2.getOffsetZ(),
               j,
               d,
               direction,
               pathNodeType2
   ...
   @Nullable
   private PathNode getJumpOnTopNode(
      int x, int y, int z, int maxYStep, double prevFeetY, Direction direction, PathNodeType nodeType, BlockPos.Mutable mutablePos
   ) {
      ...
      Box box = new Box(
         d - f,
         this.getFeetY(mutablePos.set(d, y + 1, e)) + 0.001,
         e - f,
         d + f,
   ...
   protected PathNodeType getNodeType(int x, int y, int z) {
      return (PathNodeType)this.nodeTypes.computeIfAbsent(BlockPos.asLong(x, y, z), l -> this.getNodeType(this.context, x, y, z, this.entity));
   }
   ...
   public PathNodeType getNodeType(PathContext context, int x, int y, int z, MobEntity mob) {
      ...
      if (set.contains(PathNodeType.UNPASSABLE_RAIL)) {
   ...
   public static PathNodeType getCommonNodeType(BlockView world, BlockPos pos) {
      BlockState blockState = world.getBlockState(pos);
      PathNodeType cachedType = PathNodeCache.getPathNodeType(blockState);
      if (cachedType != null) {
         return cachedType;
      }
