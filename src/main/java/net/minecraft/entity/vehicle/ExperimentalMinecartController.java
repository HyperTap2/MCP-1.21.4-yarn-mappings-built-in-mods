...
public class ExperimentalMinecartController extends MinecartController {
   ...
   private ExperimentalMinecartController.InterpolatedStep getLerpedStep(float tickDelta) {
      ...
      float f = (3 - this.ticksToNextRefresh + tickDelta) / 3.0F;
      float g = 0.0F;
      float h = 1.0F;
      boolean bl = false;
            ...
            this.stagingLerpSteps
               .add(
                  new ExperimentalMinecartController.Step(
                     vec3d3, this.getVelocity(), this.getYaw(), this.getPitch(), (float)Math.min(d, this.getMaxSpeed(world))
                  )
               );
   ...
   private Vec3d calcNewHorizontalVelocity(
      ...
      if (!iteration.slopeVelocityApplied) {
         Vec3d vec3d2 = this.applySlopeVelocity(vec3d, railShape);
