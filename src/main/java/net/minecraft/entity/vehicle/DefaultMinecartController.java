...
public class DefaultMinecartController extends MinecartController {
      ...
      if (this.getWorld() instanceof ServerWorld serverWorld) {
         ...
         if (bl) {
            ...
            if (blockState.isOf(Blocks.ACTIVATOR_RAIL)) {
               this.minecart.onActivatorRail(var11.getX(), var11.getY(), var11.getZ(), blockState.get(PoweredRailBlock.POWERED));
            }
         } else {
   ...
   public void moveOnRail(ServerWorld world) {
      EntityChangeListener listener = this.minecart.lithium$getChangeListener();
      if (!(this.minecart instanceof Inventory) || !(listener instanceof ToggleableMovementTracker toggleable)) {
         this.lithium$moveOnRail(world);
         return;
      }

      Vec3d beforeMove = this.minecart.getPos();
      int oldMask = toggleable.lithium$setNotificationMask(0);
      try {
         this.lithium$moveOnRail(world);
      } finally {
         toggleable.lithium$setNotificationMask(oldMask);
         if (!beforeMove.equals(this.minecart.getPos())) {
            listener.updateEntityPosition();
         }
      }
   }

   private void lithium$moveOnRail(ServerWorld world) {
   ...
   public boolean handleCollision() {
      ...
      if (this.minecart.isRideable() && this.getVelocity().horizontalLengthSquared() >= 0.01) {
         List<Entity> list = WorldHelper.getPushableEntities(
            this.getWorld(), this.minecart, box, EntityPredicates.canBePushedBy(this.minecart)
         );
         if (!list.isEmpty()) {
         ...
         for (AbstractMinecartEntity entity2 : this.getWorld().getEntitiesByClass(
            AbstractMinecartEntity.class, box, entity -> entity != this.minecart
         )) {
