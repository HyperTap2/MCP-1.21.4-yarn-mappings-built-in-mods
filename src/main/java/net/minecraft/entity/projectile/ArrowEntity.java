...
public class ArrowEntity extends PersistentProjectileEntity {
   ...
   private void spawnParticles(int amount) {
      ...
      if (i != -1 && amount > 0) {
         for (int j = 0; j < amount; j++) {
            this.getWorld()
               .addParticle(
                  EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, i),
   ...
   protected void onHit(LivingEntity target) {
      ...
      if (potionContentsComponent.potion().isPresent()) {
         for (StatusEffectInstance statusEffectInstance : potionContentsComponent.potion().get().value().getEffects()) {
