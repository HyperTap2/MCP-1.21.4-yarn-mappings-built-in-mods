...
public class EnderDragonEntity extends MobEntity implements Monster {
   ...
   public void tickMovement() {
      ...
      if (this.isDead()) {
         ...
         this.getWorld().addParticle(ParticleTypes.EXPLOSION, this.getX() + f, this.getY() + 2.0 + g, this.getZ() + h, 0.0, 0.0, 0.0);
      } else {
         ...
         if (this.isAiDisabled()) {
            ...
            if (this.getWorld() instanceof ServerWorld serverWorld2) {
               ...
               if (vec3d2 != null) {
                  ...
                  if (Math.abs(d) > 1.0E-5F || Math.abs(i) > 1.0E-5F) {
                     float n = MathHelper.clamp(
                        MathHelper.wrapDegrees(180.0F - (float)MathHelper.atan2(d, i) * (180.0F / (float)Math.PI) - this.getYaw()), -50.0F, 50.0F
                     );
            ...
            for (int aa = 0; aa < 3; aa++) {
               ...
               float m = MathHelper.cos(ab);
               float n = 1.5F;
               float o = (aa + 1) * 2.0F;
               this.movePart(enderDragonPart, -(v * 1.5F + ac * o) * s, frame2.y() - frame.y() - (o + 1.5F) * t + 1.5, (w * 1.5F + m * o) * s);
            }
   ...
   protected void updatePostDeath() {
      ...
      if (this.ticksSinceDeath >= 180 && this.ticksSinceDeath <= 200) {
         ...
         float g = (this.random.nextFloat() - 0.5F) * 4.0F;
         float h = (this.random.nextFloat() - 0.5F) * 8.0F;
         this.getWorld().addParticle(ParticleTypes.EXPLOSION_EMITTER, this.getX() + f, this.getY() + 2.0 + g, this.getZ() + h, 0.0, 0.0, 0.0);
      }
   ...
   public int getNearestPathNodeIndex() {
      if (this.pathNodes[0] == null) {
         for (int i = 0; i < 24; i++) {
            ...
            int m;
            if (i < 12) {
               l = MathHelper.floor(60.0F * MathHelper.cos(2.0F * ((float) -Math.PI + (float) (Math.PI / 12) * k)));
               m = MathHelper.floor(60.0F * MathHelper.sin(2.0F * ((float) -Math.PI + (float) (Math.PI / 12) * k)));
            } else if (i < 20) {
               k -= 12;
               ...
               k -= 20;
               l = MathHelper.floor(20.0F * MathHelper.cos(2.0F * ((float) -Math.PI + (float) (Math.PI / 4) * k)));
               m = MathHelper.floor(20.0F * MathHelper.sin(2.0F * ((float) -Math.PI + (float) (Math.PI / 4) * k)));
            }
   ...
   public void crystalDestroyed(ServerWorld world, EndCrystalEntity crystal, BlockPos pos, DamageSource source) {
      ...
      if (source.getAttacker() instanceof PlayerEntity) {
         ...
         playerEntity = world.getClosestPlayer(CLOSE_PLAYER_PREDICATE, pos.getX(), pos.getY(), pos.getZ());
      }
