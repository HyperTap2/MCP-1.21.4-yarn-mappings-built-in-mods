...
public class EnderDragonFight {
   ...
   public EnderDragonFight(ServerWorld world, long gatewaysSeed, EnderDragonFight.Data data, BlockPos origin) {
      ...
      this.endPortalPattern = BlockPatternBuilder.start()
         ...
         .build();
      ((BlockPatternExtended)this.endPortalPattern).lithium$setRequiredBlock(Blocks.BEDROCK, 41);
   }
   ...
   public void dragonKilled(EnderDragonEntity dragon) {
      if (dragon.getUuid().equals(this.dragonUuid)) {
         ...
         if (!this.previouslyKilled) {
            this.world
               .setBlockState(
                  this.world.getTopPosition(Heightmap.Type.MOTION_BLOCKING, EndPortalFeature.offsetOrigin(this.origin)), Blocks.DRAGON_EGG.getDefaultState()
               );
         }
   ...
   private void generateEndGateway(BlockPos pos) {
      ...
      this.world
         ...
         .ifPresent(reference -> reference.value().generate(this.world, this.world.getChunkManager().getChunkGenerator(), Random.create(), pos));
   }
   ...
   private EnderDragonEntity createDragon() {
      ...
      if (enderDragonEntity != null) {
         ...
         enderDragonEntity.refreshPositionAndAngles(
            this.origin.getX(), 128 + this.origin.getY(), this.origin.getZ(), this.world.random.nextFloat() * 360.0F, 0.0F
         );
