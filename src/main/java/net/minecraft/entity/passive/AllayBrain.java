...
public class AllayBrain {
   ...
   private static void addIdleActivities(Brain<AllayEntity> brain) {
      brain.setTaskList(
         ...
         ImmutableList.of(
            ...
            Pair.of(1, new GiveInventoryToLookTargetTask((allay) -> getLookTarget((LivingEntity)allay), 2.25F, 20)),
            Pair.of(2, WalkTowardsLookTargetTask.create((allay) -> getLookTarget((LivingEntity)allay), Predicate.not(AllayBrain::hasNearestVisibleWantedItem), 4, 16, 2.25F)),
            Pair.of(3, LookAtMobWithIntervalTask.follow(6.0F, UniformIntProvider.create(30, 60))),
            Pair.of(
               4,
               new RandomTask(
                  ImmutableList.of(
                     Pair.of(StrollTask.createSolidTargeting(1.0F), 2), Pair.of(GoToLookTargetTask.create(1.0F, 3), 2), Pair.of(new WaitTask(30, 60), 1)
                  )
               )
            )
         ),
   ...
   private static boolean shouldGoTowardsNoteBlock(LivingEntity allay, Brain<?> brain, GlobalPos pos) {
      ...
      return pos.isWithinRange(world.getRegistryKey(), allay.getBlockPos(), 1024)
         && world.getBlockState(pos.pos()).isOf(Blocks.NOTE_BLOCK)
         && optional.isPresent();
   }
   ...
   public static Optional<ServerPlayerEntity> getLikedPlayer(LivingEntity allay) {
      ...
      if (!world.isClient() && world instanceof ServerWorld serverWorld) {
         ...
         if (optional.isPresent()) {
            if (serverWorld.getEntity(optional.get()) instanceof ServerPlayerEntity serverPlayerEntity
