...
public abstract class AbstractHorseEntity extends AnimalEntity implements InventoryChangedListener, RideableInventory, Tameable, JumpingMount, Saddleable {
   ...
   public float lastAngryAnimationProgress;
   ...
   public boolean handleFallDamage(float fallDistance, float damageMultiplier, DamageSource damageSource) {
      ...
      if (this.hasPassengers()) {
         for (Entity entity : this.getPassengersDeep()) {
            entity.serverDamage(damageSource, i);
         }
      }
   ...
   public ActionResult interactHorse(PlayerEntity player, ItemStack stack) {
      boolean bl = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20_2) || this.receiveFood(player, stack);
      if (bl) {
         stack.decrementUnlessCreative(1, player);
      }

      return !bl && !this.getWorld().isClient ? ActionResult.PASS : ActionResult.SUCCESS_SERVER;
   }
   ...
   public LivingEntity getControllingPassenger() {
      return this.isSaddled() && this.getFirstPassenger() instanceof PlayerEntity playerEntity ? playerEntity : super.getControllingPassenger();
   }
