...
public class RabbitEntity extends AnimalEntity implements VariantHolder<RabbitEntity.RabbitType> {
   ...
   public float getJumpProgress(float delta) {
      return this.jumpDuration == 0 ? 0.0F : (this.jumpTicks + delta) / this.jumpDuration;
   }
   ...
   public RabbitEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
      ...
      if (rabbitEntity != null) {
         RabbitEntity.RabbitType rabbitType = getTypeFromPos(serverWorld, this.getBlockPos());
         ...
         if (this.hasReached()) {
            ...
            if (this.hasTarget && block instanceof CarrotsBlock) {
               ...
               if (i == 0) {
                  ...
                  world.setBlockState(blockPos, blockState.with(CarrotsBlock.AGE, i - 1), 2);
   ...
   public enum RabbitType implements StringIdentifiable {
      ...
      RabbitType(final int id, final String name) {
         this.id = id;
         this.name = name;
      }
