...
public class VillagerEntity extends MerchantEntity implements InteractionObserver, VillagerDataContainer {
   ...
   protected Text getDefaultName() {
      return Text.translatable(
         this.getType().getTranslationKey() + "." + Registries.VILLAGER_PROFESSION.getId(this.getVillagerData().getProfession()).getPath()
      );
   }
   ...
   protected void fillRecipes() {
      ...
      if (this.getWorld().getEnabledFeatures().contains(FeatureFlags.TRADE_REBALANCE)) {
         Int2ObjectMap<TradeOffers.Factory[]> int2ObjectMap = TradeOffers.REBALANCED_PROFESSION_TO_LEVELED_TRADE.get(villagerData.getProfession());
         int2ObjectMap2 = int2ObjectMap != null ? int2ObjectMap : TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(villagerData.getProfession());
      } else {
         int2ObjectMap2 = TradeOffers.PROFESSION_TO_LEVELED_TRADE.get(villagerData.getProfession());
      }
