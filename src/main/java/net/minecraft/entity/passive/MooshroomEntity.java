...
public class MooshroomEntity extends CowEntity implements Shearable, VariantHolder<MooshroomEntity.Type> {
   ...
   public ActionResult interactMob(PlayerEntity player, Hand hand) {
      ...
      if (itemStack.isOf(Items.BOWL) && !this.isBaby()) {
         ...
         return ActionResult.SUCCESS;
      } else if (this.getVariant() == MooshroomEntity.Type.BROWN) {
         if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_21_2) && !itemStack.isIn(ItemTags.SMALL_FLOWERS)) {
