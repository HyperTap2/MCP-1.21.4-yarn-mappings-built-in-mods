...
public class CamelEntity extends AbstractHorseEntity {
   ...
   public void onPassengerLookAround(Entity passenger) {
      if (ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20) && this.getControllingPassenger() != passenger) {
         this.clampLegacyPassengerYaw(passenger);
      }
   }
   ...
   private void clampLegacyPassengerYaw(Entity passenger) {
      passenger.setBodyYaw(this.getYaw());
      float passengerYaw = passenger.getYaw();
      float deltaDegrees = MathHelper.wrapDegrees(passengerYaw - this.getYaw());
      float clampedDelta = MathHelper.clamp(deltaDegrees, -160.0F, 160.0F);
      passenger.prevYaw += clampedDelta - deltaDegrees;
      float newYaw = passengerYaw + clampedDelta - deltaDegrees;
   ...
   public double getPassengerAttachmentY(boolean primaryPassenger, float tickDelta, EntityDimensions dimensions, float scaleFactor) {
      ...
      if (bl) {
         ...
         float l = MathHelper.clamp((float)this.getLastPoseTickDelta() + tickDelta, 0.0F, i);
         boolean bl3 = l < j;
   ...
   public Vec3d getLeashOffset(float tickDelta) {
      ...
      return new Vec3d(0.0, this.getPassengerAttachmentY(true, tickDelta, entityDimensions, f) - 0.2F * f, entityDimensions.width() * 0.56F);
   }
