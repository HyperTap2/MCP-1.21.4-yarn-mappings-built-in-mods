...
public class WolfEntity extends TameableEntity implements Angerable, VariantHolder<RegistryEntry<WolfVariant>> {
   ...
   protected void initDataTracker(DataTracker.Builder builder) {
      ...
      builder.add(VARIANT, registry.getOptional(WolfVariants.DEFAULT).or(registry::getDefaultEntry).orElseThrow());
   ...
   public void readCustomDataFromNbt(NbtCompound nbt) {
      ...
      Optional.ofNullable(Identifier.tryParse(nbt.getString("variant")))
         ...
         .flatMap(variantKey -> this.getRegistryManager().getOrThrow(RegistryKeys.WOLF_VARIANT).getOptional((RegistryKey<WolfVariant>)variantKey))
   ...
   protected SoundEvent getAmbientSound() {
      if (this.hasAngerTime()) {
         ...
         return this.isTamed() && this.viaFabricPlus$getHealth() < 20.0F ? SoundEvents.ENTITY_WOLF_WHINE : SoundEvents.ENTITY_WOLF_PANT;
      } else {
         ...
         if (this.isWet()) {
            ...
            if (this.shakeProgress > 0.4F) {
               ...
               for (int j = 0; j < i; j++) {
                  ...
                  this.getWorld().addParticle(ParticleTypes.SPLASH, this.getX() + g, f + 0.8F, this.getZ() + h, vec3d.x, vec3d.y, vec3d.z);
               }
            }
         }
      }
   ...
   public ActionResult interactMob(PlayerEntity player, Hand hand) {
      ...
      if (this.isTamed()) {
         if (this.isBreedingItem(itemStack) && this.viaFabricPlus$getHealth() < this.getMaxHealth()) {
   ...
   public float getTailAngle() {
      if (this.hasAngerTime()) {
         ...
         float g = (f - this.viaFabricPlus$getHealth()) / f;
   ...
   private float viaFabricPlus$getHealth() {
      return ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_14_4)
