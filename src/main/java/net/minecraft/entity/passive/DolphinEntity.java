...
public class DolphinEntity extends WaterAnimalEntity {
   ...
   public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
      ...
      this.setPitch(0.0F);
      EntityData entityData2 = Objects.requireNonNullElseGet(entityData, () -> new PassiveEntity.PassiveData(0.1F));
   ...
   public void tick() {
      ...
      if (this.isAiDisabled()) {
         ...
         if (this.getWorld().isClient && this.isTouchingWater() && this.getVelocity().lengthSquared() > 0.03) {
            ...
            for (int i = 0; i < 2; i++) {
               this.getWorld()
                  .addParticle(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * h + f, this.getY() - vec3d.y, this.getZ() - vec3d.z * h + g, 0.0, 0.0, 0.0);
               this.getWorld()
                  .addParticle(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * h - f, this.getY() - vec3d.y, this.getZ() - vec3d.z * h - g, 0.0, 0.0, 0.0);
            }
         }
   ...
   static class LeadToNearbyTreasureGoal extends Goal {
      ...
      public boolean shouldContinue() {
         ...
         return !BlockPos.ofFloored(blockPos.getX(), this.dolphin.getY(), blockPos.getZ()).isWithinDistance(this.dolphin.getPos(), 4.0)
