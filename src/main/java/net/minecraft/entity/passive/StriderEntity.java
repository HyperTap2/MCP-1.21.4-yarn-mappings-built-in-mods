...
public class StriderEntity extends AnimalEntity implements ItemSteerable, Saddleable {
   ...
   public LivingEntity getControllingPassenger() {
      return this.isSaddled() && this.getFirstPassenger() instanceof PlayerEntity playerEntity && playerEntity.isHolding(Items.WARPED_FUNGUS_ON_A_STICK)
         ? playerEntity
   ...
   protected float getSaddledSpeed(PlayerEntity controllingPlayer) {
      return (float)(
         this.getAttributeValue(EntityAttributes.MOVEMENT_SPEED) * (this.isCold() ? 0.35F : 0.55F) * this.saddledComponent.getMovementSpeedMultiplier()
      );
   }
   ...
   public void tick() {
      ...
      if (!this.isAiDisabled()) {
         ...
         boolean bl = blockState.isIn(BlockTags.STRIDER_WARM_BLOCKS)
            || blockState2.isIn(BlockTags.STRIDER_WARM_BLOCKS)
   ...
   static class Navigation extends MobNavigation {
      ...
      protected boolean canWalkOnPath(PathNodeType pathType) {
         return pathType != PathNodeType.LAVA && pathType != PathNodeType.DAMAGE_FIRE && pathType != PathNodeType.DANGER_FIRE
            ? super.canWalkOnPath(pathType)
            : true;
      }
