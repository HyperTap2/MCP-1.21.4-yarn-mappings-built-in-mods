         ...
         if (!this.getWorld().isClient()) {
            if (this.isBaby()) {
               ...
               if (this.getBrain().hasMemoryModule(MemoryModuleType.NEAREST_VISIBLE_ADULT)) {
                  this.getBrain().forget(MemoryModuleType.NEAREST_VISIBLE_ADULT);
               }
            }
         }
      ...
      if (this.getWorld().isClient) {
         if (this.happyTicksRemaining > 0) {
            if (this.happyTicksRemaining % 4 == 0) {
               this.getWorld()
                  .addParticle(ParticleTypes.HAPPY_VILLAGER, this.getParticleX(1.0), this.getRandomBodyY() + 0.5, this.getParticleZ(1.0), 0.0, 0.0, 0.0);
            }
