...
public record EquipmentTable(RegistryKey<LootTable> lootTable, Map<EquipmentSlot, Float> slotDropChances) {
   public static final Codec<Map<EquipmentSlot, Float>> SLOT_DROP_CHANCES_CODEC = Codec.either(
      ...
      .xmap(either -> (Map)either.map(EquipmentTable::createSlotDropChances, Function.identity()), map -> {
         boolean bl = map.values().stream().distinct().count() == 1L;
         boolean bl2 = map.keySet().containsAll(EquipmentSlot.VALUES);
         return bl && bl2 ? Either.left(map.values().stream().findFirst().orElse(0.0F)) : Either.right(map);
      });
