...
public class ArmorStandEntity extends LivingEntity {
   ...
   public void readCustomDataFromNbt(NbtCompound nbt) {
      ...
      if (nbt.contains("HandItems", 9)) {
         ...
         for (int i = 0; i < this.heldItems.size(); i++) {
            NbtCompound nbtCompound = nbtList.getCompound(i);
            ItemStack newStack = ItemStack.fromNbtOrEmpty(this.getRegistryManager(), nbtCompound);
            this.lithium$trackDirectEquipmentReplacement(this.heldItems.set(i, newStack), newStack);
         }
      }
   ...
   protected void tickCramming() {
      for (Entity entity : this.getWorld().getEntitiesByClass(
         AbstractMinecartEntity.class, this.getBoundingBox(), AbstractMinecartEntity::isRideable
      )) {
   ...
   private void onBreak(ServerWorld world, DamageSource damageSource) {
      ...
      for (int i = 0; i < this.heldItems.size(); i++) {
         ...
         if (!itemStack.isEmpty()) {
            ...
            this.lithium$trackDirectEquipmentReplacement(this.heldItems.set(i, ItemStack.EMPTY), ItemStack.EMPTY);
         }
      }
