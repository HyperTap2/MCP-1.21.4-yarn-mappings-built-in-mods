...
public class PackratParsing {
   public static <T, C, P> ArgumentParser<List<T>> createParser(PackratParsing.Callbacks<T, C, P> callbacks) {
      ...
      parsingRules.set(symbol6, Term.sequence(Term.symbol(symbol7), Term.optional(Term.sequence(Literals.character(','), Term.symbol(symbol6)))), results -> {
         ...
         return Optional.ofNullable(results.get(symbol6)).map(predicates -> Util.withPrepended(object, (List<T>)predicates)).orElse(List.of(object));
      });
      parsingRules.set(symbol7, Term.sequence(Term.symbol(symbol8), Term.optional(Term.sequence(Literals.character('|'), Term.symbol(symbol7)))), results -> {
         ...
         return Optional.ofNullable(results.get(symbol7)).map(predicates -> Util.withPrepended(object, (List<T>)predicates)).orElse(List.of(object));
      });
      parsingRules.set(
         symbol8,
         ...
         (state, results) -> {
            ...
            try {
               if (object != null) {
                  ...
                  return Optional.of(
                     nbtElement2 != null
                        ? callbacks.componentMatchPredicate((ImmutableStringReader)state.getReader(), object2, nbtElement2)
                        : callbacks.componentPresencePredicate((ImmutableStringReader)state.getReader(), object2)
                  );
               }
