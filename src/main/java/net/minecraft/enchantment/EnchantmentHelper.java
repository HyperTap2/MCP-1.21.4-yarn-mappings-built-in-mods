...
public class EnchantmentHelper {
   ...
   public static int getMobExperience(ServerWorld world, @Nullable Entity attacker, Entity mob, int baseMobExperience) {
      if (attacker instanceof LivingEntity livingEntity) {
         ...
         forEachEnchantment(
            livingEntity, (enchantment, level, context) -> enchantment.value().modifyMobExperience(world, level, context.stack(), mob, mutableFloat)
         );
         return mutableFloat.intValue();
      } else {
         ...
         if (damageSource.getAttacker() instanceof LivingEntity livingEntity) {
            forEachEnchantment(
               ...
               (enchantment, level, context) -> enchantment.value()
                  .onTargetDamaged(world, level, context, EnchantmentEffectTarget.ATTACKER, target, damageSource)
            );
   ...
   public static <T> Pair<T, Integer> getHighestLevelEffect(ItemStack stack, ComponentType<T> componentType) {
      ...
      return (Pair<T, Integer>)mutableObject.getValue();
   }
   ...
   public static ItemStack enchant(
      ...
      return enchant(
         ...
         enchantments.map(RegistryEntryList::stream)
            .orElseGet(
               () -> dynamicRegistryManager.getOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().map(reference -> (RegistryEntry<Enchantment>)reference)
            )
      );
   }
