...
public class ChunkHeightAndBiomeFix extends DataFix {
      ...
      return this.fixTypeEverywhereTyped(
         ...
         chunk -> chunk.updateTyped(
            ...
            level -> {
               ...
               OptionalDynamic<?> optionalDynamic = ((Dynamic)chunk.get(DSL.remainderFinder())).get("__context");
               ...
               MutableObject<Supplier<ProtoChunkTickListFix.PalettedSection>> mutableObject = new MutableObject(
               ...
               level = level.updateTyped(opticFinder2, type4, sections -> {
                  ...
                  List<Dynamic<?>> list = dynamic3.asStream().map(dynamic2xx -> {
                     ...
                     Dynamic<?> dynamic3x = (Dynamic<?>)DataFixUtils.orElse(dynamic2xx.get("Palette").result().flatMap(dynamic2xxx -> {
                     ...
                     if (kx >= 0 && kx < dynamics.length) {
                        dynamic4x = dynamic4x.set("biomes", dynamics[kx]);
                     }

                     intSet.add(jx);
                     if (dynamic2xx.get("Y").asInt(Integer.MAX_VALUE) == 0) {
                        mutableObject.setValue((Supplier<ProtoChunkTickListFix.PalettedSection>)() -> {
               ...
               return level.update(
                  ...
                  level2 -> {
                     ...
                     return fixLevel(
                        ...
                        (Supplier<ProtoChunkTickListFix.PalettedSection>)mutableObject.getValue()
                     );
                  }
               );
            }
   ...
   private static Dynamic<?>[] fixBiomes(Dynamic<?> level, boolean overworld, int i, MutableBoolean heightAlreadyUpdated) {
      ...
      if (is != null && is.length == 1536) {
         ...
         for (int j = 0; j < 16; j++) {
            int k = j - i;
            int l = j;
            dynamics[k] = fixBiomes(level, sectionY -> getClamped(is, l * 64 + sectionY));
         }
   ...
   private static Dynamic<?> fixLevel(
      ...
      if (optional.isPresent()) {
         ...
         if (!"empty".equals(string)) {
            level = level.set(
               "blending_data", level.createMap(ImmutableMap.of(level.createString("old_noise"), level.createBoolean(field_35668.contains(string))))
            );
            ProtoChunkTickListFix.PalettedSection palettedSection = supplier.get();
   ...
   private static Dynamic<?> fixBiomes(Dynamic<?> level, Int2IntFunction biomeGetter) {
      ...
      Dynamic<?> dynamic = level.createList(
         int2IntMap.keySet().stream().map(rawBiomeId -> level.createString((String)RAW_BIOME_IDS.getOrDefault(rawBiomeId, "minecraft:plains")))
      );
