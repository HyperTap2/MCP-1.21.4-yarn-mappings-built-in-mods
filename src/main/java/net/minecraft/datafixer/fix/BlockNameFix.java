...
public abstract class BlockNameFix extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      ...
      TypeRewriteRule typeRewriteRule3 = this.fixTypeEverywhereTyped(
         ...
         typed -> typed.update(
            DSL.remainderFinder(),
            dynamic -> (Dynamic)DataFixUtils.orElse(dynamic.asString().result().map(this::fixFlatBlockState).map(dynamic::createString), dynamic)
         )
      );
   ...
   private Dynamic<?> fixBlockState(Dynamic<?> blockStateDynamic) {
      ...
      return optional.isPresent() ? blockStateDynamic.set("Name", blockStateDynamic.createString(this.rename(optional.get()))) : blockStateDynamic;
   }
