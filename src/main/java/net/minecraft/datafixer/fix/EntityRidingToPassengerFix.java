            ...
            dynamicOps -> pair -> {
               ...
               while (true) {
                  Either<List<NewEntityTree>, Unit> either = DataFixUtils.orElse(
                     optional.map(
                        pairx -> {
                           Typed<NewEntityTree> typed = outputEntityTreeType.pointTyped(dynamicOps)
                              .orElseThrow(() -> new IllegalStateException("Could not create new entity tree"));
                           NewEntityTree object = typed.set(opticFinder2, pairx)
                  ...
                  pair2 = new Typed<>(inputEntityTreeType, dynamicOps, optional2.get())
