...
public class AttributeIdFix extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return TypeRewriteRule.seq(
         this.fixTypeEverywhereTyped(
            ...
            type,
            typed -> typed.updateTyped(opticFinder, typedx -> typedx.update(DSL.remainderFinder(), AttributeIdFix::fixItemStack))
         ),
         new TypeRewriteRule[]{
   ...
   private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> dynamicStream) {
      ...
      dynamicStream.forEach(modifierDynamic -> {
         ...
         if (string2 != null) {
            ...
            Dynamic<?> dynamic = map.get(string3);
            if (dynamic == null) {
   ...
   private static Dynamic<?> fixItemStack(Dynamic<?> stackDataDynamic) {
      return stackDataDynamic.update(
         ...
         attributeModifiersDynamic -> attributeModifiersDynamic.update(
            "modifiers",
            modifiersDynamic -> (Dynamic)DataFixUtils.orElse(
      ...
      return entityTyped.update(
         ...
         remainder -> remainder.renameAndFixField(
            ...
            attributesDynamic -> (Dynamic)DataFixUtils.orElse(
               attributesDynamic.asStreamOpt().result().map(stream -> stream.map(AttributeIdFix::fixAttribute)).map(attributesDynamic::createList),
               attributesDynamic
            )
         )
      );
   ...
   public static UUID getUuidFromIntArray(int[] uuidArray) {
      return uuidArray.length != 4
         ? null
         : new UUID((long)uuidArray[0] << 32 | uuidArray[1] & 4294967295L, (long)uuidArray[2] << 32 | uuidArray[3] & 4294967295L);
   }
}
