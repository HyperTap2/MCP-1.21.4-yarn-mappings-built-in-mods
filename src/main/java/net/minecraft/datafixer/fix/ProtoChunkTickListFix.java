...
public class ProtoChunkTickListFix extends DataFix {
            ...
            levelTyped -> {
               levelTyped = levelTyped.update(
                  ...
                  levelDynamic -> (Dynamic)DataFixUtils.orElse(
                     levelDynamic.get("LiquidTicks")
                        .result()
                        .map(liquidTicksDynamic -> levelDynamic.set("fluid_ticks", liquidTicksDynamic).remove("LiquidTicks")),
                     levelDynamic
                  )
               );
               ...
               levelTyped.getOptionalTyped(opticFinder2)
                  .ifPresent(
                     sectionsTyped -> sectionsTyped.getAllTyped(opticFinder3)
                        .forEach(
                           sectionTyped -> {
                              Dynamic<?> dynamicx = (Dynamic<?>)sectionTyped.get(DSL.remainderFinder());
               ...
               if (!levelTyped.getOptionalTyped(opticFinder7).isPresent() && !dynamic.get("fluid_ticks").result().isPresent()) {
                  ...
                  Dynamic<?> dynamic2 = this.fixToBeTicked(
                     dynamic, int2ObjectMap, b, i, j, "LiquidsToBeTicked", ProtoChunkTickListFix::getFluidBlockIdToBeTicked
                  );
   ...
   private Dynamic<?> fixToBeTicked(
      ...
      for (int i = 0; i < list.size(); i++) {
         int j = i + sectionY;
         Supplier<ProtoChunkTickListFix.PalettedSection> supplier = (Supplier<ProtoChunkTickListFix.PalettedSection>)palettedSectionsByY.get(j);
