...
public class EntityUuidFix extends AbstractUuidFix {
   ...
   private static Dynamic<?> updateAngryAtMemory(Dynamic<?> dynamic) {
      return dynamic.update(
         ...
         dynamicx -> dynamicx.update(
            "memories", dynamicxx -> dynamicxx.update("minecraft:angry_at", dynamicxxx -> updateStringUuid(dynamicxxx, "value", "value").orElseGet(() -> {
   ...
   private static Dynamic<?> updateShulkerBullet(Dynamic<?> dynamic) {
      dynamic = updateCompoundUuid(dynamic, "Owner", "Owner").orElse(dynamic);
      return updateCompoundUuid(dynamic, "Target", "Target").orElse(dynamic);
   }

   private static Dynamic<?> updateItemEntity(Dynamic<?> dynamic) {
      dynamic = updateCompoundUuid(dynamic, "Owner", "Owner").orElse(dynamic);
      return updateCompoundUuid(dynamic, "Thrower", "Thrower").orElse(dynamic);
   }

   private static Dynamic<?> updateFox(Dynamic<?> dynamic) {
      Optional<Dynamic<?>> optional = dynamic.get("TrustedUUIDs")
         .result()
         .map(dynamic2 -> dynamic.createList(dynamic2.asStream().map(dynamicxx -> createArrayFromCompoundUuid(dynamicxx).orElseGet(() -> {
   ...
   private static Dynamic<?> updateTameable(Dynamic<?> dynamic) {
      ...
      return updateStringUuid(dynamic2, "OwnerUUID", "Owner").orElse(dynamic2);
   }

   private static Dynamic<?> updateBreedable(Dynamic<?> dynamic) {
      Dynamic<?> dynamic2 = updateLeashable(dynamic);
      return updateRegularMostLeast(dynamic2, "LoveCause", "LoveCause").orElse(dynamic2);
   }

   private static Dynamic<?> updateLeashable(Dynamic<?> dynamic) {
      return updateLiving(dynamic).update("Leash", dynamicx -> updateRegularMostLeast(dynamicx, "UUID", "UUID").orElse(dynamicx));
   }
               ...
               .map(
                  dynamicxx -> dynamicxx.update(
                     ...
                     dynamic2x -> dynamicxx.createList(
                        dynamic2x.asStream().map(dynamicxxxx -> updateRegularMostLeast(dynamicxxxx, "UUID", "UUID").orElse(dynamicxxxx))
                     )
                  )
               )
         )
   ...
   public static Dynamic<?> updateSelfUuid(Dynamic<?> entityDynamic) {
      return updateRegularMostLeast(entityDynamic, "UUID", "UUID").orElse(entityDynamic);
   }
