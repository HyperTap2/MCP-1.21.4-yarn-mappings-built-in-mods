      ...
      return this.fixTypeEverywhereTyped(
         ...
         entityTyped -> {
            ...
            if (optional.isPresent()) {
               ...
               IS object = itemStackType.read(dynamic.emptyMap())
                     .result()
                     .orElseThrow(() -> new IllegalStateException("Could not parse newly created empty itemstack."))
                  .getFirst();
               if (!list.isEmpty()) {
                  either = Either.left(Lists.newArrayList(list.getFirst(), object));
               }
            ...
            if (optional2.isPresent()) {
               Iterator<? extends Dynamic<?>> iterator = Stream.concat(
                     (Stream<? extends Dynamic<?>>)optional2.get(), Stream.generate(() -> dynamic2.createInt(0))
                  )
                  .iterator();
               ...
               if (dynamic.get("ArmorDropChances").result().isEmpty()) {
                  Dynamic<?> dynamic3 = dynamic.createList(
                     Stream.of(iterator.next().asFloat(0.0F), iterator.next().asFloat(0.0F), iterator.next().asFloat(0.0F), iterator.next().asFloat(0.0F))
