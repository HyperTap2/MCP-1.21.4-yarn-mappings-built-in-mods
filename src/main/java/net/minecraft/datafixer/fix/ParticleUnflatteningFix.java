...
public class ParticleUnflatteningFix extends DataFix {
   ...
   private static NbtCompound tryParse(String snbt) {
      try {
         ...
         LOGGER.warn("Failed to parse tag: {}", snbt, exception);
         return null;
      }
   }
   ...
   private static <T> Map<Dynamic<T>, Dynamic<T>> parseBlockProperties(Dynamic<T> dynamic, String propertiesStr) {
      try {
         ...
         LOGGER.warn("Failed to parse block properties: {}", propertiesStr, exception);
         return Map.of();
      }
   }
   ...
   private <T> Dynamic<T> fixDustParticle(Dynamic<T> dynamic, String params) {
      try {
         ...
         LOGGER.warn("Failed to parse particle options: {}", params, exception);
         return dynamic;
      }
   }
   ...
   private <T> Dynamic<T> fixVibrationParticle(Dynamic<T> dynamic, String params) {
      try {
         ...
         @SuppressWarnings("unchecked")
          Dynamic<T> dynamic2 = (Dynamic<T>)dynamic.createIntList(IntStream.of(MathHelper.floor(f), MathHelper.floor(g), MathHelper.floor(h)));
