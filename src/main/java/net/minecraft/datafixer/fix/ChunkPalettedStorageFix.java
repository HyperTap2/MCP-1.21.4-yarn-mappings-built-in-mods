...
public class ChunkPalettedStorageFix extends DataFix {
   ...
   public enum Facing {
      ...
      Facing(final ChunkPalettedStorageFix.Facing.Direction direction, final ChunkPalettedStorageFix.Facing.Axis axis) {
         this.axis = axis;
   ...
   static final class Level {
      ...
      public Level(Dynamic<?> chunkTag) {
                  ...
                  blockEntityTag -> {
                     ...
                     if (this.blockEntities.put(l, blockEntityTag) != null) {
                        ChunkPalettedStorageFix.LOGGER
                           .warn("In chunk: {}x{} found a duplicate block entity at position: [{}, {}, {}]", new Object[]{this.x, this.z, ix, jx, k});
                     }
                  }
               )
            );
         ...
         chunkTag.get("Sections").asStreamOpt().ifSuccess(stream -> stream.forEach(sectionTag -> {
            ChunkPalettedStorageFix.Section sectionx = new ChunkPalettedStorageFix.Section((Dynamic<?>)sectionTag);
         ...
         for (ChunkPalettedStorageFix.Section section : this.sections) {
            if (section != null) {
               ObjectIterator var7 = section.inPlaceUpdates.int2ObjectEntrySet().iterator();

               while (var7.hasNext()) {
                  Entry<IntList> entry = (Entry<IntList>)var7.next();
                  int i = section.y << 12;
                  switch (entry.getIntKey()) {
                     ...
                     case 25:
                        ...
                        while (var28.hasNext()) {
                           int j = (Integer)var28.next();
                           j |= i;
                           Dynamic<?> dynamic = this.removeBlockEntity(j);
                           if (dynamic != null) {
                              String string = Boolean.toString(dynamic.get("powered").asBoolean(false))
                                 + (byte)Math.min(Math.max(dynamic.get("note").asInt(0), 0), 24);
                              this.setBlock(
                                 j,
                                 ChunkPalettedStorageFix.Mapping.NOTE_BLOCK_IDS_TO_STATES
                                    .getOrDefault(string, ChunkPalettedStorageFix.Mapping.NOTE_BLOCK_IDS_TO_STATES.get("false0"))
                              );
                           }
                        }
                        break;
                           ...
                           if (ChunkPalettedStorageFix.getName(dynamic).endsWith("_door")) {
                              ...
                              if ("lower".equals(ChunkPalettedStorageFix.getProperty(dynamic2, "half"))) {
                                 ...
                                 if (string4.equals(ChunkPalettedStorageFix.getName(dynamic3))) {
                                    ...
                                    this.setBlock(
                                       j, ChunkPalettedStorageFix.Mapping.DOOR_IDS_TO_STATES.get(string4 + string5 + "lower" + string7 + string6 + string8)
                                    );
                                    this.setBlock(
                                       k, ChunkPalettedStorageFix.Mapping.DOOR_IDS_TO_STATES.get(string4 + string5 + "upper" + string7 + string6 + string8)
                                    );
                                 }
                              }
                           }
                           ...
                           if (dynamic != null) {
                              ...
                              this.setBlock(
                                 ...
                                 ChunkPalettedStorageFix.Mapping.PLANT_TO_FLOWER_POT_STATES
                                    .getOrDefault(string, ChunkPalettedStorageFix.Mapping.PLANT_TO_FLOWER_POT_STATES.get("minecraft:air0"))
                              );
                           }
                        }
                        break;
                           ...
                           if (dynamic != null) {
                              ...
                              this.setBlock(
                                 ...
                                 ChunkPalettedStorageFix.Mapping.SKULL_IDS_TO_STATES
                                    .getOrDefault(string2, ChunkPalettedStorageFix.Mapping.SKULL_IDS_TO_STATES.get("0north"))
                              );
                           }
                        }
                        break;
                     case 175:
                        ...
                        while (var21.hasNext()) {
                           int j = (Integer)var21.next();
                           j |= i;
                           Dynamic<?> dynamic = this.getBlock(j);
                           if ("upper".equals(ChunkPalettedStorageFix.getProperty(dynamic, "half"))) {
                              Dynamic<?> dynamic2 = this.getBlock(adjacentTo(j, ChunkPalettedStorageFix.Facing.DOWN));
   ...
   static class Mapping {
      ...
      static final Map<String, Dynamic<?>> PLANT_TO_FLOWER_POT_STATES = DataFixUtils.make(new HashMap<String, Dynamic<?>>(), map -> {
      ...
      static final Map<String, Dynamic<?>> SKULL_IDS_TO_STATES = DataFixUtils.make(new HashMap<String, Dynamic<?>>(), map -> {
      ...
      static final Map<String, Dynamic<?>> DOOR_IDS_TO_STATES = DataFixUtils.make(new HashMap<String, Dynamic<?>>(), map -> {
         door(map, "oak_door");
         door(map, "iron_door");
      ...
      static final Map<String, Dynamic<?>> NOTE_BLOCK_IDS_TO_STATES = DataFixUtils.make(new HashMap<String, Dynamic<?>>(), map -> {
         for (int i = 0; i < 26; i++) {
      ...
      private static final Int2ObjectMap<String> COLORS_BY_IDS = (Int2ObjectMap<String>)DataFixUtils.make(new Int2ObjectOpenHashMap(), map -> {
         map.put(0, "white");
         map.put(1, "orange");
         map.put(2, "magenta");
      ...
      static final Map<String, Dynamic<?>> BED_IDS_TO_STATES = DataFixUtils.make(new HashMap<String, Dynamic<?>>(), map -> {
         ObjectIterator var1 = COLORS_BY_IDS.int2ObjectEntrySet().iterator();
      ...
      static final Map<String, Dynamic<?>> BANNER_IDS_TO_STATES = DataFixUtils.make(new HashMap<String, Dynamic<?>>(), map -> {
         ObjectIterator var1 = COLORS_BY_IDS.int2ObjectEntrySet().iterator();
   ...
   static class Section {
      ...
      public int visit(int sidesToUpgrade) {
         ...
         ChunkPalettedStorageFix.ChunkNibbleArray chunkNibbleArray = this.section
            .get("Data")
            .asByteBufferOpt()
         ...
         ChunkPalettedStorageFix.ChunkNibbleArray chunkNibbleArray2 = this.section
            .get("Add")
            .asByteBufferOpt()
            .map(byteBufferx -> new ChunkPalettedStorageFix.ChunkNibbleArray(DataFixUtils.toArray(byteBufferx)))
            .result()
