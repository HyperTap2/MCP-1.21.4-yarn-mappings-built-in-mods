...
public class ItemInstanceSpawnEggFix extends DataFix {
   ...
   private static final Map<String, String> ENTITY_SPAWN_EGGS = DataFixUtils.make(Maps.newHashMap(), map -> {
         ...
         stack -> {
            ...
            if (optional.isPresent() && Objects.equals(optional.get().getSecond(), this.spawnEggId)) {
               ...
               if (optional2.isPresent()) {
                  return stack.set(
                     opticFinder, Pair.of(TypeReferences.ITEM_NAME.typeName(), ENTITY_SPAWN_EGGS.getOrDefault(optional2.get(), "minecraft:pig_spawn_egg"))
                  );
               }
            }
