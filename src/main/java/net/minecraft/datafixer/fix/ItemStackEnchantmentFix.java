...
public class ItemStackEnchantmentFix extends DataFix {
   private static final Int2ObjectMap<String> ID_TO_ENCHANTMENTS_MAP = (Int2ObjectMap<String>)DataFixUtils.make(new Int2ObjectOpenHashMap(), map -> {
         ...
         .map(
            enchantments -> enchantments.map(
               enchantment -> enchantment.set(
                  "id", enchantment.createString((String)ID_TO_ENCHANTMENTS_MAP.getOrDefault(enchantment.get("id").asInt(0), "null"))
               )
            )
         )
               ...
               .map(
                  storedEnchantments -> storedEnchantments.map(
                     storedEnchantment -> storedEnchantment.set(
                        "id", storedEnchantment.createString((String)ID_TO_ENCHANTMENTS_MAP.getOrDefault(storedEnchantment.get("id").asInt(0), "null"))
                     )
                  )
               )
