...
public class ItemStackComponentizationFix extends DataFix {
   ...
   private static void fixStack(ItemStackComponentizationFix.StackData data, Dynamic<?> dynamic) {
      ...
      if (data.itemEquals("minecraft:filled_map")) {
         ...
         Map<? extends Dynamic<?>, ? extends Dynamic<?>> map = data.getAndRemove("Decorations")
            .asStream()
      ...
      if (data.itemEquals("minecraft:player_head")) {
         data.getAndRemove("SkullOwner")
            .result()
            .ifPresent(skullOwnerDynamic -> data.setComponent("minecraft:profile", createProfileDynamic((Dynamic<?>)skullOwnerDynamic)));
      }
   }
   ...
   private static void fixEnchantments(
      ...
      List<Pair<String, Integer>> list = optionalDynamic.asList(Function.identity())
         .stream()
         .flatMap(enchantmentsDynamic -> getEnchantmentAndLevelPair((Dynamic<?>)enchantmentsDynamic).stream())
         .toList();
   ...
   private static Optional<Pair<String, Integer>> getEnchantmentAndLevelPair(Dynamic<?> dynamic) {
      return dynamic.get("id")
         .asString()
         .apply2stable((enchantmentId, level) -> Pair.of(enchantmentId, MathHelper.clamp(level.intValue(), 0, 255)), dynamic.get("lvl").asNumber())
         .result();
   }
                  ...
                  optional.get()
                     ...
                     .map(
                        predicatesDynamic -> DataFixUtils.orElse(
                           predicatesDynamic.asString().map(string -> createBlockPredicateListDynamic(predicatesDynamic, string)).result(),
                           predicatesDynamic
                        )
                     )
               )
   ...
   private static Pair<Dynamic<?>, Dynamic<?>> fixMapDecorations(Dynamic<?> dynamic) {
      Dynamic<?> dynamic2 = (Dynamic<?>)DataFixUtils.orElseGet(dynamic.get("id").result(), () -> dynamic.createString(""));
      Dynamic<?> dynamic3 = dynamic.emptyMap()
   ...
   static class StackData {
      ...
      public Dynamic<?> finalizeDynamic() {
