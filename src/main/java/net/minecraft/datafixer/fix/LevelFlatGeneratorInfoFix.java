...
public class LevelFlatGeneratorInfoFix extends DataFix {
   ...
   private Dynamic<?> fixGeneratorOptions(Dynamic<?> levelDynamic) {
      return levelDynamic.get("generatorName").asString("").equalsIgnoreCase("flat")
         ? levelDynamic.update(
            "generatorOptions",
            generatorOptionsDynamic -> (Dynamic)DataFixUtils.orElse(
               generatorOptionsDynamic.asString().map(this::fixFlatGeneratorOptions).map(generatorOptionsDynamic::createString).result(),
               generatorOptionsDynamic
            )
         )
         : levelDynamic;
   }
   ...
   String fixFlatGeneratorOptions(String generatorOptions) {
      ...
      if (i >= 0 && i <= 3) {
         ...
         stringBuilder.append(StreamSupport.<String>stream(SPLIT_ON_COMMA.split(string2).spliterator(), false).map(stringx -> {
