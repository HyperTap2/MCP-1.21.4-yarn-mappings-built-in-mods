...
public class ChunkStructuresTemplateRenameFix extends DataFix {
   private static final ImmutableMap<String, Pair<String, ImmutableMap<String, String>>> STRUCTURES = ImmutableMap.<String,
                   Pair<String, ImmutableMap<String, String>>>builder()
      .put(
         "EndCity",
         Pair.of(
            "ECP",
            ImmutableMap.<String, String>builder().put("second_floor", "second_floor_1").put("third_floor", "third_floor_1").put("third_floor_c", "third_floor_2").build()
         )
      )
      .put(
         "Mansion",
   ...
   private Dynamic<?> fixChildren(Dynamic<?> structureFeatureDynamic) {
      return structureFeatureDynamic.update(
         ...
         childrenDynamic -> structureFeatureDynamic.createList(
            childrenDynamic.asStream().map(childDynamic -> this.fix(structureFeatureDynamic, (Dynamic<?>)childDynamic))
         )
      );
   }
   ...
   private Dynamic<?> fix(Dynamic<?> structureFeatureDynamic, Dynamic<?> childDynamic) {
      ...
      if (STRUCTURES.containsKey(string)) {
         Pair<String, ImmutableMap<String, String>> pair = (Pair<String, ImmutableMap<String, String>>)STRUCTURES.get(string);
         if (((String)pair.getFirst()).equals(childDynamic.get("id").asString(""))) {
            String string2 = childDynamic.get("Template").asString("");
            childDynamic = childDynamic.set("Template", childDynamic.createString((String)((ImmutableMap)pair.getSecond()).getOrDefault(string2, string2)));
         }
      }
