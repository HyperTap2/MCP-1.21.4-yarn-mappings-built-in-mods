...
public class Schema99 extends Schema {
   ...
   public Map<String, Supplier<TypeTemplate>> registerEntities(Schema schema) {
      ...
      schema.register(map, "Arrow", name -> DSL.optionalFields("inTile", TypeReferences.BLOCK_NAME.in(schema)));
      schema.register(map, "TippedArrow", name -> DSL.optionalFields("inTile", TypeReferences.BLOCK_NAME.in(schema)));
      schema.register(map, "SpectralArrow", name -> DSL.optionalFields("inTile", TypeReferences.BLOCK_NAME.in(schema)));
      ...
      schema.register(
         map, "ThrownPotion", name -> DSL.optionalFields("inTile", TypeReferences.BLOCK_NAME.in(schema), "Potion", TypeReferences.ITEM_STACK.in(schema))
      );
      targetInTile(schema, map, "ThrownExpBottle");
      schema.register(map, "ItemFrame", name -> DSL.optionalFields("Item", TypeReferences.ITEM_STACK.in(schema)));
      ...
      schema.register(map, "FireworksRocketEntity", name -> DSL.optionalFields("FireworksItem", TypeReferences.ITEM_STACK.in(schema)));
      schema.registerSimple(map, "Boat");
      schema.register(
         map,
         "Minecart",
         () -> DSL.optionalFields("DisplayTile", TypeReferences.BLOCK_NAME.in(schema), "Items", DSL.list(TypeReferences.ITEM_STACK.in(schema)))
      );
      ...
      schema.register(map, "Enderman", name -> DSL.optionalFields("carried", TypeReferences.BLOCK_NAME.in(schema), targetEquipment(schema)));
      ...
      schema.register(map, "AreaEffectCloud", string -> DSL.optionalFields("Particle", TypeReferences.PARTICLE.in(schema)));
   ...
   public Map<String, Supplier<TypeTemplate>> registerBlockEntities(Schema schema) {
      ...
      schema.register(map, "RecordPlayer", name -> DSL.optionalFields("RecordItem", TypeReferences.ITEM_STACK.in(schema)));
      targetItems(schema, map, "Trap");
      targetItems(schema, map, "Dropper");
      schema.registerSimple(map, "Sign");
      schema.register(map, "MobSpawner", name -> TypeReferences.UNTAGGED_SPAWNER.in(schema));
      ...
      schema.register(map, "FlowerPot", name -> DSL.optionalFields("Item", DSL.or(DSL.constType(DSL.intType()), TypeReferences.ITEM_NAME.in(schema))));
