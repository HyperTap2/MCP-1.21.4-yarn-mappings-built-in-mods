...
public class BrushItem extends Item {
         ...
         if (hitResult instanceof BlockHitResult blockHitResult && hitResult.getType() == HitResult.Type.BLOCK) {
            ...
            if (bl) {
               ...
               if (world instanceof ServerWorld serverWorld && world.getBlockEntity(blockPos) instanceof BrushableBlockEntity brushableBlockEntity) {
                  ...
                  if (bl2) {
                     EquipmentSlot equipmentSlot = stack.equals(playerEntity.getEquippedStack(EquipmentSlot.OFFHAND))
   ...
   private HitResult getHitResult(PlayerEntity user) {
      double interactionRange = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20_2)
         ? 5.0
         : user.getBlockInteractionRange();
      return ProjectileUtil.getCollision(user, EntityPredicates.CAN_HIT, interactionRange);
   }
