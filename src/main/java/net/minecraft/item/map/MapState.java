...
public class MapState extends PersistentState {
   ...
   public static MapState fromNbt(NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
      ...
      for (MapBannerMarker mapBannerMarker : MapBannerMarker.LIST_CODEC
      ...
      for (int k = 0; k < nbtList.size(); k++) {
         ...
         if (mapFrameMarker != null) {
            ...
            mapState.addDecoration(
               ...
               mapFrameMarker.getPos().getX(),
               mapFrameMarker.getPos().getZ(),
               mapFrameMarker.getRotation(),
               null
            );
         }
      }
      ...
      for (int i = 0; i < this.updateTrackers.size(); i++) {
         ...
         if (!playerEntity.isRemoved() && (playerEntity.getInventory().contains(predicate) || stack.isInFrame())) {
            if (!stack.isInFrame() && playerEntity.getWorld().getRegistryKey() == this.dimension && this.showDecorations) {
               this.addDecoration(
                  MapDecorationTypes.PLAYER, playerEntity.getWorld(), string, playerEntity.getX(), playerEntity.getZ(), playerEntity.getYaw(), null
               );
            }
         } else {
   ...
   private MapState.Marker getMarker(RegistryEntry<MapDecorationType> type, @Nullable WorldAccess world, double rotation, float dx, float dz) {
      ...
      if (type.matches(MapDecorationTypes.PLAYER)) {
         ...
         return pair == null ? null : new MapState.Marker((RegistryEntry<MapDecorationType>)pair.getFirst(), b, c, (Byte)pair.getSecond());
      } else {
