   ...
   public ActionResult use(World world, PlayerEntity user, Hand hand) {
      ...
      if (this.fluid == Fluids.EMPTY) {
         ...
         if (this.placeFluid(user, world, blockPos3, blockHitResult)) {
            ...
            user.incrementStat(Stats.USED.getOrCreateStat(this));
            ItemStack emptiedStack = getEmptiedStack(itemStack, user);
            ItemStack itemStack2 = ProtocolTranslator.getTargetVersion().olderThanOrEqualTo(ProtocolVersion.v1_20_5)
               ? emptiedStack
               : ItemUsage.exchangeStack(itemStack, user, emptiedStack);
   ...
   public boolean placeFluid(@Nullable PlayerEntity player, World world, BlockPos pos, @Nullable BlockHitResult hitResult) {
      if (!(this.fluid instanceof FlowableFluid flowableFluid)) {
         ...
         boolean bl2 = blockState.isAir()
            || bl
            || block instanceof FluidFillable fluidFillable && fluidFillable.canFillWithFluid(player, world, pos, blockState, this.fluid);
         if (!bl2) {
         ...
         if (world.getDimension().ultrawarm() && this.fluid.isIn(FluidTags.WATER)) {
            ...
            return true;
         } else if (block instanceof FluidFillable fluidFillable && this.fluid == Fluids.WATER) {
