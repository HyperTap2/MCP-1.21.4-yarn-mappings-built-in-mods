...
public class BlockPredicatesChecker {
   ...
   private static final Codec<BlockPredicatesChecker> FULL_CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
            ...
            Codec.BOOL.optionalFieldOf("show_in_tooltip", true).forGetter(BlockPredicatesChecker::showInTooltip)
         )
   ...
   private static List<Text> createTooltipText(List<BlockPredicate> blockPredicates) {
      ...
      return blockPredicates.stream()
         ...
         .distinct()
         .map(registryEntry -> registryEntry.value().getName().copy().formatted(Formatting.DARK_GRAY))
