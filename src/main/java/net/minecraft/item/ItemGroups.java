...
public class ItemGroups {
   ...
   private static ProtocolVersion viaLastTargetVersion;
   private static int viaLastCreativeFilterMode;
            ...
            .entries(
               (displayContext, entries) -> {
                  ...
                  displayContext.lookup()
                     ...
                     .ifPresent(
                        wrapper -> addInstruments(
                           entries, wrapper, Items.GOAT_HORN, InstrumentTags.GOAT_HORNS, ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS
                        )
                     );
         ...
         ItemGroup.create(ItemGroup.Row.BOTTOM, 2)
            ...
            .entries((displayContext, entries) -> {
               ...
               displayContext.lookup().getOptional(RegistryKeys.POTION).ifPresent(registryWrapper -> {
                  ...
                  addPotions(
                     entries, registryWrapper, Items.LINGERING_POTION, ItemGroup.StackVisibility.PARENT_AND_SEARCH_TABS, displayContext.enabledFeatures()
                  );
               });
            })
            .build()
   ...
   public static void collect() {
      ...
      for (RegistryKey<ItemGroup> registryKey : Registries.ITEM_GROUP.getKeys()) {
         ...
         String string2 = map.put(Pair.of(itemGroup.getRow(), itemGroup.getColumn()), string);
         if (string2 != null) {
   ...
   private static void addMaxLevelEnchantedBooks(
      ...
      registryWrapper.streamEntries()
         .map(enchantmentEntry -> EnchantmentHelper.getEnchantedBookWith(new EnchantmentLevelEntry(enchantmentEntry, enchantmentEntry.value().getMaxLevel())))
      ...
      registryWrapper.getOptional(instrumentTag)
         .ifPresent(
            entryList -> entryList.stream()
               .map(instrument -> GoatHornItem.getStackForInstrument(item, (RegistryEntry<Instrument>)instrument))
               .forEach(stack -> entries.add(stack, visibility))
         );
   }
      ...
      registryWrapper.streamEntries()
         ...
         .forEach(
            paintingVariantEntry -> {
               NbtComponent nbtComponent = ((NbtComponent)NbtComponent.DEFAULT
   ...
   public static boolean updateDisplayContext(FeatureSet enabledFeatures, boolean operatorEnabled, RegistryWrapper.WrapperLookup registries) {
      ...
      if (viaLastTargetVersion != targetVersion || viaLastCreativeFilterMode != filterMode) {
         viaLastTargetVersion = targetVersion;
         viaLastCreativeFilterMode = filterMode;
