...
public class HoneycombItem extends Item implements SignChangingItem {
   ...
   public ActionResult useOnBlock(ItemUsageContext context) {
      ...
      return getWaxedState(blockState).<ActionResult>map(state -> {
   ...
   public static Optional<BlockState> getWaxedState(BlockState state) {
      return Optional.ofNullable((Block)UNWAXED_TO_WAXED_BLOCKS.get().get(state.getBlock())).map(block -> ((Block)block).getStateWithProperties(state));
   }
