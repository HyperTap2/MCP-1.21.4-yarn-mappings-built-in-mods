...
public class ItemGroup {
   ...
   static class EntriesImpl implements ItemGroup.Entries {
      ...
      public void add(ItemStack stack, ItemGroup.StackVisibility visibility) {
         ...
         if (this.isItemEnabled(stack.getItem())) {
            switch (visibility) {
      ...
      private boolean isItemEnabled(Item item) {
         boolean enabled = item.isEnabled(this.enabledFeatures);
         int filterMode = GeneralSettings.INSTANCE.removeNotAvailableItemsFromCreativeTab.getIndex();
         MinecraftClient client = MinecraftClient.getInstance();
         if (filterMode == 2 || client == null || client.isInSingleplayer()) {
            return enabled;
         }

         Identifier groupId = Registries.ITEM_GROUP.getId(this.group);
         if (filterMode == 1 && groupId != null && !groupId.getNamespace().equals("minecraft")) {
            return enabled;
         }

         return ItemRegistryDiff.keepItem(item) && enabled;
      }
   }
