...
public class TradeOffer {
   public static final Codec<TradeOffer> CODEC = RecordCodecBuilder.create(
      instance -> instance.group(
            ...
            Codec.BOOL.lenientOptionalFieldOf("rewardExp", true).forGetter(tradeOffer -> tradeOffer.rewardingPlayerExperience),
            Codec.INT.lenientOptionalFieldOf("specialPrice", 0).forGetter(tradeOffer -> tradeOffer.specialPrice),
            Codec.INT.lenientOptionalFieldOf("demand", 0).forGetter(tradeOffer -> tradeOffer.demandBonus),
            Codec.FLOAT.lenientOptionalFieldOf("priceMultiplier", 0.0F).forGetter(tradeOffer -> tradeOffer.priceMultiplier),
   ...
   public TradeOffer(
      TradedItem firstBuyItem, Optional<TradedItem> secondBuyItem, ItemStack sellItem, int maxUses, int merchantExperience, float priceMultiplier
   ) {
   ...
   public boolean matchesBuyItems(ItemStack stack, ItemStack buyItem) {
      if (!this.firstBuyItem.matches(stack) || stack.getCount() < this.getFirstBuyItemCount(this.firstBuyItem)) {
         ...
         return !this.secondBuyItem.isPresent()
            ? buyItem.isEmpty()
            : this.secondBuyItem.get().matches(buyItem) && buyItem.getCount() >= this.secondBuyItem.get().count();
      }
   }
