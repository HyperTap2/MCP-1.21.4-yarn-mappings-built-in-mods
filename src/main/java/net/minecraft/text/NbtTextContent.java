         ...
         if (this.interpret) {
            ...
            return stream.flatMap(textx -> {
               try {
                  ...
                  LOGGER.warn("Failed to parse component: {}", textx, exception);
                  return Stream.of();
               }
            ...
            return Texts.parse(source, this.separator, sender, depth)
               .map(textx -> stream.map(Text::literal).reduce((accumulator, current) -> accumulator.append(textx).append(current)).orElseGet(Text::empty))
               .orElseGet(() -> Text.literal(stream.collect(Collectors.joining(", "))));
         }
      } else {
         return Text.empty();
      }
