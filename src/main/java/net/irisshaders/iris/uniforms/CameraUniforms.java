...
public class CameraUniforms {
   ...
   public static void addCameraUniforms(UniformHolder uniforms, FrameUpdateNotifier notifier) {
      CameraUniforms.CameraPositionTracker tracker = new CameraUniforms.CameraPositionTracker(notifier);
      uniforms.uniform1f(UniformUpdateFrequency.ONCE, "near", () -> 0.05)
         .uniform1f(UniformUpdateFrequency.PER_FRAME, "far", CameraUniforms::getRenderDistanceInBlocks)
         .uniform3d(UniformUpdateFrequency.PER_FRAME, "cameraPosition", tracker::getCurrentCameraPosition)
         .uniform1f(UniformUpdateFrequency.PER_FRAME, "eyeAltitude", tracker::getCurrentCameraPositionY)
         .uniform3d(UniformUpdateFrequency.PER_FRAME, "previousCameraPosition", tracker::getPreviousCameraPosition)
         .uniform3i(UniformUpdateFrequency.PER_FRAME, "cameraPositionInt", () -> getCameraPositionInt(getUnshiftedCameraPosition()))
         .uniform3f(UniformUpdateFrequency.PER_FRAME, "cameraPositionFract", () -> getCameraPositionFract(getUnshiftedCameraPosition()))
         .uniform3i(UniformUpdateFrequency.PER_FRAME, "previousCameraPositionInt", () -> getCameraPositionInt(tracker.getPreviousCameraPositionUnshifted()))
         .uniform3f(UniformUpdateFrequency.PER_FRAME, "previousCameraPositionFract", () -> getCameraPositionFract(tracker.getPreviousCameraPositionUnshifted()));
   }

   private static int getRenderDistanceInBlocks() {
      return client.options.getClampedViewDistance() * 16;
   }

   public static Vector3d getUnshiftedCameraPosition() {
      return JomlConversions.fromVec3(client.gameRenderer.getCamera().getPos());
   }
   ...
   static class CameraPositionTracker {
      ...
      private static double getShift(double value, double prevValue) {
         return !(Math.abs(value) > 30000.0) && !(Math.abs(value - prevValue) > 1000.0) ? 0.0 : -(value - value % 30000.0);
      }
