...
public class CompositeTransformer {
   private static final AutoHintedMatcher<Expression> glTextureMatrix0To7 = new AutoHintedMatcher<Expression>("gl_TextureMatrix[index]", ParseShape.EXPRESSION) {
      {
         this.markClassedPredicateWildcard(
            "index",
            ((Identifier)((Expression)this.pattern).getRoot().identifierIndex.getOne("index")).getAncestor(ReferenceExpression.class),
   ...
   public static void transform(ASTParser t, TranslationUnit tree, Root root, Parameters parameters) {
      ...
      if (parameters.type.glShaderType == ShaderType.VERTEX) {
         tree.parseAndInjectNodes(t, ASTInjectionPoint.BEFORE_DECLARATIONS, new String[]{"in vec3 Position;"});
      ...
      root.replaceReferenceExpressions(
         t, "gl_ProjectionMatrix", "mat4(vec4(2.0, 0.0, 0.0, 0.0), vec4(0.0, 2.0, 0.0, 0.0), vec4(0.0), vec4(-1.0, -1.0, 0.0, 1.0))"
      );
